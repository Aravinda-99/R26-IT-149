...
export class Level44Scene extends Phaser.Scene {
  ...
  init() {
    ...
    this.baseTutorialScene = "Level43Scene";
  }
  ...
  update(time, delta) {
    ...
    if (GameManager.interventionInFlight) {
      if (this._armTween && this._armTween.isPlaying()) {
        this._armTween.pause();
      }
      return; // Return early to stop particles and bulb flicker from animating
    } else {
      // Resume the round timer tween if the menu was just dismissed
      if (this._armTween && this._armTween.isPaused()) {
        this._armTween.resume();
      }
    }
  ...
  createPressroomDecor() {
    ...
    this.add.text(125, 218, "NO SMOKING", { font: "bold 11px Georgia", color: HEX_BRASS }).setOrigin(0.5).setAlpha(0.5).setDepth(2);
    ...
    [60, 1220].forEach((x) => {
      ...
      this.add.text(x, 560, "INK", { font: "bold 10px Georgia", color: HEX_BRASS }).setOrigin(0.5).setAlpha(0.4).setDepth(2);
    });
    ...
    this.add.text(400, 26, "PRESSROOM — LIVE", { font: "bold 16px Georgia", color: HEX_BRASS }).setOrigin(0.5).setAlpha(0.7).setDepth(2);
  ...
  createPressFrame() {
    ...
    this.add.text(220, 440, "MODEL C — 1892", { font: "bold 9px Georgia", color: HEX_BRASS }).setOrigin(0, 0.5).setAlpha(0.4).setDepth(5);
  ...
  createPressBed() {
    ...
    this.bedLabel = this.add.text(BED_X1 - 12, BED_Y0 + 10, "", { font: "bold 11px Courier New", color: HEX_GRAY }).setOrigin(1, 0).setAlpha(0.7).setDepth(6);
    this.bedQuestionText = this.add.text(BED_CX, BED_Y1 - 16, "", { font: "13px Arial", color: "#b0bec5" }).setOrigin(0.5).setDepth(6);
  ...
  createReferenceLog() {
    ...
    this.add.text(LOG_X + 32, LOG_Y + 20, "BROADCAST LOG — LIVE", { font: "bold 12px Arial", color: HEX_CYAN }).setOrigin(0, 0.5).setDepth(12);
  ...
  renderLogFromScratch() {
    ...
    this.rows.forEach((rowStr, i) => {
      ...
      const numT = this.add.text(LOG_X + 10, y, String(i + 1).padStart(2, "0"), { font: "12px Courier New", color: "#3d4450" }).setOrigin(0, 0.5);
  ...
  createHUD() {
    ...
    this.add.text(20, 14, "THE PRESSES", { font: "bold 17px Arial", color: "#b0bec5" }).setDepth(50);
    this.add.text(20, 32, "Tuning Phase — printf()", { font: "13px Arial", color: "#546e7a" }).setDepth(50);

    this.waveText = this.add.text(BED_CX, 18, "WAVE 1 / 3", { font: "bold 16px Arial", color: HEX_GOLD }).setOrigin(0.5).setDepth(50);
    this._waveSquares = [];
    for (let i = 0; i < 5; i++) {
  ...
  async stampBed(kind) {
    ...
    const stamp = this.add.text(BED_CX, BED_CY, labels[kind], { font: "bold 23px Arial", color: colors[kind] }).setOrigin(0.5).setDepth(30).setScale(1.4).setAngle(-6).setAlpha(0);
  ...
  async renderFormatOnBed(fmtDisplaySource) {
    ...
    while (i < inner.length) {
      ...
      if (inner[i] === "\\" && inner[i + 1] === "n") {
        const t = this.add.text(x, y, "\\n", { font: "bold 17px Courier New", color: HEX_MAGENTA }).setOrigin(0, 0.5).setAlpha(0);
      ...
      const t = this.add.text(x, y, ch, { font: "bold 20px Courier New", color }).setOrigin(0, 0.5).setAlpha(0);
      this.bedContainer.add(t);
  ...
  _makeBedSlot(label, specifier, precision, x, y) {
    ...
    const tmp = this.add.text(0, 0, label, { font: "bold 19px Courier New" });
    ...
    const txt = this.add.text(0, 0, label, { font: "bold 19px Courier New", color: colorHex }).setOrigin(0.5);
    c.add([g, txt]);
    this.bedContainer.add(c);
    this.tweens.add({ targets: c, scale: 1, alpha: 1, duration: 160, ease: "Back.easeOut" });
    if (precision !== null && precision !== undefined) {
      const pm = this.add.text(x + w / 2, y - 22, `.${precision}`, { font: "bold 11px Courier New", color: "#ff6f00" }).setOrigin(0.5).setAlpha(0);
  ...
  async renderSlugsOnRail(argVals) {
    ...
    for (let idx = 0; idx < argVals.length; idx++) {
      ...
      const tmp = this.add.text(0, 0, display, { font: "bold 17px Courier New" });
      const w = tmp.width + 14;
      tmp.destroy();
      ...
      const txt = this.add.text(0, 0, display, { font: "bold 17px Courier New", color }).setOrigin(0.5);
      c.add([g, txt]);
  ...
  setupBugHunt(config) {
    this.clearBed();
    const header = this.add.text(BED_CX, BED_Y0 + 14, "CLICK THE BUG", { font: "bold 15px Arial", color: HEX_MAGENTA }).setOrigin(0.5);
    ...
    config.lines.forEach((line, li) => {
      ...
      if (line.trim().startsWith("//")) {
        const t = this.add.text(BED_CX, y, line, { font: "italic 13px Courier New", color: "#546e7a" }).setOrigin(0.5);
        this.bedContainer.add(t);
        return;
      }
      const tokens = this._syntaxTokenize(line);
  ...
  scoreForAttempt(timePctUsed) {
    ...
    if (remaining > 0.6) { points += 50; this.fastBonusCount++; this.createFloatingText(BED_CX, BED_Y0 - 20, "⚡ CLEAN CATCH +50", HEX_GOLD, "bold 15px Arial", 900); }
    else if (remaining > 0.3) { points += 25; this.fastBonusCount++; this.createFloatingText(BED_CX, BED_Y0 - 20, "⚡ +25", HEX_GOLD, "bold 14px Arial", 800); }
    return points;
  }
  ...
  async runBehavioralCheck() {
    ...
    try {
      ...
      console.warn("Level44Scene: /api/wellbeing/predict-struggle unreachable, skipping behavioral signal for this level:", e);
    }
  }
  ...
  gameOver() {
    ...
    (async () => {
      ...
      this.add.text(640, 350, `Impressions Cleared: ${this.currentRound} / 15`, { font: "18px Arial", color: HEX_GRAY }).setOrigin(0.5).setDepth(91);
      this._makeButton(525, 420, "CLEAR THE JAM", 200, 50, { stroke: C_RED, textColor: HEX_RED }, () => this.scene.restart());
  ...
  levelComplete() {
    ...
    try { GameManager.completeLevel(44, Math.round((this.correctFirstTry / 15) * 100)); } catch (_) {}
  ...
  showScoreTally() {
    ...
    const sluglet = this.add.text(0, 3, "%d", { font: "bold 12px Courier New", color: HEX_GOLD }).setOrigin(0.5);
    badge.add([bg, sluglet]);
    this.tweens.add({ targets: badge, alpha: 1, duration: 300, delay: 2000 });
    const badgeLbl = this.add.text(640, 458, "printf() SCHEMA TUNED", { font: "bold 15px Arial", color: HEX_GOLD }).setOrigin(0.5).setDepth(91).setAlpha(0);
