...
export class Level43Scene extends Phaser.Scene {
  ...
  createRoomDecor() {
    ...
    this.add.text(400, 26, "COMPOSING ROOM", { font: "bold 16px Georgia", color: HEX_BRASS }).setOrigin(0.5).setAlpha(0.6).setDepth(3);
  ...
  createComposingTray() {
    ...
    this.trayLabel = this.add.text(50, 172, "COMPOSING TRAY", { font: "bold 12px Georgia", color: HEX_BRASS }).setAlpha(0.7).setDepth(6);
  ...
  createTypeCase() {
    ...
    this.add.text(50, 387, "TYPE CASE", { font: "bold 12px Georgia", color: HEX_BRASS }).setAlpha(0.7).setDepth(5);
  ...
  createBroadcastLog() {
    ...
    this.add.text(LOG_X + 40, LOG_Y + 22, "BROADCAST LOG", { font: "bold 13px Arial", color: "#00e5ff" }).setOrigin(0, 0.5).setDepth(12);
  ...
  renderLogFromScratch() {
    ...
    this.rows.forEach((rowStr, i) => {
      ...
      const numT = this.add.text(LOG_X + 10, y, String(i + 1).padStart(2, "0"), { font: "13px Courier New", color: "#3d4450" }).setOrigin(0, 0.5);
      const textT = this.add.text(LOG_TEXT_X, y, this._displayRow(rowStr), { font: "bold 18px Courier New", color: HEX_CYAN }).setOrigin(0, 0.5);
  ...
  ensureRow(idx) {
    while (this.rowObjs.length <= idx) {
      ...
      const y = this._rowY(i);
      const numT = this.add.text(LOG_X + 10, y, String(i + 1).padStart(2, "0"), { font: "13px Courier New", color: "#3d4450" }).setOrigin(0, 0.5);
      const textT = this.add.text(LOG_TEXT_X, y, "", { font: "bold 18px Courier New", color: HEX_CYAN }).setOrigin(0, 0.5).setAlpha(0);
  ...
  createExpressionMonitor() {
    ...
    this.monitorText = this.add.text(W / 2, 32, "", { font: "15px Courier New", color: "#e8dfc8" }).setOrigin(0.5).setDepth(51);
  }
  ...
  showCompileErrorStamp() {
    const stamp = this.add.text(380, TRAY_Y, "COMPILE ERROR", { font: "bold 23px Arial", color: HEX_RED }).setOrigin(0.5).setDepth(80).setScale(1.6).setAngle(-6).setAlpha(0);
  ...
  async showTrayErrorStamp(text) {
    const stamp = this.add.text(380, TRAY_Y, text, { font: "bold 16px Courier New", color: HEX_RED }).setOrigin(0.5).setDepth(80).setAngle(-8).setAlpha(0);
    this.trayContainer.add(stamp);
  ...
  createHUD() {
    ...
    this.add.text(20, 14, "THE COMPOSING ROOM", { font: "bold 17px Arial", color: "#b0bec5" }).setDepth(50);
    this.add.text(20, 32, "Accretion Phase — printf()", { font: "13px Arial", color: "#546e7a" }).setDepth(50);
  ...
  async renderFormatOnTray(fmtDisplaySource) {
    ...
    while (i < inner.length) {
      ...
      if (inner[i] === "\\" && inner[i + 1] === "n") {
        const t = this.add.text(x, y, "\\n", { font: "bold 18px Courier New", color: HEX_MAGENTA }).setOrigin(0, 0.5).setAlpha(0);
      ...
      const t = this.add.text(x, y, ch, { font: "bold 23px Courier New", color }).setOrigin(0, 0.5).setAlpha(0);
      this.trayContainer.add(t);
  ...
  _makeTraySlot(label, specifier, precision, x, y) {
    ...
    if (precision !== null && precision !== undefined) {
      const pm = this.add.text(x + w / 2, y - 24, `.${precision}`, { font: "bold 12px Courier New", color: "#ff6f00" }).setOrigin(0.5).setAlpha(0);
  ...
  async renderSlugsInCase(argVals) {
    ...
    for (let idx = 0; idx < argVals.length; idx++) {
      ...
      const tmp = this.add.text(0, 0, display, { font: "bold 19px Courier New" });
      const w = tmp.width + 16;
      tmp.destroy();
      ...
      const txt = this.add.text(0, -6, display, { font: "bold 19px Courier New", color }).setOrigin(0.5);
      const idxLbl = this.add.text(0, 16, `arg${idx + 1}`, { font: "bold 10px Courier New", color: HEX_BRASS }).setOrigin(0.5);
      c.add([g, txt, idxLbl]);
  ...
  showQuestionCard(promptText) {
    ...
    const badgeT = this.add.text(-230, -10, String(this.currentRound + 1), { font: "bold 16px Arial", color: "#0f0a06" }).setOrigin(0.5);
  ...
  showOptionBubbles(options, config) {
    ...
    shuffled.forEach((opt, i) => {
      ...
      const txt = this.add.text(0, 0, label, { font: "bold 16px Courier New", color: "#e8dfc8" }).setOrigin(0.5);
  ...
  createCartridgeTray(config) {
    ...
    bdraw(false, false);
    const bt = this.add.text(0, 0, "COMPOSE", { font: "bold 16px Arial", color: "#0f0a06" }).setOrigin(0.5);
    btn.add([bg, bt]);
    btn.setSize(140, 44);
  ...
  async runBehavioralCheck() {
    ...
    try {
      ...
      console.warn("Level43Scene: /api/wellbeing/predict-struggle unreachable, skipping behavioral signal for this level:", e);
    }
  }
  ...
  gameOver() {
    ...
    (async () => {
      ...
      this._makeButton(525, 420, "BACK TO THE TRAY", 200, 50, { stroke: C_RED, textColor: HEX_RED }, () => this.scene.restart());
  ...
  levelComplete() {
    ...
    try { GameManager.completeLevel(43, Math.round((this.correctFirstTry / 12) * 100)); } catch (_) {}
