...
export class Level45Scene extends Phaser.Scene {
  ...
  createPrintFloorInterior() {
    ...
    this.add.text(400, 26, "PRINT FLOOR — PRODUCTION", { font: "bold 16px Georgia", color: HEX_BRASS }).setOrigin(0.5).setAlpha(0.7).setDepth(3);
  }
  ...
  createProductionSign() {
    ...
    this.productionText = this.add.text(890, 62, "IN PRODUCTION", { font: "bold 12px Arial", color: "#e53935" }).setOrigin(0.5).setAlpha(0.35).setDepth(5);
  ...
  createCodeCanvas() {
    ...
    this.tabFilename = this.add.text(CX + CW - 12, CY + TAB_H / 2, "Press1.java", { font: "13px Courier New", color: "#546e7a" }).setOrigin(1, 0.5).setDepth(11);
  ...
  createBlockPalette() {
    ...
    this.add.text(PX + 10, PY + 8, "PRESS COPY", { font: "bold 11px Arial", color: "#3d4450" }).setDepth(11);
  ...
  createRigWindow() {
    ...
    this.add.text(OX + 10, OY + 6, "PRODUCTION RIG — LIVE", { font: "bold 11px Arial", color: HEX_BRASS }).setAlpha(0.7).setDepth(11);
  ...
  renderLogFromScratch() {
    ...
    this.rows.forEach((rowStr, i) => {
      ...
      const textT = this.add.text(LOG_X0, y, this._displayRow(rowStr), { font: "bold 13px Courier New", color: HEX_CYAN }).setOrigin(0, 0.5);
  ...
  ensureRow(idx) {
    while (this.rowObjs.length <= idx) {
      ...
      const y = this._rowY(i);
      const textT = this.add.text(LOG_X0, y, "", { font: "bold 13px Courier New", color: HEX_CYAN }).setOrigin(0, 0.5).setAlpha(0);
  ...
  async animateComposingBed(fmtDisplaySource, argVals, evalResult) {
    ...
    for (const av of argVals) {
      ...
      const t = this.add.text(rx, RAIL_Y, display, { font: "bold 12px Courier New", color }).setOrigin(0, 0.5).setAlpha(0);
  ...
  async showCompileErrorOnBed() {
    this.clearBed();
    const stamp = this.add.text(OX + OW / 2, BED_Y, "COMPILE ERROR", { font: "bold 16px Arial", color: HEX_RED }).setOrigin(0.5).setAlpha(0);
  ...
  async showCrashStamp(crash) {
    ...
    const stamp = this.add.text(OX + OW / 2, RAIL_Y + 10, labels[crash] || "RUNTIME CRASH", { font: "bold 12px Courier New", color: HEX_RED }).setOrigin(0.5).setAlpha(0);
  ...
  buildReportRows(mission) {
    ...
    mission.tests.forEach((test, i) => {
      ...
      const expT = this.add.text(200, 0, expected, { font: "11px Courier New", color: "#78909c" }).setOrigin(0, 0.5);
      const statusT = this.add.text(RW - 16, 0, "…", { font: "15px Arial", color: "#78909c" }).setOrigin(0.5);
  ...
  createHUD() {
    ...
    this.add.text(20, 14, "THE PRINT FLOOR", { font: "bold 15px Arial", color: "#b0bec5" }).setDepth(51);
    this.add.text(20, 32, "Restructuring Phase — printf()", { font: "12px Arial", color: "#546e7a" }).setDepth(51);
  ...
  createAnnotation(x, y, text, colorHex) {
    const t = this.add.text(x, y, text, { font: "bold 10px Arial", color: colorHex }).setOrigin(0.5).setDepth(21).setAlpha(0);
    this.rigLayer.add(t);
  ...
  async runBehavioralCheck() {
    ...
    try {
      ...
      console.warn("Level45Scene: /api/wellbeing/predict-struggle unreachable, skipping behavioral signal for this level:", e);
    }
  }
  ...
  gameOver() {
    ...
    this._makeButton(525, 420, "RESUME PRODUCTION", 200, 50, { stroke: C_RED, textColor: HEX_RED }, () => this.scene.restart());
  ...
  levelComplete() {
    ...
    try { GameManager.completeLevel(45, Math.round((this.flawlessCount / MISSIONS.length) * 100)); } catch (_) {}
  ...
  async ceremonyPhase3_CentralPanel() {
    ...
    const totalText = this.add.text(480, 148 + 6 * 24, "TOTAL: 0", { font: "bold 21px Arial", color: HEX_GOLD }).setOrigin(0, 0.5).setDepth(91).setAlpha(0);
  ...
  async ceremonyPhase4_WingSeal() {
    ...
    const wingTitle = this.add.text(0, -20, "OUTPUT WING — COMPLETE", { font: "bold 19px Georgia", color: HEX_GOLD }).setOrigin(0.5);
    ...
    const caption = this.add.text(640, 600, "9 levels · 3 methods · 1 production-ready output schema", { font: "italic 13px Georgia", color: HEX_BRASS }).setOrigin(0.5).setDepth(93).setAlpha(0);
