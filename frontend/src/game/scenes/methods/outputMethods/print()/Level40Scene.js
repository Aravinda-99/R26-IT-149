...
export class Level40Scene extends Phaser.Scene {
  ...
  createWhisperBooth() {
    ...
    this.add.text(100, 168, "RECORDING", { font: "bold 12px Arial", color: "#e53935" }).setOrigin(0.5).setAlpha(0.35).setDepth(3);
  }
  ...
  createWhisperDesk() {
    ...
    this.add.text(DESK_X0 + 10, DESK_Y0 + 8, "WHISPER TX", { font: "bold 12px Courier New", color: HEX_VIOLET }).setDepth(5);
  ...
  async assembleArgumentDisplay(displayText, isString) {
    ...
    const cy = MARQUEE_Y;
    let x = MARQUEE_X0 + 10;
    if (isString) { const q = this.add.text(x, cy, '"', { font: "bold 18px Courier New", color: HEX_GRAY }).setOrigin(0, 0.5); this.marqueeContainer.add(q); x += q.width; }
    let i = 0;
    while (i < displayText.length) {
      ...
      const t = this.add.text(x, cy, glyph, { font: "bold 19px Courier New", color }).setOrigin(0, 0.5).setAlpha(0);
    ...
    if (isString) { const q = this.add.text(x, cy, '"', { font: "bold 18px Courier New", color: HEX_GRAY }).setOrigin(0, 0.5); this.marqueeContainer.add(q); }
    await this.delay(250);
  }

  async showNoArgumentError() {
    this.clearMarquee();
    const t = this.add.text(MARQUEE_X0 + (MARQUEE_X1 - MARQUEE_X0) / 2, MARQUEE_Y, "NO ARGUMENT", { font: "bold 16px Arial", color: HEX_RED }).setOrigin(0.5).setAlpha(0);
    this.marqueeContainer.add(t);
  ...
  renderLogFromScratch() {
    ...
    this.rows.forEach((rowStr, i) => {
      ...
      const numT = this.add.text(LOG_X + 12, y, String(i + 1).padStart(2, "0"), { font: "13px Courier New", color: "#3d4450" }).setOrigin(0, 0.5);
      const textT = this.add.text(LOG_TEXT_X, y, this._displayRow(rowStr), { font: "bold 19px Courier New", color: HEX_CYAN }).setOrigin(0, 0.5);
  ...
  ensureRow(idx) {
    while (this.logRowObjs.length <= idx) {
      ...
      const y = this._rowY(i);
      const numT = this.add.text(LOG_X + 12, y, String(i + 1).padStart(2, "0"), { font: "13px Courier New", color: "#3d4450" }).setOrigin(0, 0.5);
      const textT = this.add.text(LOG_TEXT_X, y, "", { font: "bold 19px Courier New", color: HEX_CYAN }).setOrigin(0, 0.5).setAlpha(0);
  ...
  showCompileErrorStamp() {
    const stamp = this.add.text(420, SRC_Y + 40, "COMPILE ERROR", { font: "bold 23px Arial", color: HEX_RED }).setOrigin(0.5).setDepth(80).setScale(1.6).setAngle(-6).setAlpha(0);
  ...
  createAnnotation(x, y, text, colorHex) {
    const t = this.add.text(x, y, text, { font: "italic 13px Arial", color: colorHex }).setOrigin(0.5).setDepth(70).setAlpha(0);
  ...
  async fireCall(method, argExpr, decls) {
    ...
    if (!evalResult.ok) {
      if (evalResult.noArgPrint) await this.showNoArgumentError();
        ...
        this.clearMarquee();
        const t = this.add.text(MARQUEE_X0 + 100, MARQUEE_Y, "?", { font: "bold 19px Courier New", color: HEX_RED }).setOrigin(0.5);
        this.marqueeContainer.add(t);
        this.screenShake(0.004, 150);
        this.flareMicLED();
  ...
  showQuestionCard(promptText) {
    ...
    const badgeT = this.add.text(-230, -10, String(this.currentRound + 1), { font: "bold 16px Arial", color: "#0a0817" }).setOrigin(0.5);
  ...
  createCartridgeTray(config) {
    ...
    bdraw(false, false);
    const bt = this.add.text(0, 0, "WHISPER", { font: "bold 16px Arial", color: "#0a0817" }).setOrigin(0.5);
    btn.add([bg, bt]);
    btn.setSize(140, 44);
  ...
  async onWhisperPressed(config) {
    ...
    if (config.revealNote) this.createFloatingText(LOG_X + LOG_W / 2, LOG_Y + LOG_H + 30, config.revealNote, HEX_GRAY, "13px Arial", 3000);
    await this.delay(500);
    if (!this._alive) return;
  ...
  async runBehavioralCheck() {
    ...
    try {
      ...
      console.warn("Level40Scene: /api/wellbeing/predict-struggle unreachable, skipping behavioral signal for this level:", e);
    }
  }
  ...
  gameOver() {
    ...
    (async () => {
      ...
      this._makeButton(525, 420, "MIC CHECK", 200, 50, { stroke: C_RED, textColor: HEX_RED }, () => this.scene.restart());
  ...
  showScoreTally() {
    ...
    const badgeLbl = this.add.text(640, 445, "print() SCHEMA ACQUIRED", { font: "bold 15px Arial", color: HEX_GOLD }).setOrigin(0.5).setDepth(91).setAlpha(0);
