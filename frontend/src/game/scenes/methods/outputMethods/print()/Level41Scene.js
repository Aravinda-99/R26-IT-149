...
export class Level41Scene extends Phaser.Scene {
  ...
  createPreviewMonitor() {
    ...
    this.add.text(MON_X, MON_Y + MON_H / 2 + 32, "BCAST-A", { font: "bold 10px Courier New", color: HEX_GRAY }).setOrigin(0.5).setDepth(11);
    ...
    this.roundLabel = this.add.text(MON_X + MON_W / 2 - 8, MON_Y - MON_H / 2 + 8, "", { font: "bold 11px Courier New", color: HEX_GRAY }).setOrigin(1, 0).setAlpha(0.7).setDepth(14);
    this.monLayer.add(this.roundLabel);
  }

  createAirTimeBar() {
    this.add.text(BAR_X0, 138, "AIR TIME", { font: "bold 12px Arial", color: HEX_GRAY }).setDepth(10);
    this.airTimeReadout = this.add.text(700, 138, "", { font: "bold 13px Courier New", color: "#e0e0e0" }).setDepth(10);
  ...
  async stampMonitor(kind) {
    ...
    const stamp = this.add.text(MON_X, MON_Y, label, { font: "bold 21px Arial", color }).setOrigin(0.5).setAngle(-6).setScale(1.5).setAlpha(0).setDepth(30);
    this.monLayer.add(stamp);
  ...
  showTrialOnMonitor(lines, questionText) {
    ...
    if (questionText) {
      const qt = this.add.text(MON_X, MON_Y + 55, questionText, { font: "14px Arial", color: "#b0bec5" }).setOrigin(0.5).setAlpha(0);
  ...
  async assembleArgumentDisplay(displayText, isString) {
    ...
    const cy = MARQUEE_Y;
    let x = MARQUEE_X0 + 6;
    if (isString) { const q = this.add.text(x, cy, '"', { font: "bold 15px Courier New", color: HEX_GRAY }).setOrigin(0, 0.5); this.marqueeContainer.add(q); x += q.width; }
    let i = 0;
    while (i < displayText.length) {
      ...
      const t = this.add.text(x, cy, glyph, { font: "bold 15px Courier New", color }).setOrigin(0, 0.5).setAlpha(0);
    ...
    if (isString) { const q = this.add.text(x, cy, '"', { font: "bold 15px Courier New", color: HEX_GRAY }).setOrigin(0, 0.5); this.marqueeContainer.add(q); }
    await this.delay(180);
  }

  async showNoArgumentError() {
    this.clearMarquee();
    const t = this.add.text(MARQUEE_X0 + (MARQUEE_X1 - MARQUEE_X0) / 2, MARQUEE_Y, "NO ARG", { font: "bold 13px Arial", color: HEX_RED }).setOrigin(0.5).setAlpha(0);
    this.marqueeContainer.add(t);
  ...
  createHUD() {
    ...
    this.add.text(20, 14, "THE LIVE FEED", { font: "bold 17px Arial", color: "#b0bec5" }).setDepth(50);
    this.add.text(20, 32, "Tuning Phase — print()", { font: "13px Arial", color: "#546e7a" }).setDepth(50);
  ...
  async fireCall(method, argExpr, decls) {
    ...
    if (!evalResult.ok) {
      if (evalResult.noArgPrint) await this.showNoArgumentError();
        ...
        this.clearMarquee();
        const t = this.add.text(MARQUEE_X0 + 80, MARQUEE_Y, "?", { font: "bold 18px Courier New", color: HEX_RED }).setOrigin(0.5);
        this.marqueeContainer.add(t);
        this.screenShake(0.004, 150);
        this.flareTallyLight();
  ...
  showCompileErrorStamp() {
    const stamp = this.add.text(MON_X, MON_Y - MON_H / 2 - 26, "COMPILE ERROR", { font: "bold 18px Arial", color: HEX_RED }).setOrigin(0.5).setDepth(80).setScale(1.3).setAngle(-6).setAlpha(0);
    this.monLayer.add(stamp);
    this.tweens.add({ targets: stamp, scale: 1, alpha: 1, duration: 180 });
  ...
  setupBugHunt(config) {
    ...
    const header = this.add.text(MON_X, MON_Y - MON_H / 2 + 14, "CLICK THE BUG", { font: "bold 15px Arial", color: HEX_MAGENTA }).setOrigin(0.5);
    ...
    config.lines.forEach((line, li) => {
      ...
      const t = this.add.text(MON_X, y, line, { font: "bold 16px Courier New", color: "#e0e0e0" }).setOrigin(0.5, 0.5).setInteractive({ useHandCursor: true });
  ...
  async onTokenClicked(tokenObj, config) {
    ...
    if (correct) {
      ...
      const fixed = this.add.text(tokenObj.x, tokenObj.y - 20, config.fixedToken, { font: "bold 14px Courier New", color: HEX_GREEN_BRIGHT }).setOrigin(0.5).setAlpha(0);
  ...
  scoreForAttempt(timePctUsed) {
    ...
    if (remaining > 0.6) { points += 50; this.fastBonusCount++; this.createFloatingText(MON_X, MON_Y - MON_H / 2 - 20, "⚡ CLEAN CLEAR +50", HEX_GOLD, "bold 15px Arial", 900); }
    else if (remaining > 0.3) { points += 25; this.fastBonusCount++; this.createFloatingText(MON_X, MON_Y - MON_H / 2 - 20, "⚡ +25", HEX_GOLD, "bold 14px Arial", 800); }
    return points;
  }
  ...
  async runBehavioralCheck() {
    ...
    try {
      ...
      console.warn("Level41Scene: /api/wellbeing/predict-struggle unreachable, skipping behavioral signal for this level:", e);
    }
  }
  ...
  gameOver() {
    ...
    (async () => {
      ...
      this.add.text(640, 350, `Items Cleared: ${this.currentRound} / 15`, { font: "18px Arial", color: HEX_GRAY }).setOrigin(0.5).setDepth(91);
      this._makeButton(525, 420, "BACK TO PRODUCTION", 200, 50, { stroke: C_RED, textColor: HEX_RED }, () => this.scene.restart());
  ...
  levelComplete() {
    ...
    try { GameManager.completeLevel(41, Math.round((this.correctFirstTry / 15) * 100)); } catch (_) {}
