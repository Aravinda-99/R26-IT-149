...
export class Level42Scene extends Phaser.Scene {
  ...
  createBlockPalette() {
    ...
    this.add.text(PX + 10, PY + 8, "NEWS COPY", { font: "bold 11px Arial", color: "#3d4450" }).setDepth(11);
  ...
  createRigWindow() {
    ...
    this.add.text(OX + 10, OY + 6, "NEWSROOM RIG — LIVE", { font: "bold 11px Arial", color: "#3d4450" }).setDepth(11);
  ...
  async assembleArgumentDisplay(displayText, isString) {
    ...
    const cy = MARQUEE_Y;
    let x = MARQUEE_X0 + 4;
    if (isString) { const q = this.add.text(x, cy, '"', { font: "bold 12px Courier New", color: "#78909c" }).setOrigin(0, 0.5); this.marqueeContainer.add(q); x += q.width; }
    let i = 0;
    while (i < displayText.length) {
      ...
      const t = this.add.text(x, cy, glyph, { font: "bold 12px Courier New", color }).setOrigin(0, 0.5).setAlpha(0);
    ...
    if (isString) { const q = this.add.text(x, cy, '"', { font: "bold 12px Courier New", color: "#78909c" }).setOrigin(0, 0.5); this.marqueeContainer.add(q); }
  ...
  renderMiniTape() {
    ...
    this.tapeState.slice(-Math.floor(totalW / cellW)).forEach((cell, i) => {
      ...
      const t = this.add.text(x, TAPE_Y, disp, { font: "bold 10px Courier New", color }).setOrigin(0.5);
  ...
  async fireCall(method, argExpr, decls) {
    ...
    if (!evalResult.ok) {
      this.clearMarquee();
      const t = this.add.text(MARQUEE_X0 + 30, MARQUEE_Y, "?", { font: "bold 15px Courier New", color: HEX_RED }).setOrigin(0.5);
      this.marqueeContainer.add(t);
  ...
  gameOver() {
    ...
    this._makeButton(525, 420, "BACK TO THE DESK", 200, 50, { stroke: C_RED, textColor: HEX_RED }, () => this.scene.restart());
  ...
  showScoreTally() {
    ...
    lines.forEach((s, i) => {
      const t = this.add.text(500, 148 + i * 25, s, { font: "15px Arial", color: HEX_GRAY }).setOrigin(0, 0.5).setDepth(91).setAlpha(0);
      this.tweens.add({ targets: t, alpha: 1, duration: 250, delay: 300 + i * 130 });
    });
    const totalText = this.add.text(500, 148 + 5 * 25, "TOTAL: 0", { font: "bold 21px Arial", color: HEX_GOLD }).setOrigin(0, 0.5).setDepth(91).setAlpha(0);
    ...
    const wingLbl = this.add.text(640, barY - 16, "OUTPUT WING — 2 of 3 trilogies complete", { font: "bold 13px Arial", color: "#8c7ae6" }).setOrigin(0.5).setDepth(91).setAlpha(0);
    const tickLabels = ["println ✓", "print ✓", "printf"];
    const tickTexts = tickLabels.map((lbl, i) => this.add.text(640 - 150 + i * 150, barY + 28, lbl, { font: "12px Arial", color: i < 2 ? HEX_GREEN_BRIGHT : "#546e7a" }).setOrigin(0.5).setDepth(91).setAlpha(0));
