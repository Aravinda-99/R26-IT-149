...
const MY = OY + 75;
const TOWER_X = OX + 40, TOWER_TOP_Y = OY + 35;
const MARQUEE_X = OX + 165, MARQUEE_Y = MY;
const TAPE_X0 = OX + 20, TAPE_X1 = OX + 110, TAPE_Y = OY + 185;
const CONT_X = OX + 60, CONT_Y0 = OY + 145;
const LOG_X0 = OX + 280, LOG_Y0 = OY + 35, LOG_W0 = 170;
const PRESS_X = OX + 400, PRESS_Y = OY + 185;
...
export class Level39Scene extends Phaser.Scene {
  ...
  createOnAirSign() {
    const c = this.add.container(890, 60).setDepth(20);
    const g = this.add.graphics();
    g.fillStyle(0x0d0d1f, 1);
    g.fillRoundedRect(-45, -12, 90, 24, 4);
    g.lineStyle(1.5, 0xe53935, 1);
    g.strokeRoundedRect(-45, -12, 90, 24, 4);
    const t = this.add.text(0, 0, "ON AIR", { font: "bold 14px Arial", color: "#e53935" }).setOrigin(0.5);
    c.add([g, t]);
    c.setAlpha(0.3);
    this.onAirSign = c;
    ...
    mission.skeleton.forEach((rawLine, i) => {
      ...
      for (let pi = 0; pi < parts.length; pi++) {
        ...
        if (!isSlotToken && !isSubToken) {
          ...
          const t = this.add.text(x, y, String(val), { font: "bold 14px Courier New", color: "#546e7a" }).setAlpha(0.7);
          this.codeContainer.add(t);
          x += t.width;
        } else if (isSlotToken) {
          const slotId = part;
  ...
  _drawSlotPlaceholder(slotId) {
    ...
    if (!filled) {
      const label = this.add.text(x + w / 2, y + h / 2, def.id, { font: "italic 11px Courier New", color: "#3d4450" }).setOrigin(0.5).setDepth(22);
      def.hintLabel = label;
  ...
  createBlockPalette() {
    ...
    this.add.text(PX + 10, PY + 8, "STUDIO PARTS", { font: "bold 11px Arial", color: "#3d4450" }).setDepth(11);
  ...
  createRigWindow() {
    ...
    this.add.text(OX + 10, OY + 6, "BROADCAST RIG — LIVE", { font: "bold 11px Arial", color: "#3d4450" }).setDepth(11);
  ...
  addLogLine(text, styleType) {
    ...
    const t = this.add.text(LOG_X0 + 6, y, text, { font: "12px Courier New", color: this._typeColor(styleType) }).setAlpha(0).setScale(0.7);
    this.logLayer.add(t);
  ...
  createMiniCrossWingCameos() {
    ...
    ["int", "string"].forEach((key, i) => {
      ...
      const valT = this.add.text(0, 0, "", { font: "10px Courier New", color: Phaser.Display.Color.IntegerToColor(color).rgba }).setOrigin(0.5);
      c.add([g, valT]);
      this.rigLayer.add(c);
  ...
  renderMiniTape(cells) {
    ...
    cells.forEach((cell, i) => {
      ...
      const t = this.add.text(startX + i * cellW, TAPE_Y, disp, { font: "10px Courier New", color }).setOrigin(0, 0.5);
  ...
  buildReportRows(tests) {
    ...
    tests.forEach((test, i) => {
      ...
      const arrow = this.add.text(150, 0, "→", { font: "14px Arial", color: HEX_GRAY }).setOrigin(0, 0.5);
      const expT = this.add.text(168, 0, test.expectedOutput.replace(/⏎/g, " ⏎ "), { font: "11px Courier New", color: HEX_GRAY, wordWrap: { width: 190 } }).setOrigin(0, 0.5);
  ...
  createHUD() {
    ...
    this.add.text(20, 14, "THE STUDIO", { font: "bold 15px Arial", color: "#b0bec5" }).setDepth(51);
    this.add.text(20, 32, "Restructuring Phase — Output Methods: println()", { font: "12px Arial", color: "#546e7a" }).setDepth(51);
  ...
  floatingAnnotation(x, y, text, colorHex) {
    ...
    const t = this.add.text(x, y, text, { font: "bold 13px Arial", color: c }).setOrigin(0.5).setDepth(70).setAlpha(0);
  ...
  async playReveal(mission, test, result) {
    ...
    if (result.crash) {
      ...
      const stamp = this.add.text(OX + OW / 2, OY + OH - 30, label, { font: "bold 12px Courier New", color: HEX_RED }).setOrigin(0.5).setDepth(30);
  ...
  async assembleAndFire(text, type) {
    this.clearMarquee();
    const t = this.add.text(MARQUEE_X, MARQUEE_Y, text || "⏎", { font: "bold 12px Courier New", color: this._typeColor(type === "int" ? "int" : "string") }).setOrigin(0.5).setAlpha(0).setScale(0.6);
  ...
  levelComplete() {
    ...
    try { GameManager.completeLevel(39, Math.round((this.flawlessCount / MISSIONS.length) * 100)); } catch (_) {}
  ...
  showScoreTally() {
    ...
    lines.forEach((s, i) => {
      const t = this.add.text(500, 140 + i * 26, s, { font: "16px Arial", color: HEX_GRAY }).setOrigin(0, 0.5).setDepth(91).setAlpha(0);
      this.tweens.add({ targets: t, alpha: 1, duration: 250, delay: 300 + i * 130 });
    });
    const totalText = this.add.text(500, 140 + 5 * 26, "TOTAL: 0", { font: "bold 23px Arial", color: HEX_GOLD }).setOrigin(0, 0.5).setDepth(91).setAlpha(0);
    ...
    const badgeLbl = this.add.text(640, 440, "println() MASTERY", { font: "bold 16px Arial", color: HEX_GOLD }).setOrigin(0.5).setDepth(91).setAlpha(0);
    const badgeSub = this.add.text(640, 458, "Accretion ✓  Tuning ✓  Restructuring ✓", { font: "12px Arial", color: "#78909c" }).setOrigin(0.5).setDepth(91).setAlpha(0);
    ...
    this.add.text(640, barY - 16, "OUTPUT WING — 1 of 3 trilogies complete", { font: "bold 13px Arial", color: HEX_PURPLE }).setOrigin(0.5).setDepth(91).setAlpha(0);
    const trilogyLabels = this.add.container(0, 0).setDepth(92).setAlpha(0);
    ["println ✓", "print", "printf"].forEach((label, i) => {
      const t = this.add.text(490 + i * 150, barY + 26, label, { font: "12px Arial", color: i === 0 ? HEX_GOLD : "#546e7a" }).setOrigin(0.5);
      trilogyLabels.add(t);
    });
