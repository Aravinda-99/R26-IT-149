...
export class Level38Scene extends Phaser.Scene {
  ...
  init() {
    ...
    this.baseTutorialScene = "Level37Scene";
    this._conceptNoted = {};
  }
  ...
  update(time, delta) {
    ...
    if (GameManager.interventionInFlight) {
      if (this._creepTween && this._creepTween.isPlaying()) {
        this._creepTween.pause();
      }
      return; // Return early to stop ambient effects and server LEDs from animating
    } else {
      // Resume the round timer tween if the menu was just dismissed
      if (this._creepTween && this._creepTween.isPaused()) {
        this._creepTween.resume();
      }
    }
  ...
  createCRTMonitor() {
    ...
    this.add.text(CRT_X, CRT_Y + CRT_H / 2 + 22, "SIG-01", { font: "bold 10px Courier New", color: HEX_GRAY }).setOrigin(0.5).setDepth(11);
    ...
    this.roundLabel = this.add.text(CRT_X + CRT_W / 2 - 8, CRT_Y - CRT_H / 2 + 8, "", { font: "bold 11px Courier New", color: HEX_GRAY }).setOrigin(1, 0).setAlpha(0.7).setDepth(14);
  ...
  async stampCRT(kind) {
    ...
    const stamp = this.add.text(CRT_X, CRT_Y, label, { font: "bold 21px Arial", color }).setOrigin(0.5).setAngle(-6).setScale(1.5).setAlpha(0).setDepth(30);
    this.crtLayer.add(stamp);
  ...
  showTrialOnCRT(config, lines, questionText) {
    ...
    if (questionText) {
      const qt = this.add.text(CRT_X, CRT_Y + 60, questionText, { font: "14px Arial", color: "#b0bec5" }).setOrigin(0.5).setAlpha(0);
  ...
  createReferenceLog() {
    ...
    this.add.text(LOG_X + 30, LOG_Y + 18, "BROADCAST LOG — LIVE", { font: "bold 12px Arial", color: HEX_CYAN }).setOrigin(0, 0.5).setDepth(12);
  ...
  addLogLine(text, styleType) {
    ...
    const y = LOG_CONTENT_Y0 + 14 + rowIndex * ROW_H;
    const numT = this.add.text(LOG_X + 8, y, String(rowIndex + 1).padStart(2, "0"), { font: "12px Courier New", color: "#3d4450" }).setOrigin(0, 0.5);
    const textT = this.add.text(LOG_X + 32, y, text, { font: "bold 17px Courier New", color: this._typeColor(styleType) }).setOrigin(0, 0.5).setAlpha(0).setScale(0.7);
  ...
  createHUD() {
    ...
    this.add.text(20, 14, "THE SIGNAL ROOM", { font: "bold 17px Arial", color: "#b0bec5" }).setDepth(50);
    this.add.text(20, 32, "Tuning Phase — Output Methods: println()", { font: "13px Arial", color: "#546e7a" }).setDepth(50);
  ...
  async assembleArgument(evalResult) {
    ...
    if (evalResult.isEmpty) {
      const t = this.add.text(cx, cy, "⏎", { font: "bold 18px Arial", color: HEX_PURPLE }).setOrigin(0.5).setScale(0);
    ...
    for (let i = 0; i < chars.length; i++) {
      ...
      const t = this.add.text(x, cy, ch, { font: "bold 16px Courier New", color: chColor }).setOrigin(0, 0.5).setAlpha(0);
      this.marqueeContainer.add(t);
      x += t.width;
  ...
  _measureWidth(text, isString) {
    const t = this.add.text(0, 0, isString ? '"' + text + '"' : text, { font: "bold 16px Courier New" });
  ...
  async animateConcatenation(evaluated) {
    ...
    evaluated.forEach((e, i) => {
      ...
      const t = this.add.text(x, cy, disp, { font: "bold 13px Courier New", color: this._typeColor(e.type) }).setOrigin(0, 0.5).setAlpha(0);
      this.marqueeContainer.add(t);
      this.tweens.add({ targets: t, alpha: 1, duration: 120 });
      x += t.width + 4;
      if (i < evaluated.length - 1) {
        const p = this.add.text(x, cy, "+", { font: "bold 13px Courier New", color: plusColor }).setOrigin(0, 0.5).setAlpha(0);
  ...
  async fireCall(argExpr, decls) {
    ...
    if (!evalResult.ok) {
      this.clearMarquee();
      const t = this.add.text(MARQUEE_X, MARQUEE_Y, "?", { font: "bold 18px Courier New", color: HEX_RED }).setOrigin(0.5);
      this.marqueeContainer.add(t);
  ...
  showCompileErrorStamp() {
    const stamp = this.add.text(MARQUEE_X, MARQUEE_Y - MARQUEE_H / 2 - 20, "COMPILE ERROR", { font: "bold 14px Arial", color: HEX_RED }).setOrigin(0.5).setDepth(80).setScale(1.3).setAlpha(0);
  ...
  async showWaveBanner(text) {
    ...
    const t = this.add.text(0, 0, text, { font: "bold 17px Arial", color: HEX_GOLD }).setOrigin(0.5);
    c.add([g, t]);
    await new Promise((res) => {
      this.tweens.add({
  ...
  showOptionBubbles(options, config, decls) {
    ...
    shuffled.forEach((opt, i) => {
      ...
      const txt = this.add.text(0, 0, label, { font: "bold 15px Courier New", color: "#e0e0e0" }).setOrigin(0.5);
  ...
  async onBubbleSelected(opt, config, bubbleContainer, decls) {
    ...
    if (config.revealNote) this.createFloatingText(LOG_X + LOG_W / 2, LOG_Y + LOG_H + 30, config.revealNote, HEX_GRAY, "13px Arial", 2600);
  ...
  setupBugHunt(config) {
    ...
    const header = this.add.text(CRT_X, CRT_TOP_Y + 16, "CLICK THE BUG", { font: "bold 15px Arial", color: HEX_MAGENTA }).setOrigin(0.5);
    ...
    config.lines.forEach((line, li) => {
      ...
      words.forEach((word) => {
        ...
        const t = this.add.text(x, y, word, { font: "bold 16px Courier New", color: "#e0e0e0" }).setOrigin(0, 0.5).setInteractive({ useHandCursor: true });
  ...
  _lineWidth(text) {
    const t = this.add.text(0, 0, text, { font: "bold 16px Courier New" });
    const w = t.width; t.destroy(); return w;
  }
  ...
  async onTokenClicked(tokenObj, config) {
    ...
    if (correct) {
      ...
      const fixed = this.add.text(tokenObj.x, tokenObj.y - 18, config.fixedToken || config.fix, { font: "bold 14px Courier New", color: HEX_GREEN_BRIGHT }).setOrigin(0, 0.5).setAlpha(0);
  ...
  scoreForAttempt(timePctUsed) {
    ...
    if (signalRemaining > 0.6) { points += 50; this.fastBonusCount++; this.createFloatingText(CRT_X, CRT_Y - CRT_H / 2 - 20, "⚡ CLEAR SIGNAL +50", HEX_GOLD, "bold 15px Arial", 900); }
    else if (signalRemaining > 0.3) { points += 25; this.fastBonusCount++; this.createFloatingText(CRT_X, CRT_Y - CRT_H / 2 - 20, "⚡ +25", HEX_GOLD, "bold 14px Arial", 800); }
    return points;
  }
  ...
  async runBehavioralCheck() {
    ...
    try {
      ...
      console.warn("Level38Scene: /api/wellbeing/predict-struggle unreachable, skipping behavioral signal for this level:", e);
    }
  }
  ...
  gameOver() {
    ...
    (async () => {
      ...
      this._makeButton(525, 420, "RESTORE THE FEED", 200, 50, { stroke: C_RED, textColor: HEX_RED }, () => this.scene.restart());
  ...
  levelComplete() {
    ...
    try { GameManager.completeLevel(38, Math.round((this.correctFirstTry / 15) * 100)); } catch (_) {}
  ...
  showScoreTally() {
    ...
    const badgeLbl = this.add.text(640, 458, "println() SCHEMA TUNED", { font: "bold 15px Arial", color: HEX_GOLD }).setOrigin(0.5).setDepth(91).setAlpha(0);
