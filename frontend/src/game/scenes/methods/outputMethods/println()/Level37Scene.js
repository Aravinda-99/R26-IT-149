...
export class Level37Scene extends Phaser.Scene {
  ...
  createLaunchBay() {
    ...
    this.add.text(LAUNCH_X, LAUNCH_Y - 34, "TX", { font: "bold 11px Arial", color: HEX_CYAN }).setOrigin(0.5).setAlpha(0.6).setDepth(4);
  }
  ...
  async assembleArgument(evalResult) {
    ...
    if (evalResult.isEmpty) {
      const t = this.add.text(cx, cy, "⏎", { font: "bold 23px Arial", color: HEX_PURPLE }).setOrigin(0.5).setScale(0);
      const hint = this.add.text(cx, cy + 20, "empty line", { font: "italic 11px Arial", color: HEX_PURPLE }).setOrigin(0.5).setAlpha(0);
    ...
    const quoteL = isString ? this.add.text(0, cy, '"', { font: "bold 21px Courier New", color: HEX_GRAY }) : null;
    ...
    for (let i = 0; i < chars.length; i++) {
      ...
      const t = this.add.text(x, cy, ch, { font: "bold 21px Courier New", color: chColor }).setOrigin(0, 0.5).setAlpha(0);
    ...
    if (isString) {
      const quoteR = this.add.text(x, cy, '"', { font: "bold 21px Courier New", color: HEX_GRAY }).setOrigin(0, 0.5);
  ...
  _measureWidth(text, isString) {
    const t = this.add.text(0, 0, (isString ? '"' + text + '"' : text), { font: "bold 21px Courier New" });
    const w = t.width;
    t.destroy();
    return w;
  }
  ...
  async animateConcatenation(tokens, evaluated) {
    ...
    evaluated.forEach((e, i) => {
      ...
      const t = this.add.text(x, cy, disp, { font: "bold 17px Courier New", color: this._typeColor(e.type) }).setOrigin(0, 0.5).setAlpha(0);
      this.marqueeContainer.add(t);
      this.tweens.add({ targets: t, alpha: 1, duration: 150 });
      x += t.width + 6;
      if (i < evaluated.length - 1) {
        const p = this.add.text(x, cy, "+", { font: "bold 17px Courier New", color: plusColor }).setOrigin(0, 0.5).setAlpha(0);
    ...
    if (!this._conceptNoted[note]) {
      ...
      const t = this.add.text(MARQUEE_X + MARQUEE_W / 2, MARQUEE_Y - 12, note, { font: "italic 12px Arial", color: plusColor }).setOrigin(0.5).setAlpha(0);
      this.marqueeContainer.add(t);
  ...
  createBroadcastLog() {
    ...
    this.add.text(LOG_X + 40, LOG_Y + 22, "BROADCAST LOG — CONSOLE OUTPUT", { font: "bold 13px Arial", color: HEX_CYAN }).setOrigin(0, 0.5).setDepth(12);
    this.add.text(LOG_X + LOG_W - 15, LOG_Y + 22, "CH 01", { font: "12px Arial", color: HEX_GRAY }).setOrigin(1, 0.5).setDepth(12);
  ...
  _createLogRow(text, styleType) {
    ...
    const numT = this.add.text(LOG_X + 6, y, String(rowIndex + 1).padStart(2, "0"), { font: "13px Courier New", color: "#3d4450" }).setOrigin(0, 0.5);
    const textT = this.add.text(LOG_X + 34, y, text, { font: "bold 19px Courier New", color: this._typeColor(styleType) }).setOrigin(0, 0.5).setAlpha(0).setScale(0.7);
  ...
  createHUD() {
    ...
    this.add.text(20, 14, "THE BROADCAST TOWER", { font: "bold 17px Arial", color: "#b0bec5" }).setDepth(51);
    this.add.text(20, 32, "Accretion Phase — Output Methods: println()", { font: "13px Arial", color: "#546e7a" }).setDepth(51);

    this.add.text(1060, 8, "SCORE", { font: "11px Arial", color: "#546e7a" }).setDepth(51);
    this.scoreText = this.add.text(1060, 20, "0", { font: "bold 19px Arial", color: "#ffffff" }).setDepth(51);
  ...
  async fireCall(argExpr, decls, method = "println") {
    ...
    if (!evalResult.ok) {
      this.clearMarquee();
      const t = this.add.text(MARQUEE_X + MARQUEE_W / 2, MARQUEE_Y + MARQUEE_H / 2, "?", { font: "bold 23px Courier New", color: HEX_RED }).setOrigin(0.5);
      this.marqueeContainer.add(t);
  ...
  showFirstNewlineAnnotation() {
    ...
    const t = this.add.text(LOG_X + LOG_W / 2, LOG_Y + LOG_H + 14, "println always adds a newline", { font: "italic 13px Arial", color: HEX_PURPLE }).setOrigin(0.5).setDepth(20).setAlpha(0);
    this.tweens.add({ targets: t, alpha: 1, duration: 200 });
  ...
  showQuestionCard(promptText) {
    ...
    const badgeT = this.add.text(-230, -10, String(this.currentRound + 1), { font: "bold 16px Arial", color: "#050914" }).setOrigin(0.5);
    const t = this.add.text(-200, -10, promptText, { font: "16px Arial", color: "#e0e0e0", wordWrap: { width: 420 } }).setOrigin(0, 0.5);
  ...
  showOptionBubbles(options, config) {
    ...
    shuffled.forEach((opt, i) => {
      ...
      const txt = this.add.text(0, 0, label, { font: "bold 16px Courier New", color: "#e0e0e0" }).setOrigin(0.5);
  ...
  async onBubbleSelected(opt, config, bubbleContainer) {
    ...
    if (config.revealNote) this.createFloatingText(640, LOG_Y + LOG_H + 30, config.revealNote, HEX_GRAY, "13px Arial", 2600);
    await this.delay(500);
    if (!this._alive) return;
  ...
  createCartridgeTray(config) {
    ...
    bdraw(false, false);
    const bt = this.add.text(0, 0, "BROADCAST", { font: "bold 16px Arial", color: "#050914" }).setOrigin(0.5);
    btn.add([bg, bt]);
    btn.setSize(150, 44);
  ...
  async onBroadcastPressed(config) {
    ...
    if (config.revealNote) this.createFloatingText(640, LOG_Y + LOG_H + 30, config.revealNote, HEX_GRAY, "13px Arial", 3000);
    await this.delay(500);
    if (!this._alive) return;
  ...
  async runBehavioralCheck() {
    ...
    try {
      ...
      console.warn("Level37Scene: /api/wellbeing/predict-struggle unreachable, skipping behavioral signal for this level:", e);
    }
  }
  ...
  gameOver() {
    ...
    (async () => {
      ...
      this._makeButton(525, 420, "RESTORE POWER", 200, 50, { stroke: C_RED, textColor: HEX_RED }, () => this.scene.restart());
  ...
  showScoreTally() {
    ...
    lines.forEach((s, i) => {
      const t = this.add.text(500, 195 + i * 28, s, { font: "16px Arial", color: HEX_GRAY }).setOrigin(0, 0.5).setDepth(91).setAlpha(0);
