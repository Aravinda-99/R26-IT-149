...
export class Level27Scene extends Phaser.Scene {
  ...
  _drawSlotPlaceholder(slotId) {
    ...
    if (!filled) {
      const label = this.add.text(x + w / 2, y + h / 2, def.hint, {
        font: "italic 14px Courier New", color: "#3d4450",
  ...
  createBlockPalette() {
    ...
    this.add.text(PX + 10, PY + 8, "BLOCK SUPPLY", { font: "bold 11px Arial", color: "#3d4450" }).setDepth(11);
  ...
  populatePalette(mission) {
    ...
    shuffled.forEach((def) => {
      const style = { font: "bold 17px Courier New", color: def.tag ? HEX_CYAN : HEX_CYAN };
  ...
  createObservationWindow() {
    ...
    this.add.text(OX + OW / 2, OY + OH + 12, "TEST FLOOR — LIVE FEED", {
      font: "bold 11px Arial", color: "#3d4450",
  ...
  createTestMachine() {
    ...
    this.machineLayer.add(this.add.text(MACHINE_CX + 105, MACHINE_CY - 25, "PASS", { font: "11px Arial", color: HEX_GREEN }).setOrigin(0.5));
    this.machineLayer.add(this.add.text(MACHINE_CX + 105, MACHINE_CY + 25, "FAIL", { font: "11px Arial", color: HEX_RED }).setOrigin(0.5));
    ...
    this.machineFace = this.add.text(MACHINE_CX, MACHINE_CY, "", {
      font: "13px Courier New", color: HEX_CYAN, wordWrap: { width: 180 },
    }).setOrigin(0.5);
  ...
  buildReportRows(tests) {
    ...
    tests.forEach((test, i) => {
      ...
      const inputT = this.add.text(0, 0, label, { font: "14px Courier New", color: "#b0bec5" }).setOrigin(0, 0.5);
      const arrow = this.add.text(190, 0, "→", { font: "14px Arial", color: HEX_GRAY }).setOrigin(0, 0.5);
      const expT = this.add.text(210, 0, String(test.expected ?? test.expectedValue ?? test.expectedVisits), {
        font: "14px Courier New", color: HEX_GRAY,
      }).setOrigin(0, 0.5);
  ...
  renderMissionBrief(mission) {
    ...
    const brief = this.add.text(BX + 14, BY + 42, mission.brief, {
      font: "14px Arial", color: "#90a4ae", wordWrap: { width: BW - 28 },
    }).setOrigin(0, 0);
    const hint = this.add.text(BX + BW - 12, BY + BH - 12, "HINT", {
      font: "bold 13px Arial", color: "#546e7a",
  ...
  createHUD() {
    ...
    this.add.text(20, 14, "THE CONTROL ROOM", { font: "bold 17px Arial", color: "#b0bec5" }).setDepth(51);
    this.add.text(20, 36, "Restructuring Phase — String Methods: length()", { font: "13px Arial", color: "#546e7a" }).setDepth(51);
  ...
  async showCompileError(err) {
    ...
    const msg = this.add.text(CX + CW / 2, CY + CH - 30, err.message, {
      font: "bold 14px Arial", color: HEX_RED, wordWrap: { width: CW - 40 },
  ...
  async runTestCase(mission, test, index, result) {
    ...
    const capsule = this.add.text(MACHINE_CX - 150, MACHINE_CY, inputLabel.replace(/ /g, "␣"), {
      font: "13px Courier New", color: "#e0e0e0",
  ...
  async onMissionComplete() {
    ...
    if (flawless) this.createFloatingText(CX + CW / 2, CY + 40, "FLAWLESS +100", HEX_AMBER, "bold 19px Arial");
  ...
  gameOver() {
    ...
    this._makeButton(525, 420, "REQUEST NEW KEYCARD", 200, 50, { stroke: C_RED, textColor: HEX_RED }, () => this.scene.restart());
  ...
  showScoreTally() {
    ...
    lines.forEach((s, i) => {
      const t = this.add.text(500, 230 + i * 30, s, { font: "16px Arial", color: HEX_GRAY }).setOrigin(0, 0.5).setDepth(91).setAlpha(0);
    ...
    const badgeLbl = this.add.text(640, 528, "length() MASTERY", { font: "bold 16px Arial", color: HEX_AMBER }).setOrigin(0.5).setDepth(91).setAlpha(0);
    const badgeSub = this.add.text(640, 546, "Accretion ✓  Tuning ✓  Restructuring ✓", { font: "12px Arial", color: "#78909c" }).setOrigin(0.5).setDepth(91).setAlpha(0);
