...
export class Level26Scene extends Phaser.Scene {
  ...
  update(time, delta) {
    ...
    if (GameManager.interventionInFlight) {
      if (this.crateTween && this.crateTween.isPlaying()) {
        this.crateTween.pause();
      }
      return; // Return early to stop belt, rollers, and dust from animating
    } else {
      // Resume crate tween if the menu was just dismissed
      if (this.crateTween && this.crateTween.isPaused()) {
  ...
  createRejectionPit() {
    ...
    this.rejectSignText = this.add.text(1215, 380, "REJECT", {
      font: "bold 13px Arial", color: HEX_RED,
  ...
  createReferencePanel() {
    ...
    this.add.text(202, 140, "ACTIVE SPECIMENS", { font: "bold 14px Arial", color: "#3d4450" }).setDepth(11);
    this.refGroup = null;
  }
  ...
  buildMiniTileStrip(str, x, y) {
    ...
    str.split("").forEach((ch, i) => {
      ...
      const txt = this.add.text(0, 0, meta.display, { font: "bold 18px Courier New", color: meta.color }).setOrigin(0.5);
      if (meta.isSpace) txt.setAlpha(0.85).setFontSize(13);
      tc.add([bg, txt]);
      c.add(tc);
    });
    return c;
  ...
  updateReferencePanel(decls, hideTiles) {
    ...
    if (!decls || decls.length === 0) {
      const t = this.add.text(640, 190, "(literals used directly — no declarations)", {
        font: "italic 17px Arial", color: "#3d4450",
    ...
    decls.forEach((d, i) => {
      ...
      const { container } = this._renderTokenLine(210, y, declLine, 17);
      ...
      if (d.kind === "string" && !hideTiles) {
        ...
        const hint = this.add.text(210, y + 22, "(count mentally — no tiles shown)", {
          font: "italic 14px Arial", color: "#3d4450",
  ...
  bitSay(text) {
    ...
    const inner = this.add.text(0, 0, text, {
      font: "15px Arial", color: "#e0e0e0", wordWrap: { width: 330 },
    });
      ...
      this._resolveAnswer(cfg, correct, choice, correct ? null : cfg.wrongTag, async () => {
        if (cfg.correct === "valid") {
          const val = this.add.text(this.crate.container.x, CRATE_Y - 100, `= ${cfg.resultValue}`, {
            font: "bold 19px Courier New", color: HEX_GREEN,
  ...
  revealFaultToken(cfg) {
    ...
    const anno = this.add.text(640, 300, cfg.explanation, {
      font: "bold 14px Arial", color: HEX_RED, wordWrap: { width: 700 },
    }).setOrigin(0.5).setDepth(35).setAlpha(0);
  ...
  setupBugHunt(cfg) {
    const strip = this.add.text(640, 265, "CLICK THE BUG", {
      font: "bold 15px Arial", color: HEX_MAGENTA,
    ...
    this._resolveAnswer(cfg, correct, token.raw, correct ? null : cfg.wrongTag, async () => {
      if (correct) {
        ...
        const fixed = this.add.text(token.text.x, token.text.y - 20, cfg.fixedToken, {
          font: "bold 15px Courier New", color: HEX_GREEN,
  ...
  async runBehavioralCheck() {
    ...
    try {
      ...
      console.warn("Level26Scene: /api/wellbeing/predict-struggle unreachable:", e);
    }
  }
  ...
  async advanceRound() {
    if (this.currentRound === 2) {
      ...
      // We must wait up to 1.5 seconds to give it a chance to process the new behavioral flag
      // and open the menu BEFORE we mistakenly decide to advance to Round 4.
      let waitTime = 0;
      ...
      // Now, if the menu DID open, wait indefinitely until the user closes it.
  ...
  addLife() {
    if (this.lives < 5) {
      // Find the icon that corresponds to the currently lost life (which is at index: this.lives)
      const icon = this.lifeIcons[this.lives];
      if (icon) {
  ...
  gameOver() {
    ...
    this.add.text(640, 350, `Crates Inspected: ${this.currentRound} / ${ROUNDS.length}`, {
      font: "18px Arial", color: HEX_GRAY,
    }).setOrigin(0.5).setDepth(91);

    this._makeButton(525, 420, "RESTART SHIFT", 200, 50, { stroke: C_RED, textColor: HEX_RED }, () => this.scene.restart());
