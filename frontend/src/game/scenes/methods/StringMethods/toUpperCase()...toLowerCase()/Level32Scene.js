...
export class Level32Scene extends Phaser.Scene {
  ...
  update(time, delta) {
    ...
    if (GameManager.interventionInFlight) {
      if (this.coolingTween && this.coolingTween.isPlaying()) {
        this.coolingTween.pause();
      }
      return; // Return early to stop particles, furnace, and stopwatch from animating
    } else {
      // Resume the cooling timer tween if the menu was just dismissed
      if (this.coolingTween && this.coolingTween.isPaused()) {
        this.coolingTween.resume();
      }
    }
  ...
  createPress() {
    ...
    this.dieText = this.add.text(0, 32, "?", { font: "bold 16px Courier New", color: HEX_GRAY }).setOrigin(0.5);
  ...
  createOriginalCase() {
    ...
    this.add.text(CASE_CX, CASE_CY + 78, "ORIGINAL — SEALED", { font: "bold 11px Arial", color: HEX_CYAN }).setOrigin(0.5).setDepth(11);
  ...
  loadOriginalCase(str) {
    ...
    if (n === 0) {
      const t = this.add.text(CASE_CX, CASE_CY, "(empty)", { font: "italic 14px Arial", color: "#3d4450" }).setOrigin(0.5);
  ...
  updateSourceLiteral(varName, str) {
    ...
    let x = 0;
    const put = (t, color) => {
      const txt = this.add.text(x, 0, t, { font: "17px Courier New", color }).setOrigin(0, 0.5);
  ...
  createOutputTray() {
    ...
    this.add.text(TRAY_CX, TRAY_CY - 62, "RESULT", { font: "bold 11px Arial", color: "#546e7a" }).setOrigin(0.5).setDepth(11);
    this.trayTileLayer = this.add.container(0, 0).setDepth(11);
    this.trayResultText = this.add.text(TRAY_CX, TRAY_CY - 80, "", { font: "bold 18px Courier New", color: HEX_GREEN }).setOrigin(0.5).setDepth(12);
  ...
  async discardTrayToChute() {
    ...
    const label = this.add.text(TRAY_CX + 140, TRAY_CY + 60, "UNSAVED", { font: "bold 12px Arial", color: HEX_RED }).setOrigin(0.5).setDepth(20).setAlpha(0);
  ...
  async hookRefeed() {
    const hook = this.add.text(TRAY_CX, TRAY_CY - 40, "⌐", { font: "bold 21px Arial", color: HEX_GRAY }).setOrigin(0.5).setDepth(20);
  ...
  async miniClawCameo(str, index) {
    ...
    const resultText = this.add.text(cx, cy + 30, `'${ch}'`, { font: "bold 23px Courier New", color: HEX_MAGENTA }).setOrigin(0.5).setDepth(21).setScale(0);
  ...
  async counterPanelCameo(len) {
    ...
    const t = this.add.text(0, 0, "0", { font: "bold 18px Courier New", color: HEX_AMBER }).setOrigin(0.5);
    panel.add([g, t]);
  ...
  showEvaluationFloat(lines) {
    return new Promise((res) => {
      const t = this.add.text(640, 460, lines[0], { font: "bold 15px Courier New", color: HEX_AMBER }).setOrigin(0.5).setDepth(20).setAlpha(0);
  ...
  createHUD() {
    ...
    this.add.text(20, 14, "THE PRESS GAUNTLET", { font: "bold 17px Arial", color: "#b0bec5" }).setDepth(51);
    this.add.text(20, 36, "Tuning Phase — toUpperCase() / toLowerCase()", { font: "13px Arial", color: "#546e7a" }).setDepth(51);
  ...
  createTickerTape() {
    ...
    this.tickerText = this.add.text(1000, 78, "", { font: "bold 14px Courier New", color: "#263238" }).setOrigin(0.5, 0).setDepth(56).setAlpha(0);
  }
  ...
  showWaveBanner(text) {
    return new Promise((res) => {
      ...
      const t = this.add.text(0, 0, text, { font: "bold 23px Arial", color: HEX_AMBER }).setOrigin(0.5);
      banner.add([g, t]);
      this.tweens.add({
  ...
  spawnBillet(cfg) {
    ...
    if (cfg.round) {
      const label = this.add.text(-width / 2 + 10, -height / 2 + 10, `BILLET ${cfg.round}/15`, {
        font: "bold 11px Courier New", color: "#0b0a0e",
      }).setOrigin(0, 0.5);
      container.add(label);
    ...
    if (cfg.billetLines && cfg.type === "bughunt") {
      ...
      if (lw > width - 40) { lc.destroy(); fontSize = 15; ({ container: lc, width: lw } = this._renderTokenLine(0, 0, text, fontSize, "#1a1017")); }
      lc.x = -lw / 2;
      container.add(lc);
  ...
  _renderClickableTokenLine(parent, x, y, line, maxWidth) {
    ...
    const measured = tokens.map((tok) => {
      const t = this.add.text(0, 0, tok.t, { font: "15px Courier New" });
      const w = t.width;
      t.destroy();
      return w;
    });
    ...
    tokens.forEach((tok, i) => {
      const t = this.add.text(cx, y, tok.t, { font: "15px Courier New", color: "#1a1017" }).setOrigin(0, 0.5);
  ...
  onTokenClicked(token, lines, cfg) {
    ...
    this._resolveAnswer(cfg, correct, token.raw, correct ? null : cfg.wrongTag, async () => {
      if (correct) {
        ...
        const fixed = this.add.text(token.text.x, token.text.y - 18, cfg.fixedToken, { font: "bold 14px Courier New", color: HEX_GREEN }).setOrigin(0, 0.5).setAlpha(0).setDepth(36);
  ...
  onCorrectAnswer(cfg, timePctUsed) {
    ...
    if (remaining > 0.6) { points += 50; this.fastBonusCount++; this.createFloatingText(640, 300, "⚡ WHITE-HOT +50", HEX_AMBER, "bold 17px Arial"); }
    else if (remaining > 0.3) { points += 25; this.fastBonusCount++; this.createFloatingText(640, 300, "⚡ WHITE-HOT +25", HEX_AMBER, "bold 17px Arial"); }
  ...
  gameOver() {
    ...
    this.add.text(640, 350, `Billets Tempered: ${this.currentRound} / ${ROUNDS.length}`, { font: "18px Arial", color: HEX_GRAY }).setOrigin(0.5).setDepth(91);

    this._makeButton(525, 420, "RELIGHT THE FORGE", 200, 50, { stroke: C_RED, textColor: HEX_RED }, () => this.scene.restart());
