...
export class Level31Scene extends Phaser.Scene {
  ...
  createOutputTray() {
    ...
    this.trayTypeTag = this.add.text(TRAY_CX, TRAY_CY - 60, "", { font: "bold 13px Courier New", color: "#4fc3f7" }).setOrigin(0.5).setDepth(12).setAlpha(0);
  ...
  async discardTrayToChute() {
    ...
    const label = this.add.text(chuteX, TRAY_CY + 60, "UNSAVED", { font: "bold 12px Arial", color: HEX_RED }).setOrigin(0.5).setDepth(20).setAlpha(0);
  ...
  async captureToShelf(name, resultStr) {
    ...
    this.shelfLabel = this.add.text(shelfX, shelfY - 18, `"${name}" holds "${resultStr}"`, {
      font: "bold 13px Courier New", color: HEX_CYAN,
  ...
  async runTransformation(str, mode) {
    ...
    for (let i = 0; i < ghosts.length; i++) {
      ...
      if (/[a-zA-Z]/.test(ch)) {
        ...
        if (converted === ch) {
          const check = this.add.text(ghost.container.x, ANVIL_Y - 30, "✓", { font: "12px Arial", color: HEX_GREEN }).setOrigin(0.5).setDepth(14);
        ...
        ghost.container.setAlpha(1);
        const chime = this.add.text(ghost.container.x, ANVIL_Y - 30, "—", { font: "bold 16px Arial", color: HEX_GRAY }).setOrigin(0.5).setDepth(14);
        ...
        if (firstPassThrough) {
          ...
          const anno = this.add.text(ghost.container.x, ANVIL_Y - 55, "not a letter — passes through", {
            font: "12px Arial", color: HEX_GRAY,
  ...
  createTickerTape() {
    ...
    this.tickerText = this.add.text(1000, 25, "", { font: "bold 14px Courier New", color: "#263238" }).setOrigin(0.5, 0).setDepth(56).setAlpha(0);
  }
  ...
  setupCommand(cfg) {
    ...
    const widths = cfg.cartridges.map((c) => {
      const t = this.add.text(0, 0, c.code, { font: "bold 15px Courier New", color: HEX_AMBER });
    ...
    cfg.cartridges.forEach((cart, i) => {
      ...
      const t = this.add.text(0, 0, cart.code, { font: "bold 15px Courier New", color: HEX_AMBER }).setOrigin(0.5);
      c.add([g, t]);
      c.setSize(w, h);
      c.setData("w", w);
      c.setData("cart", cart);
    ...
    const st = this.add.text(0, 0, "STAMP", { font: "bold 16px Arial", color: "#0a0e14" }).setOrigin(0.5);
    stampBtn.add([bg, st]);
  ...
  async runBehavioralCheck() {
    ...
    try {
      ...
      console.warn("Level31Scene: /api/wellbeing/predict-struggle unreachable, skipping behavioral signal for this level:", e);
    }
  }
  ...
  onCorrectAnswer(cfg) {
    ...
    if (timeMs <= 6000) { points += 25; this.createFloatingText(TRAY_CX, TRAY_CY - 100, "+25 QUICK!", HEX_AMBER, "bold 15px Arial"); }
  ...
  gameOver() {
    ...
    this._makeButton(525, 420, "RESTART THE PRESS", 200, 50, { stroke: C_RED, textColor: HEX_RED }, () => this.scene.restart());
