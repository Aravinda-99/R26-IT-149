...
export class Level33Scene extends Phaser.Scene {
  ...
  createBlockPalette() {
    ...
    this.add.text(PX + 10, PY + 8, "CASTING BIN", { font: "bold 11px Arial", color: "#3d4450" }).setDepth(11);
  ...
  createRigWindow() {
    ...
    this.add.text(OX + OW / 2, OY + OH + 12, "PRODUCTION LINE — LIVE", { font: "bold 11px Arial", color: "#3d4450" }).setOrigin(0.5).setDepth(11);
  ...
  _buildStationIcon(x, y, kind) {
    ...
    const label = this.add.text(0, 22, kind.toUpperCase(), { font: "10px Arial", color: "#546e7a" }).setOrigin(0.5);
    c.add([glow, g, label]);
  ...
  buildMiniTileRow(str, indexPlates = false) {
    ...
    str.split("").forEach((ch, i) => {
      ...
      drawBody(0x2a3a4a);
      const charText = this.add.text(0, -3, meta.display, { font: "bold 13px Courier New", color: meta.color }).setOrigin(0.5);
      container.add([body, charText]);
      if (indexPlates) {
        const plateText = this.add.text(0, 12, String(i), { font: "bold 10px Courier New", color: HEX_AMBER }).setOrigin(0.5);
  ...
  miniUnsavedPuff() {
    const puff = this.add.text(STATION_PRESS_X, STATION_Y - 30, "UNSAVED", { font: "bold 10px Arial", color: HEX_RED }).setOrigin(0.5).setAlpha(0);
  ...
  createOutputTicker() {
    ...
    this.rigLayer.add(g);
    this.tickerText = this.add.text(OX + 18, TICKER_Y, "", { font: "bold 15px Courier New", color: HEX_GREEN }).setOrigin(0, 0.5).setDepth(17);
  ...
  renderMissionBrief(mission) {
    ...
    if (mission.blueprint) {
      const scroll = this.add.text(BX + BW - 14, BY + 14, "📜", { font: "18px Arial" }).setOrigin(1, 0.5).setAlpha(0.8);
  ...
  showProjectBriefing(index) {
    ...
    const blueprintTag = mission.blueprint
      ? this.add.text(-195, -60, "📜 BLUEPRINT REOPENED", { font: "bold 12px Arial", color: HEX_AMBER }).setOrigin(0, 0.5)
      : null;
  ...
  gameOver() {
    ...
    this._makeButton(525, 420, "RELIGHT THE CRUCIBLE", 200, 50, { stroke: C_RED, textColor: HEX_RED }, () => this.scene.restart());
  ...
  showScoreTally() {
    ...
    const ribbonText = this.add.text(640, 522, "STRING FOUNDATIONS WING — COMPLETE", { font: "bold 14px Arial", color: HEX_AMBER }).setOrigin(0.5).setDepth(92).setAlpha(0);
    const ribbonSub = this.add.text(640, 542, "length() ✓   charAt() ✓   toUpperCase()/toLowerCase() ✓", { font: "12px Courier New", color: HEX_PURPLE }).setOrigin(0.5).setDepth(92).setAlpha(0);
