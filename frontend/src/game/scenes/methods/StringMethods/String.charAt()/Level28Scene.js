...
export class Level28Scene extends Phaser.Scene {
  ...
  createTile(ch, index, x, y, tw = 52) {
    ...
    const spaceLabel = meta.isSpace
      ? this.add.text(0, 16, "space", { font: "10px Arial", color: HEX_MAGENTA }).setOrigin(0.5).setAlpha(0.6)
      : null;
    ...
    const plateText = this.add.text(0, plateY, String(index), { font: "bold 15px Courier New", color: HEX_AMBER }).setOrigin(0.5);
  ...
  createResultDisplay() {
    ...
    this.resultTypeTag = this.add.text(RESULT_CX, RESULT_CY + 26, "", { font: "bold 14px Courier New", color: "#4fc3f7" }).setOrigin(0.5).setDepth(15).setAlpha(0);
    this.resultSpaceNote = this.add.text(RESULT_CX, RESULT_CY + 42, "", { font: "12px Arial", color: HEX_MAGENTA }).setOrigin(0.5).setDepth(15).setAlpha(0);
  }
  ...
  updateLiteralDisplay(varName, str) {
    ...
    let x = 0;
    const put = (t, color) => {
      const txt = this.add.text(x, 0, t, { font: "18px Courier New", color }).setOrigin(0, 0.5);
      c.add(txt);
      x += txt.width;
    };
  ...
  createHUD() {
    ...
    this.add.text(20, 14, "THE RETRIEVAL CLAW", { font: "bold 17px Arial", color: "#b0bec5" }).setDepth(51);
    this.add.text(20, 36, "Accretion Phase — String Methods: charAt()", { font: "13px Arial", color: "#546e7a" }).setDepth(51);
  ...
  updateCommandMonitor(mode, data) {
    ...
    const put = (t, color, font) => {
      const txt = this.add.text(x, 0, t, { font: font || "bold 17px Courier New", color }).setOrigin(0, 0.5);
      c.add(txt);
      x += txt.width;
    };
    if (mode === "call") {
      ...
      put("(", HEX_MAGENTA);
      put(String(data.index), HEX_AMBER, "bold 18px Courier New");
      put(")", HEX_MAGENTA);
      c.x = 640 - x / 2;
    } else if (mode === "snippet") {
      const t = this.add.text(0, 0, data.snippet, { font: "15px Courier New", color: "#e0e0e0" }).setOrigin(0.5);
      c.add(t);
      c.x = 640;
    } else if (mode === "mission") {
      const t = this.add.text(0, 0, data.text, { font: "15px Arial", color: HEX_AMBER, wordWrap: { width: 460 } }).setOrigin(0.5);
      c.add(t);
      c.x = 640;
    }
    this.monitorGroup = c;
  ...
  showCharStringCard() {
    ...
    const l1 = this.add.text(0, -14, "char   → 'b'", { font: "bold 14px Courier New", color: HEX_CYAN }).setOrigin(0.5);
    const l2 = this.add.text(0, 14, 'String → "b"', { font: "bold 14px Courier New", color: HEX_GRAY }).setOrigin(0.5);
    c.add([g, l1, l2]);
    c.setAlpha(0);
    this.tweens.add({ targets: c, alpha: 1, duration: 300 });
    this.roundElements.push(c);
  ...
  async runTutorial() {
    ...
    if (!A()) return;
    const anno = this.add.text(640, 420, "length() = 5, last index = 4", { font: "bold 13px Arial", color: HEX_AMBER }).setOrigin(0.5).setDepth(20).setAlpha(0);
  ...
  showQuestionCard(cfg) {
    ...
    const badgeNum = this.add.text(-228, 0, String(cfg.round), { font: "bold 16px Arial", color: "#0a0e14" }).setOrigin(0.5);
    const text = this.add.text(-200, 0, this._defaultQuestionText(cfg), {
      font: "18px Arial", color: "#e0e0e0", wordWrap: { width: 440 },
    }).setOrigin(0, 0.5);
  ...
  showOptionBubbles(options, onSelect) {
    ...
    const style = { font: "bold 18px Courier New", color: HEX_CYAN };
      ...
      if (!correct) {
        ...
        if (wrongTile) {
          ...
          const anno = this.add.text(wrongTile.x, TILE_Y - 60, `that's '${actualCh}'!`, {
            font: "bold 14px Arial", color: HEX_RED,
  ...
  showJudgmentButtons(onSelect) {
    const mk = (x, label, color) => {
      ...
      const t = this.add.text(0, 0, label, { font: "bold 17px Arial", color: hex }).setOrigin(0.5);
      c.add([g, t]);
      c.setSize(190, 54);
      c.setInteractive({ useHandCursor: true });
    ...
    this.showJudgmentButtons(async (choice) => {
      ...
      if (cfg.crashType === "compile") {
        ...
        const anno = this.add.text(640, 62, cfg.explanation, {
          font: "bold 14px Arial", color: HEX_RED, wordWrap: { width: 560 },
  ...
  async showEvaluationFloat(lines) {
    for (let i = 0; i < lines.length; i++) {
      if (!this._alive) return;
      const t = this.add.text(640, 440 - i * 24, lines[i], { font: "bold 15px Courier New", color: HEX_AMBER }).setOrigin(0.5).setDepth(20).setAlpha(0);
      this.roundElements.push(t);
  ...
  setupCommand(cfg) {
    ...
    const tw = cfg.capsules.map((c) => {
      const t = this.add.text(0, 0, c.label, { font: "bold 16px Courier New", color: HEX_CYAN });
    ...
    cfg.capsules.forEach((cap, i) => {
      ...
      const t = this.add.text(0, 0, cap.label, { font: "bold 16px Courier New", color: HEX_AMBER }).setOrigin(0.5);
      c.add([g, t]);
      c.setSize(w, h);
      c.setData("w", w);
      c.setData("cap", cap);
    ...
    const et = this.add.text(0, 0, "EXECUTE", { font: "bold 15px Arial", color: "#0a0e14" }).setOrigin(0.5);
    exec.add([eg, et]);
    exec.setSize(120, 40);
  ...
  onCorrectAnswer(cfg) {
    ...
    if (timeMs <= 6000) { points += 25; this.createFloatingText(RESULT_CX, RESULT_CY - 90, "+25 QUICK!", HEX_AMBER, "bold 15px Arial"); }
  ...
  async celebrationJuggle() {
    ...
    for (let i = 0; i < str.length; i++) {
      ...
      const flyChar = this.add.text(tile.x, TILE_Y, str[i] === " " ? "␣" : str[i], {
        font: "bold 21px Courier New", color: this._charMeta(str[i]).color,
      }).setOrigin(0.5).setDepth(30);
  ...
  gameOver() {
    ...
    this.add.text(640, 350, `Rounds Completed: ${this.currentRound} / ${ROUNDS.length}`, { font: "18px Arial", color: HEX_GRAY }).setOrigin(0.5).setDepth(91);

    this._makeButton(525, 420, "RESET SYSTEMS", 200, 50, { stroke: C_RED, textColor: HEX_RED }, () => this.scene.restart());
  ...
  showScoreTally(accuracy) {
    ...
    lines.forEach((s, i) => {
      const t = this.add.text(500, 260 + i * 30, s, { font: "16px Arial", color: HEX_GRAY }).setOrigin(0, 0.5).setDepth(91).setAlpha(0);
    ...
    const badgeLbl = this.add.text(640, 508, "charAt() SCHEMA ACQUIRED", { font: "bold 15px Arial", color: HEX_AMBER }).setOrigin(0.5).setDepth(91).setAlpha(0);
