...
export class Level30Scene extends Phaser.Scene {
  ...
  createBlockPalette() {
    ...
    this.add.text(PX + 10, PY + 8, "PARTS BIN", { font: "bold 11px Arial", color: "#3d4450" }).setDepth(11);
  ...
  createRigWindow() {
    ...
    this.add.text(OX + OW / 2, OY + OH + 12, "AUTOMATION RIG — LIVE", { font: "bold 11px Arial", color: "#3d4450" }).setOrigin(0.5).setDepth(11);
    ...
    this.watchdogText = this.add.text(OX + OW / 2, MINI_TILE_Y, "", {
      font: "bold 15px Arial", color: HEX_RED, wordWrap: { width: OW - 30 }, align: "center",
  ...
  createOutputTicker() {
    ...
    this.rigLayer.add(g);
    this.tickerText = this.add.text(OX + 18, TICKER_Y, "", { font: "bold 16px Courier New", color: HEX_GREEN }).setOrigin(0, 0.5).setDepth(17);
  ...
  createCounterDial() {
    ...
    c.add(g);
    const t = this.add.text(0, 0, "0", { font: "bold 17px Courier New", color: HEX_AMBER }).setOrigin(0.5);
    c.add(t);
    this.rigLayer.add(c);
  ...
  buildMiniTileRow(str) {
    ...
    str.split("").forEach((ch, i) => {
      ...
      drawBody(0x2a3a4a);
      const charText = this.add.text(0, -6, meta.display, { font: "bold 15px Courier New", color: meta.color }).setOrigin(0.5);
      ...
      const plateText = this.add.text(0, 19, String(i), { font: "bold 11px Courier New", color: HEX_AMBER }).setOrigin(0.5);
  ...
  flashTilePair(i, j, match) {
    ...
    const glyph = this.add.text(midX, MINI_TILE_Y - 30, match ? "=" : "✗", { font: "bold 16px Arial", color: match ? HEX_GREEN : HEX_RED }).setOrigin(0.5).setDepth(1);
  ...
  miniClawCrash(x) {
    ...
    const stamp = this.add.text(OX + OW / 2, MINI_TILE_Y, "IndexOutOfBounds!", { font: "bold 13px Courier New", color: HEX_RED }).setOrigin(0.5).setDepth(2);
  ...
  buildReportRows(tests) {
    ...
    tests.forEach((test, i) => {
      ...
      const inputT = this.add.text(0, 0, `"${test.input}"`, { font: "14px Courier New", color: "#b0bec5" }).setOrigin(0, 0.5);
      ...
      const expT = this.add.text(170, 0, String(expected), { font: "14px Courier New", color: HEX_GRAY }).setOrigin(0, 0.5);
      const actualT = this.add.text(300, 0, "?", { font: "14px Courier New", color: HEX_GRAY }).setOrigin(0, 0.5);
      const statusT = this.add.text(RW - 24, 0, "…", { font: "16px Arial", color: HEX_GRAY }).setOrigin(0.5);
  ...
  createHUD() {
    ...
    this.add.text(20, 14, "THE WORKSHOP", { font: "bold 15px Arial", color: "#b0bec5" }).setDepth(51);
    this.add.text(20, 32, "Restructuring Phase — String Methods: charAt()", { font: "12px Arial", color: "#546e7a" }).setDepth(51);
  ...
  showProjectBriefing(index) {
    ...
    const badgeNum = this.add.text(-215, -70, String(mission.mission), { font: "bold 18px Arial", color: "#0a0e14" }).setOrigin(0.5);
    const title = this.add.text(-185, -80, mission.title, { font: "bold 21px Arial", color: "#ffffff" }).setOrigin(0, 0.5);
    const desc = this.add.text(-215, -45, mission.brief, { font: "15px Arial", color: "#b0bec5", wordWrap: { width: 460 } }).setOrigin(0, 0);
  ...
  async showCompileError(err) {
    ...
    if (!this._alive) return;
    const msg = this.add.text(CX + CW / 2, CY + CH - 30, err.message, { font: "bold 14px Arial", color: HEX_RED, wordWrap: { width: CW - 40 } }).setOrigin(0.5).setDepth(80).setAlpha(0);
  ...
  onMissionComplete() {
    ...
    if (flawless) this.createFloatingText(OX + OW / 2, OY + 30, "FLAWLESS +100", HEX_AMBER, "bold 18px Arial");
  ...
  gameOver() {
    ...
    this._makeButton(525, 420, "REOPEN WORKSHOP", 200, 50, { stroke: C_RED, textColor: HEX_RED }, () => this.scene.restart());
  ...
  showScoreTally() {
    ...
    lines.forEach((s, i) => {
      const t = this.add.text(500, 220 + i * 28, s, { font: "16px Arial", color: HEX_GRAY }).setOrigin(0, 0.5).setDepth(91).setAlpha(0);
