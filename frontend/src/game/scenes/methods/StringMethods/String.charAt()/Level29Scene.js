...
export class Level29Scene extends Phaser.Scene {
  ...
  createTrialBanners() {
    [200, 1080].forEach((x) => {
      ...
      "TRIAL".split("").forEach((ch, i) => {
        this.add.text(x, 75 + i * 20, ch, { font: "bold 13px Arial", color: HEX_AMBER }).setOrigin(0.5).setAlpha(0.25).setDepth(4);
      });
    });
  }
  ...
  buildTileRow(decls) {
    ...
    if (decls.length === 1 && decls[0].value === "") {
      const t = this.add.text(640, TILE_Y, "(empty)", { font: "italic 16px Arial", color: "#3d4450" }).setOrigin(0.5).setDepth(12);
      this.emptyLabel = t;
      return;
    }
    ...
    if (decls.length === 1) {
      ...
      decls.forEach((d, di) => {
        ...
        const label = this.add.text(cursorX + halfW / 2, TILE_Y - 70, d.name, { font: "bold 13px Courier New", color: HEX_CYAN }).setOrigin(0.5).setDepth(12);
  ...
  createTile(ch, index, x, y, tw = 52, groupId = 0) {
    ...
    const charText = this.add.text(0, meta.isSpace ? -6 : 0, meta.display, { font: "bold 23px Courier New", color: meta.color }).setOrigin(0.5);
    ...
    const plateText = this.add.text(0, plateY, String(index), { font: "bold 14px Courier New", color: HEX_AMBER }).setOrigin(0.5);
  ...
  createResultDisplay() {
    ...
    this.add.text(RESULT_CX, RESULT_CY - 68, "EXTRACTED", { font: "bold 11px Arial", color: "#546e7a" }).setOrigin(0.5).setDepth(15);
    ...
    this.resultTypeTag = this.add.text(RESULT_CX, RESULT_CY + 26, "", { font: "bold 13px Courier New", color: "#4fc3f7" }).setOrigin(0.5).setDepth(15).setAlpha(0);
  }
  ...
  updateDeclarations(decls) {
    ...
    decls.forEach((d, i) => {
      ...
      const put = (t, color) => {
        const txt = this.add.text(x, y, t, { font: "16px Courier New", color }).setOrigin(0, 0.5);
        c.add(txt);
        x += txt.width;
      };
  ...
  createHUD() {
    ...
    this.add.text(20, 14, "THE CLAW TRIALS", { font: "bold 17px Arial", color: "#b0bec5" }).setDepth(51);
    this.add.text(20, 36, "Tuning Phase — String Methods: charAt()", { font: "13px Arial", color: "#546e7a" }).setDepth(51);
  ...
  bitSay(text) {
    this.hideBubble();
    const inner = this.add.text(0, 0, text, { font: "15px Arial", color: "#e0e0e0", wordWrap: { width: 330 } });
  ...
  createAnnotation(x, y, text, colorHex, arrowTarget = null) {
    ...
    const txt = this.add.text(x, y, text, { font: "bold 15px Arial", color: colorHex }).setOrigin(0.5);
    c.add(txt);
    if (arrowTarget) {
      const g = this.add.graphics();
  ...
  spawnPod(cfg) {
    ...
    if (cfg.round !== undefined) {
      const label = this.add.text(-width / 2 + 10, -height / 2 + 10, `TRIAL ${cfg.round}/15`, {
        font: "bold 11px Courier New", color: "#546e7a",
      }).setOrigin(0, 0.5);
      container.add(label);
    }
    ...
    if (cfg.podLines) {
      ...
      if (lw > width - 40) { lc.destroy(); fontSize = 15; ({ container: lc, width: lw } = this._renderTokenLine(0, 0, text, fontSize)); }
      lc.x = -lw / 2;
      container.add(lc);
    }
  ...
  _renderTokenLine(x, y, line, fontSize = 18) {
  ...
  _renderClickableTokenLine(parent, x, y, line, maxWidth) {
    ...
    const measured = tokens.map((tok) => {
      const t = this.add.text(0, 0, tok.t, { font: "15px Courier New", color: tok.c });
      const w = t.width;
      t.destroy();
      return w;
    });
    ...
    tokens.forEach((tok, i) => {
      const t = this.add.text(cx, y, tok.t, { font: "15px Courier New", color: tok.c }).setOrigin(0, 0.5);
  ...
  async clawExtract(index, groupId = 0) {
    ...
    const flyChar = this.add.text(tile.x, TILE_Y, tile.ch === " " ? "␣" : tile.ch, {
      font: "bold 23px Courier New", color: tile.ch === " " ? HEX_MAGENTA : this._charMeta(tile.ch).color,
  ...
  async clawDoubleExtract(i1, i2, groupId = 0) {
    ...
    const eq = this.add.text(RESULT_CX, RESULT_CY - 8, "==", { font: "bold 21px Arial", color: HEX_GRAY }).setOrigin(0.5).setDepth(16);
  ...
  showExceptionStamp(index) {
    const t = this.add.text(1280, 250, `StringIndexOutOfBoundsException: index ${index}`, {
      font: "bold 15px Courier New", color: HEX_RED,
  ...
  showBracketContrastCard() {
    ...
    const l1 = this.add.text(0, -26, "array  → arr[0] ✓", { font: "bold 14px Courier New", color: HEX_AMBER }).setOrigin(0.5);
    const l2 = this.add.text(0, 0, "String → s.charAt(0) ✓", { font: "bold 14px Courier New", color: HEX_CYAN }).setOrigin(0.5);
    const l3 = this.add.text(0, 26, "String → s[0] ✗", { font: "bold 14px Courier New", color: HEX_RED }).setOrigin(0.5);
    c.add([g, l1, l2, l3]);
    c.setAlpha(0);
  ...
  async showEvaluationFloat(lines) {
    for (let i = 0; i < lines.length; i++) {
      if (!this._alive) return;
      const t = this.add.text(640, 490 - i * 22, lines[i], { font: "bold 15px Courier New", color: HEX_AMBER }).setOrigin(0.5).setDepth(20).setAlpha(0);
      this.roundElements.push(t);
  ...
  showBooleanVerdict(value) {
    const t = this.add.text(RESULT_CX, RESULT_CY + 45, String(value), {
      font: "bold 19px Arial", color: value ? HEX_GREEN : HEX_RED,
  ...
  async runWalkerReveal(startIndex, str) {
    ...
    if (startIndex > 0) {
      ...
      const skip = this.add.text(startX, TILE_Y + 40, "skipped!", { font: "12px Arial", color: HEX_AMBER }).setOrigin(0, 0.5).setDepth(17);
  ...
  showOptionBubbles(options, onSelect) {
    ...
    const style = { font: "bold 17px Courier New", color: HEX_CYAN };
  ...
  showJudgmentButtons(onSelect) {
    const mk = (x, label, color, key) => {
      ...
      const t = this.add.text(0, -6, label, { font: "bold 17px Arial", color: hex }).setOrigin(0.5);
      const keyHint = this.add.text(0, 16, `[${key}]`, { font: "12px Arial", color: "#546e7a" }).setOrigin(0.5);
      c.add([g, t, keyHint]);
      c.setSize(180, 54);
      c.setInteractive({ useHandCursor: true });
    ...
    this.showJudgmentButtons((choice) => {
      ...
      this._resolveAnswer(cfg, correct, choice, correct ? null : cfg.wrongTag, async () => {
        if (cfg.crashType === "compile") {
          ...
          await this.delay(300);
          if (!this._alive) return;
          const anno = this.add.text(640, 62, cfg.explanation, { font: "bold 14px Arial", color: HEX_RED, wordWrap: { width: 560 } }).setOrigin(0.5).setDepth(60).setAlpha(0);
          ...
          if (!this._alive) return;
          const anno = this.add.text(640, 490, cfg.explanation, { font: "bold 14px Arial", color: HEX_RED, wordWrap: { width: 560 } }).setOrigin(0.5).setDepth(20).setAlpha(0);
          this.roundElements.push(anno);
          this.tweens.add({ targets: anno, alpha: 1, duration: 250 });
          await this.delay(500);
        } else {
  ...
  setupBugHunt(cfg) {
    const strip = this.add.text(640, 265, "CLICK THE BUG", { font: "bold 15px Arial", color: HEX_MAGENTA }).setOrigin(0.5).setDepth(25);
  ...
  onTokenClicked(token, lines, cfg) {
    ...
    this._resolveAnswer(cfg, correct, token.raw, correct ? null : cfg.wrongTag, async () => {
      if (correct) {
        ...
        const fixed = this.add.text(token.text.x, token.text.y - 18, cfg.fixedToken, { font: "bold 15px Courier New", color: HEX_GREEN }).setOrigin(0, 0.5).setAlpha(0).setDepth(36);
  ...
  onCorrectAnswer(cfg, timePctUsed) {
    ...
    if (remaining > 0.6) { points += 50; this.fastBonusCount++; this.createFloatingText(640, 300, "⚡ FAST +50", HEX_AMBER, "bold 17px Arial"); }
    else if (remaining > 0.3) { points += 25; this.fastBonusCount++; this.createFloatingText(640, 300, "⚡ FAST +25", HEX_AMBER, "bold 17px Arial"); }
  ...
  gameOver() {
    ...
    this.add.text(640, 350, `Trials Passed: ${this.currentRound} / ${ROUNDS.length}`, { font: "18px Arial", color: HEX_GRAY }).setOrigin(0.5).setDepth(91);

    this._makeButton(525, 420, "RETAKE TRIALS", 200, 50, { stroke: C_RED, textColor: HEX_RED }, () => this.scene.restart());
  ...
  levelComplete() {
    ...
    try { GameManager.completeLevel(29, Math.round(accuracy * 100)); } catch (_) {}
  ...
  showScoreTally(accuracy, avgTimePct) {
    ...
    lines.forEach((s, i) => {
      const t = this.add.text(500, 250 + i * 30, s, { font: "16px Arial", color: HEX_GRAY }).setOrigin(0, 0.5).setDepth(91).setAlpha(0);
    ...
    const badgeLbl = this.add.text(640, 533, "charAt() SCHEMA TUNED", { font: "bold 15px Arial", color: HEX_AMBER }).setOrigin(0.5).setDepth(91).setAlpha(0);
