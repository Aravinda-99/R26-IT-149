...
export class Level57Scene extends Phaser.Scene {
  ...
  createChamberInterior() {
    ...
    this.add.text(590, 25, "T H E   C A L C U L A T I O N   C H A M B E R", { font: "bold 12px Georgia", color: HEX_BRASS }).setOrigin(0.5).setAlpha(0.7).setDepth(3);
  }
  ...
  createResultsBoard() {
    ...
    const t = this.add.text(0, 0, "IN COMPUTATION", { font: "bold 9px Georgia", color: HEX_BLUE_GRAY }).setOrigin(0.5);
    c.add([g, t]);
    c.setAlpha(0.4);
  ...
  createCodeCanvas() {
    ...
    this.tabFilename = this.add.text(CX + CW - 12, CY + TAB_H / 2, "Calc1.java", { font: "11px Courier New", color: "#546e7a" }).setOrigin(1, 0.5).setDepth(11);
  ...
  createBlockPalette() {
    ...
    this.add.text(PX + 10, PY + 8, "CALCULATOR'S PARTS", { font: "bold 9px Arial", color: "#3d4450" }).setDepth(11);
  ...
  createRigWindow() {
    ...
    this.add.text(OX + 10, OY + 6, "CALCULATION RIG — LIVE", { font: "bold 9px Georgia", color: HEX_BRASS }).setAlpha(0.7).setDepth(11);
  ...
  createMiniComparator() {
    ...
    const npT = this.add.text(MC_BEAM_CX, MC_BEAM_Y - 16, "Math", { font: "bold 9px Courier New", color: HEX_GOLD }).setOrigin(0.5);
    ...
    this.mcModeText = this.add.text(MC_BEAM_CX, MC_BEAM_Y + 9, ".max", { font: "bold 8px Courier New", color: HEX_CYAN }).setOrigin(0.5);
  ...
  async mcInstanceCallShudder(value, type) {
    ...
    const q = this.add.text(star.container.x, star.container.y - 18, "?", { font: "bold 12px Georgia", color: HEX_RED }).setOrigin(0.5);
    this.rigLayer.add(q);
  ...
  async placeOnRail(entries, bracket) {
    ...
    for (const e of entries) {
      ...
      const lbl = this.add.text(0, -9, e.type === "double" ? Number(e.value).toFixed(1) : String(e.value), { font: "bold 7px Courier New", color: e.type === "double" ? HEX_ORANGE : HEX_GOLD }).setOrigin(0.5);
      m.add([dot, lbl]);
  ...
  createMiniContainers() {
    const hdr = this.add.text(CONT_X, CONT_Y0 - 14, "VARIABLES", { font: "bold 7px Georgia", color: HEX_BRASS }).setAlpha(0.7);
  ...
  miniDispenseTo(name, value, type) {
    if (!this.containerObjs[name]) {
      ...
      const t = this.add.text(CONT_X + 5, y + 6.5, "", { font: "bold 7px Courier New", color: HEX_CYAN }).setOrigin(0, 0.5);
  ...
  createMiniSurveyTracker() {
    const hdr = this.add.text(TRK_X, TRK_Y0 - 14, "SURVEY TRACKER", { font: "bold 7px Georgia", color: HEX_BRASS }).setAlpha(0.7);
    this.trackerContainer = this.add.container(0, 0);
    this.rigLayer.add([hdr, this.trackerContainer]);
    this._trackerRows = [];
    this._trackerDash = this.add.text(TRK_X + TRK_W / 2, TRK_Y0 + 60, "—", { font: "bold 11px Courier New", color: "#3a2618" }).setOrigin(0.5);
  ...
  appendTrackerRow(text, isCrash, bold) {
    ...
    const t = this.add.text(TRK_X, y, text, { font: "7px Courier New", color: isCrash ? HEX_RED : (bold ? HEX_GOLD : "#e8dfc8") }).setAlpha(0);
  ...
  clearTracker() {
    ...
    this._trackerRows = [];
    if (!this._trackerDash) { this._trackerDash = this.add.text(TRK_X + TRK_W / 2, TRK_Y0 + 60, "—", { font: "bold 11px Courier New", color: "#3a2618" }).setOrigin(0.5); this.rigLayer.add(this._trackerDash); }
  }
  ...
  populateMiniShelf(initialList) {
    ...
    const hdr = this.add.text(CONT_X, SHELF_Y - 14, "readings", { font: "bold 7px Georgia", color: HEX_BRASS }).setAlpha(0.7);
    ...
    this.currentList.forEach((entry, i) => {
      ...
      const t = this.add.text(0, -1, String(entry.value), { font: "bold 7px Courier New", color: "#0a0704" }).setOrigin(0.5);
      if (t.width > 20) t.setFontSize(5.5);
      const lbl = this.add.text(0, 12, `[${i}]`, { font: "bold 6px Courier New", color: HEX_GRAY }).setOrigin(0.5);
      c.add([g, t, lbl]);
  ...
  async retrieveGhost(index) {
    ...
    const t = this.add.text(0, 0, String(entry.value), { font: "bold 7px Courier New", color: "#0a0704" }).setOrigin(0.5);
    ghost.add([g, t]);
  ...
  createHUD() {
    ...
    this.add.text(20, 14, "THE CALCULATION CHAMBER", { font: "bold 13px Georgia", color: "#b0bec5" }).setDepth(51);
    this.add.text(20, 32, "Restructuring Phase — Math Methods: max() & min()", { font: "10px Arial", color: "#546e7a" }).setDepth(51);
  ...
  async crashGet(index) {
    const stamp = this.add.text(OX + OW / 2, OY + 120, "IndexOutOfBoundsException", { font: "bold 9px Courier New", color: HEX_RED }).setOrigin(0.5).setAngle(-6).setAlpha(0);
  ...
  gameOver() {
    ...
    (async () => {
      ...
      this._makeButton(640, 420, "RESUME THE NIGHT'S WORK", 270, 50, { stroke: C_RED, textColor: HEX_RED }, () => this.scene.restart());
    })();
  }
  ...
  levelComplete() {
    ...
    try { GameManager.completeLevel(56, Math.round((this.flawlessCount / MISSIONS.length) * 100)); } catch (_) {}
  ...
  showScoreTally() {
    ...
    const badgeLbl = this.add.text(640, 462, "max()/min() MASTERY", { font: "bold 13px Georgia", color: HEX_GOLD }).setOrigin(0.5).setDepth(91).setAlpha(0);
    ...
    const progressLbl = this.add.text(640, barY + 26, "MATH WING — 1 of 3 trilogies complete", { font: "bold 11px Georgia", color: HEX_CYAN }).setOrigin(0.5).setDepth(91).setAlpha(0);
