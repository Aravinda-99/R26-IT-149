...
export class Level54Scene extends Phaser.Scene {
  ...
  createGrandHallInterior() {
    ...
    this.add.text(400, 26, "THE GRAND RESHELVING", { font: "bold 13px Georgia", color: HEX_GOLD }).setOrigin(0.5).setAlpha(0.8).setDepth(3);
  }
  ...
  createCodeCanvas() {
    ...
    this.tabFilename = this.add.text(CX + CW - 12, CY + TAB_H / 2, "Reshelve1.java", { font: "11px Courier New", color: "#546e7a" }).setOrigin(1, 0.5).setDepth(11);
  ...
  createBlockPalette() {
    ...
    this.add.text(PX + 10, PY + 8, "RESHELVER'S PARTS", { font: "bold 9px Arial", color: "#3d4450" }).setDepth(11);
  ...
  createRigWindow() {
    ...
    this.add.text(OX + 10, OY + 6, "RESHELVING RIG — LIVE", { font: "bold 9px Georgia", color: HEX_BRASS }).setAlpha(0.7).setDepth(11);
  ...
  createMiniBookshelf() {
    ...
    this.typeStampText = this.add.text(MS_CX, OY + 42, "", { font: "bold 7px Courier New", color: HEX_CYAN }).setOrigin(0.5);
    this.sizeCounterText = this.add.text(MS_CX, MS_BASE_Y + 22, "size: 0", { font: "bold 7px Courier New", color: HEX_BRASS }).setOrigin(0.5);
    ...
    for (let i = 0; i < 6; i++) {
      ...
      const idxText = this.add.text(MS_X0 + 9, y, String(i), { font: "bold 7px Courier New", color: HEX_GRAY }).setOrigin(0.5);
  ...
  createMiniCrate() {
    ...
    this.rigLayer.add(g);
    this.crateLabel = this.add.text(MS_CX, CRATE_Y0 + 4, "WITHDRAWN", { font: "bold 6px Georgia", color: HEX_STAMP_RED }).setOrigin(0.5).setAlpha(0.5).setAngle(-2);
  ...
  _makeBookVisual(entry, x, y) {
    ...
    const txt = this.add.text(0, 0, display, { font: "bold 8px Georgia", color: "#0a0704" }).setOrigin(0.5);
    if (display.length > 5) { txt.setAngle(-90); if (txt.width > 22) txt.setFontSize(6); }
    const label = this.add.text(0, 16, "", { font: "bold 6px Courier New", color: HEX_BRASS }).setOrigin(0.5).setAlpha(0.6);
    c.add([g, txt, label]);
  ...
  async withdrawBook(index, opts = {}) {
    ...
    const mark = this.add.text(0, 0, "X", { font: "bold 10px Georgia", color: HEX_STAMP_RED }).setOrigin(0.5).setAngle(-20).setAlpha(0);
  ...
  _popReturnValue(entry, fromX, fromY) {
    const valText = this.add.text(fromX, fromY, String(entry.value), { font: "bold 11px Courier New", color: this._typeColorHex(entry.type) }).setOrigin(0.5).setDepth(30);
  ...
  flagArgumentAsIndex(value) {
    const t = this.add.text(OX + OW / 2, STRIP_Y - 30, "index", { font: "bold 8px Courier New", color: HEX_GOLD }).setOrigin(0.5).setDepth(20).setAlpha(0);
  ...
  renderBoxedArgument(value) {
    ...
    const t = this.add.text(0, 0, String(value), { font: "bold 10px Courier New", color: HEX_GOLD }).setOrigin(0.5);
    const flag = this.add.text(0, -16, "Integer", { font: "bold 6px Courier New", color: HEX_CYAN }).setOrigin(0.5);
    c.add([g, t, flag]);
  ...
  async crashWithdrawal(index) {
    ...
    const stamp = this.add.text(OX + OW / 2, OY + 120, "IndexOutOfBoundsException", { font: "bold 9px Courier New", color: HEX_RED }).setOrigin(0.5).setAngle(-6).setAlpha(0).setDepth(25);
  ...
  showSkippedFlag(index) {
    ...
    const t = this.add.text(MS_X1 + 40, y, "skipped!", { font: "italic 8px Georgia", color: HEX_RED }).setOrigin(0.5).setDepth(20).setAlpha(0);
    this.missionElements.push(t);
  ...
  createMiniListStatePanel() {
    const hdr = this.add.text(LP_X, OY + 40, "LIST STATE", { font: "bold 7px Georgia", color: HEX_BRASS }).setAlpha(0.7);
    this.bracketText = this.add.text(LP_CX, OY + 78, "[]", { font: "bold 9px Courier New", color: HEX_GRAY, wordWrap: { width: 130 }, align: "center" }).setOrigin(0.5);
    this.panelSizeText = this.add.text(LP_CX, OY + 110, "size: 0", { font: "8px Courier New", color: HEX_BRASS }).setOrigin(0.5).setAlpha(0.85);
    this.panelIndexText = this.add.text(LP_CX, OY + 124, "", { font: "bold 7px Courier New", color: "#8a6435", wordWrap: { width: 130 }, align: "center" }).setOrigin(0.5).setAlpha(0.7);
    this.add.text(LP_X, OY + 148, "removed:", { font: "7px Georgia", color: "#8a6435" });
    this.removedValueText = this.add.text(LP_X + 46, OY + 148, "—", { font: "bold 9px Courier New", color: HEX_GRAY }).setOrigin(0, 0.5);
    this.add.text(LP_X, OY + 166, "before:", { font: "6px Courier New", color: "#8a6435" });
    this.beforeRowText = this.add.text(LP_X + 38, OY + 166, "", { font: "bold 6px Courier New", color: "#b0bec5" });
    this.add.text(LP_X, OY + 178, "after:", { font: "6px Courier New", color: "#8a6435" });
    this.afterRowText = this.add.text(LP_X + 38, OY + 178, "", { font: "bold 6px Courier New", color: HEX_GREEN_BRIGHT });
  ...
  createMiniMutationTracker() {
    const hdr = this.add.text(TRK_X, OY + 40, "MUTATION TRACKER", { font: "bold 7px Georgia", color: HEX_BRASS }).setAlpha(0.7);
    this.trackerContainer = this.add.container(0, 0);
    this.rigLayer.add([hdr, this.trackerContainer]);
    this._trackerRows = [];
    this._trackerDash = this.add.text(TRK_X + TRK_W / 2, OY + 110, "—", { font: "bold 12px Courier New", color: "#3a2618" }).setOrigin(0.5);
  ...
  appendTrackerRow(text, isCrash) {
    ...
    const t = this.add.text(TRK_X, y, text, { font: "8px Courier New", color: isCrash ? HEX_RED : "#e8dfc8" }).setAlpha(0);
  ...
  clearTracker() {
    ...
    this._trackerRows = [];
    if (!this._trackerDash) { this._trackerDash = this.add.text(TRK_X + TRK_W / 2, OY + 110, "—", { font: "bold 12px Courier New", color: "#3a2618" }).setOrigin(0.5); this.rigLayer.add(this._trackerDash); }
  }
  ...
  createMiniCrossWingCameos() {
    ...
    this.pressText = this.add.text(TRK_X + (TRK_W - 20) / 2, OY + 203, "", { font: "bold 7px Courier New", color: HEX_ORANGE }).setOrigin(0.5);
  ...
  miniDispenseTo(name, value, javaType) {
    ...
    if (!this.containerObjs[name]) {
      ...
      const t = this.add.text(LP_X + 5, y + 7, "", { font: "bold 6px Courier New", color: HEX_CYAN }).setOrigin(0, 0.5);
  ...
  buildReportRows(mission) {
    ...
    mission.tests.forEach((test, i) => {
      ...
      const expT = this.add.text(150, 0, `[${(test.expectedList || []).join(",")}]`.slice(0, 20), { font: "9px Courier New", color: "#78909c" }).setOrigin(0, 0.5);
      const outStatusT = this.add.text(RW - 44, 0, "", { font: "9px Arial", color: "#78909c" }).setOrigin(0.5);
      const listStatusT = this.add.text(RW - 20, 0, "…", { font: "13px Arial", color: "#78909c" }).setOrigin(0.5);
  ...
  createHUD() {
    ...
    this.add.text(20, 14, "THE GRAND RESHELVING", { font: "bold 13px Georgia", color: "#b0bec5" }).setDepth(51);
    this.add.text(20, 32, "Restructuring Phase — ArrayList Methods: remove()", { font: "10px Arial", color: "#546e7a" }).setDepth(51);
  ...
  gameOver() {
    ...
    (async () => {
      ...
      this._makeButton(640, 420, "RESUME THE RESHELVING", 250, 50, { stroke: C_RED, textColor: HEX_RED }, () => this.scene.restart());
    })();
  }
  ...
  async ceremonyPhase3_CentralPanel() {
    ...
    const totalText = this.add.text(480, 148 + 7 * 24, "TOTAL: 0", { font: "bold 20px Arial", color: HEX_GOLD }).setOrigin(0, 0.5).setDepth(91).setAlpha(0);
    ...
    const badgeLbl = this.add.text(640, 488, "remove() MASTERY", { font: "bold 13px Georgia", color: HEX_GOLD }).setOrigin(0.5).setDepth(91).setAlpha(0);
    const badgeSub = this.add.text(640, 504, "Accretion ✓  Tuning ✓  Restructuring ✓", { font: "10px Arial", color: "#78909c" }).setOrigin(0.5).setDepth(91).setAlpha(0);
  ...
  async ceremonyPhase4_WingSeal() {
    ...
    const wingTitle = this.add.text(0, -20, "ARRAYLIST WING — COMPLETE", { font: "bold 16px Georgia", color: HEX_GOLD }).setOrigin(0.5);
    ...
    const caption = this.add.text(640, 600, "9 levels · 3 methods · 1 living collection", { font: "italic 11px Georgia", color: HEX_BRASS }).setOrigin(0.5).setDepth(93).setAlpha(0);
