...
export class Level79Scene extends Phaser.Scene {
  ...
  createBureauInterior() {
    ...
    this._instrumentSilhouettes = silDefs.map((d) => {
      ...
      const lbl = this.add.text(0, 30, d.label, { font: "6px Georgia", color: d.color === C_COPPER ? HEX_COPPER : d.color === C_ORANGE ? HEX_ORANGE : HEX_CREAM }).setOrigin(0.5);
      c.add([sg, lbl]);
    ...
    this.add.text(640, 25, "T H E   A S S A Y   B U R E A U", { font: "bold 13px Georgia", color: HEX_GOLD }).setOrigin(0.5).setAlpha(0.8).setDepth(3);
  }
  ...
  createWingCrest() {
    ...
    const tStr = this.add.text(pStr.x, pStr.y - 10, "String", { font: "bold 7px Georgia", color: HEX_CREAM }).setOrigin(0.5);
    const tInt = this.add.text(pInt.x, pInt.y + 10, "int", { font: "bold 7px Georgia", color: HEX_GOLD }).setOrigin(0.5);
    const tDbl = this.add.text(pDbl.x, pDbl.y + 10, "double", { font: "bold 7px Georgia", color: HEX_ORANGE }).setOrigin(0.5);
  ...
  createCodeCanvas() {
    ...
    this.tabFilename = this.add.text(CX + CW - 12, CY + TAB_H / 2, "Bureau1.java", { font: "11px Courier New", color: "#546e7a" }).setOrigin(1, 0.5).setDepth(11);
  ...
  renderSkeleton(mission) {
    ...
    mission.skeleton.forEach((rawLine, i) => {
      ...
      if (this._isDimmedInfrastructure(rawLine)) {
        const t = this.add.text(CODE_X, y, rawLine, { font: "11px Courier New", color: "#3d4450" }).setAlpha(0.6);
      ...
      parts.forEach((part, pi) => {
        if (pi % 2 === 0) {
          ...
          this._syntaxTokens(part).forEach((tok) => {
            const t = this.add.text(x, y, tok.t, { font: "bold 11px Courier New", color: tok.c });
            this.codeContainer.add(t);
            x += t.width;
          });
        } else {
  ...
  _drawSlotPlaceholder(slotId) {
    ...
    if (!filled) {
      const label = this.add.text(x + w / 2, y + h / 2, def.hint, { font: "italic 8px Courier New", color: "#3d4450" }).setOrigin(0.5).setDepth(22);
      def.hintLabel = label;
  ...
  createBlockPalette() {
    ...
    this.add.text(PX + 10, PY + 8, "ASSAYER'S BUREAU PARTS", { font: "bold 9px Arial", color: "#3d4450" }).setDepth(11);
  ...
  populatePalette(mission) {
    ...
    shuffled.forEach((def) => {
      const style = { font: "bold 10px Courier New", color: HEX_CYAN };
  ...
  createRigWindow() {
    ...
    this.add.text(OX + 10, OY + 4, "BUREAU RIG — LIVE", { font: "bold 9px Georgia", color: HEX_GOLD }).setAlpha(0.7).setDepth(11);
  ...
  createTripleApparatus() {
    const drawFrame = (x0, x1, label, colorHex, labelHex) => {
      ...
      this.rigLayer.add(g);
      const t = this.add.text((x0 + x1) / 2, MINI_Y0 - 9, label, { font: "bold 6px Georgia", color: labelHex }).setOrigin(0.5).setAlpha(0.5);
      this.rigLayer.add(t);
      return { g, t };
    };
    ...
    this._mfContText = this.add.text(mfCx, MINI_Y1 - 9, "int —", { font: "bold 7px Courier New", color: HEX_GRAY }).setOrigin(0.5);
    this._mcContText = this.add.text(mcCx, MINI_Y1 - 9, "double —", { font: "bold 7px Courier New", color: HEX_GRAY }).setOrigin(0.5);
    this._mpContText = this.add.text(mpCx, MINI_Y1 - 9, "String —", { font: "bold 7px Courier New", color: HEX_GRAY }).setOrigin(0.5);
    this.rigLayer.add([this._mfContText, this._mcContText, this._mpContText]);

    this.routeArrow = this.add.text(ROUTE_ARROW_X, ROUTE_ARROW_Y, "?", { font: "bold 10px Georgia", color: HEX_GRAY }).setOrigin(0.5).setAlpha(0.4);
  ...
  _miniStrip(layer, cx, value) {
    ...
    const txt = this.add.text(0, 0, value, { font: "bold 6px Courier New", color: "#241a0e" }).setOrigin(0.5);
  ...
  async runFurnaceConversion(strValue) {
    ...
    if (!valid) {
      ...
      const nfe = this.add.text(cx, MINI_Y0 + 55, "NFE", { font: "bold 7px Courier New", color: HEX_RED }).setOrigin(0.5).setAlpha(0);
    ...
    const txt = this.add.text(0, 0, String(value), { font: "bold 6px Courier New", color: "#241a0e" }).setOrigin(0.5);
    bar.add([bg, txt]);
  ...
  async runPressConversion(value, type, strRepr) {
    ...
    const chipTxt = this.add.text(0, 0, inputDisplay, { font: "bold 6px Courier New", color: "#1a1408" }).setOrigin(0.5);
  ...
  async runCastAnimation(doubleValue, intResult) {
    ...
    const funnel = this.add.text((mcCx + mfCx) / 2, funnelY, "(int)", { font: "bold 6px Courier New", color: HEX_GOLD }).setOrigin(0.5).setAlpha(0);
    ...
    drop.destroy();
    const bar = this.add.text(mfCx, MINI_Y0 + 48, String(intResult), { font: "bold 7px Courier New", color: HEX_GOLD }).setOrigin(0.5).setAlpha(0);
  ...
  showCompileErrorStamp() {
    const stamp = this.add.text(CX + CW / 2, CY + CH / 2, "COMPILE ERROR", { font: "bold 16px Arial", color: HEX_RED }).setOrigin(0.5).setDepth(70).setScale(1.3).setAngle(-6).setAlpha(0);
  ...
  showRuntimeHaltStamp() {
    const stamp = this.add.text(CX + CW / 2, CY + CH / 2, "BUILD HALTED", { font: "bold 15px Arial", color: HEX_RED }).setOrigin(0.5).setDepth(70).setScale(1.2).setAngle(-4).setAlpha(0);
    this.missionElements.push(stamp);
    this.tweens.add({ targets: stamp, scale: 1, alpha: 1, duration: 130 });
    this.screenShake(0.004, 110);
    this.time.delayedCall(750, () => { if (stamp.active) this.tweens.add({ targets: stamp, alpha: 0, duration: 160, onComplete: () => stamp.destroy() }); });
  ...
  createMiniArithmeticStage() {
    ...
    this.rigLayer.add(g);
    const t = this.add.text(STAGE_X0 + 4, STAGE_Y0 + 1, "MATH", { font: "bold 6px Georgia", color: HEX_INDIGO }).setAlpha(0.6);
    this.rigLayer.add(t);
  ...
  async runArithmeticAnimation(aVal, op, bVal, result) {
    ...
    const opText = this.add.text(midX, cy, op, { font: "bold 10px Georgia", color: HEX_INDIGO }).setOrigin(0.5).setAlpha(0);
  ...
  renderMiniTape() {
    ...
    this.tapeState.slice(-Math.floor(totalW / cellW)).forEach((cell, i) => {
      ...
      const t = this.add.text(x, TAPE_Y, disp, { font: "bold 5px Courier New", color }).setOrigin(0.5);
  ...
  createMiniOutputTicker() {
    ...
    this.tickerText = this.add.text(OX + 14, TICKER_Y, "", { font: "bold 8px Courier New", color: HEX_GREEN_BRIGHT }).setOrigin(0, 0.5);
  ...
  createVariablesStrip() {
    const hdr = this.add.text(OX + 14, VARS_Y - 10, "VARIABLES", { font: "bold 7px Georgia", color: HEX_GOLD }).setAlpha(0.7);
  ...
  createManifestStrip() {
    ...
    this.manifestStripText = this.add.text(OX + 8, MANIFEST_Y + 8, "", { font: "10px Arial", color: HEX_GOLD }).setOrigin(0, 0.5).setDepth(17);
  ...
  buildReportRows(mission) {
    ...
    mission.tests.forEach((test, i) => {
      ...
      const expT = this.add.text(190, 0, test.expectedOutput.replace(/⏎/g, " / ").slice(0, 22), { font: "9px Courier New", color: "#78909c" }).setOrigin(0, 0.5);
  ...
  createHUD() {
    ...
    this.add.text(20, 14, "THE ASSAY BUREAU", { font: "bold 13px Georgia", color: "#b0bec5" }).setDepth(51);
    this.add.text(20, 32, "Restructuring Phase — Type Conversion: valueOf()", { font: "10px Arial", color: "#546e7a" }).setDepth(51);
  ...
  onMissionComplete() {
    ...
    if (flawless) this.createFloatingText(OX + OW / 2, OY - 12, "FLAWLESS +100", HEX_GOLD, "bold 14px Arial");
  ...
  gameOver() {
    ...
    (async () => {
      ...
      this.add.text(640, 350, `Missions Published: ${this.currentMission} / 6`, { font: "16px Arial", color: HEX_GRAY }).setOrigin(0.5).setDepth(91);
      this._makeButton(640, 420, "REOPEN THE BUREAU", 240, 50, { stroke: C_RED, textColor: HEX_RED }, () => this.scene.restart());
    })();
  }
  ...
  async ceremonyPhase2_InstrumentsAssemble() {
    ...
    const makeMiniIcon = (startX, startY, endX, endY, colorHex, colorText, label) => {
      ...
      const t = this.add.text(0, 24, label, { font: "bold 8px Georgia", color: colorText }).setOrigin(0.5);
      c.add([g, t]);
  ...
  async ceremonyPhase3_CentralPanel() {
    ...
    lines.forEach((s, i) => {
      const t = this.add.text(370, 140 + i * 22, s, { font: "13px Arial", color: HEX_GRAY }).setOrigin(0, 0.5).setDepth(91).setAlpha(0);
      this.tweens.add({ targets: t, alpha: 1, duration: 250, delay: 300 + i * 110 });
      this._ceremonyElements.push(t);
    });
    const totalText = this.add.text(370, 140 + 7 * 22, "TOTAL: 0", { font: "bold 20px Arial", color: HEX_GOLD }).setOrigin(0, 0.5).setDepth(91).setAlpha(0);
    ...
    const pressIcon = this.add.text(-14, -6, "🖨️", { font: "bold 11px Arial" }).setOrigin(0.5);
    const wax = this.add.text(0, -6, "🕯️", { font: "bold 11px Arial" }).setOrigin(0.5);
    const chain = this.add.text(14, -6, "⛓️", { font: "bold 11px Arial" }).setOrigin(0.5);
    badge.add([bg, pressIcon, wax, chain]);
    this.tweens.add({ targets: badge, alpha: 1, duration: 300, delay: 2250 });
    const badgeLbl = this.add.text(640, 492, "valueOf() MASTERY", { font: "bold 13px Georgia", color: HEX_GOLD }).setOrigin(0.5).setDepth(91).setAlpha(0);
    const badgeSub = this.add.text(640, 508, "Accretion ✓  Tuning ✓  Restructuring ✓", { font: "10px Arial", color: "#78909c" }).setOrigin(0.5).setDepth(91).setAlpha(0);
  ...
  async ceremonyPhase4_WingSeal() {
    ...
    bg.fillCircle(230, 0, 3);
    const title = this.add.text(0, -22, "TYPE CONVERSION WING — COMPLETE", { font: "bold 15px Georgia", color: HEX_GOLD }).setOrigin(0.5);
    banner.add([bg, title]);
    ...
    for (const col of cols) {
      const icon = this.add.text(banner.x + col.x - 14, banner.y + 8, col.icon, { font: "10px Arial" }).setOrigin(0.5).setDepth(93).setAlpha(0);
      const lbl = this.add.text(banner.x + col.x, banner.y + 8, col.label, { font: "bold 11px Georgia", color: col.color }).setOrigin(0.5).setDepth(93).setAlpha(0);
    ...
    const caption = this.add.text(640, banner.y + 28, "9 levels · 3 methods · one office of conversion", { font: "italic 11px Georgia", color: HEX_CYAN }).setOrigin(0.5).setDepth(93).setAlpha(0);
