...
export class Level47Scene extends Phaser.Scene {
  ...
  createCardCatalogBanner() {
    ...
    this.add.text(530, 83, "C A R D   C A T A L O G", { font: "bold 14px Georgia", color: HEX_BRASS }).setOrigin(0.5).setAlpha(0.7).setDepth(3);
  }
  ...
  createTrialCard() {
    ...
    this.cardLabel = this.add.text(CARD_X1 - 10, CARD_Y0 + 8, "", { font: "bold 9px Courier New", color: "#8a6435" }).setOrigin(1, 0).setAlpha(0.7).setDepth(7);
    this.cardQuestionText = this.add.text(CARD_CX, CARD_Y1 - 16, "", { font: "12px Georgia", color: "#241a0e" }).setOrigin(0.5).setDepth(7);
  ...
  createReferenceBookshelf() {
    ...
    this.sizeCounterText = this.add.text(SHELF_X1 - 45, 106, "size: 0", { font: "bold 10px Courier New", color: HEX_BRASS }).setOrigin(0.5).setDepth(6);
  ...
  _makeBookVisual(entry, x, y) {
    ...
    const label = this.add.text(0, 22, "", { font: "bold 7px Courier New", color: HEX_BRASS }).setOrigin(0.5).setAlpha(0.6);
    c.add([g, txt, label]);
  ...
  createBookPrepArea() {
    ...
    this.add.text(PREP_X0 + 6, PREP_Y0 + 4, "NEW ENTRY", { font: "bold 8px Georgia", color: HEX_BRASS }).setAlpha(0.7).setDepth(5);
  ...
  async rejectBook(book, idx) {
    ...
    const stamp = this.add.text(SHELF_CX, SHELF_Y0 + 50, "TYPE MISMATCH", { font: "bold 12px Courier New", color: HEX_RED }).setOrigin(0.5).setAngle(-8).setAlpha(0).setDepth(20);
  ...
  async rejectInvalidIndex(entry, targetIndex) {
    ...
    const stamp = this.add.text(SHELF_CX, SHELF_Y0 + 50, "IndexOutOfBoundsException", { font: "bold 10px Courier New", color: HEX_RED }).setOrigin(0.5).setAngle(-6).setAlpha(0).setDepth(20);
    this.tweens.add({ targets: stamp, alpha: 1, duration: 130 });
    this.screenShake(0.005, 160);
  ...
  showCompileErrorStamp() {
    const stamp = this.add.text(CARD_CX, CARD_Y0 - 14, "COMPILE ERROR", { font: "bold 16px Arial", color: HEX_RED }).setOrigin(0.5).setDepth(30).setScale(1.4).setAngle(-6).setAlpha(0);
  ...
  createReferenceListStatePanel() {
    ...
    this.bracketText = this.add.text(PANEL_X + PANEL_W / 2, PANEL_Y + 130, "[]", { font: "bold 13px Courier New", color: HEX_GRAY, wordWrap: { width: PANEL_W - 24 }, align: "center" }).setOrigin(0.5).setDepth(12);
    this.panelSizeText = this.add.text(PANEL_X + PANEL_W / 2, PANEL_Y + 200, "size: 0", { font: "10px Courier New", color: HEX_BRASS }).setOrigin(0.5).setAlpha(0.85).setDepth(12);
    this.panelIndexText = this.add.text(PANEL_X + PANEL_W / 2, PANEL_Y + 224, "", { font: "bold 9px Courier New", color: "#8a6435", wordWrap: { width: PANEL_W - 24 }, align: "center" }).setOrigin(0.5).setAlpha(0.7).setDepth(12);
  }
  ...
  createHUD() {
    ...
    this.add.text(20, 14, "THE CARD CATALOG", { font: "bold 15px Georgia", color: "#b0bec5" }).setDepth(50);
    this.add.text(20, 32, "Tuning Phase — ArrayList Methods: add()", { font: "11px Arial", color: "#546e7a" }).setDepth(50);
  ...
  async onBubbleSelected(opt, config, bubbleContainer) {
    ...
    if (config.revealNote) this.createFloatingText(PANEL_X + PANEL_W / 2 - 60, PANEL_Y + PANEL_H + 60, config.revealNote, HEX_GRAY, "11px Arial", 2800);
  ...
  setupBugHunt(config) {
    this.clearCard();
    const header = this.add.text(CARD_CX, CARD_Y0 + 16, "CLICK THE BUG", { font: "bold 13px Georgia", color: "#c62828" }).setOrigin(0.5);
  ...
  async onTokenClicked(tokenObj, config, lineY) {
    ...
    if (correct) {
      ...
      const fixT = this.add.text(CARD_CX, lineY - 16, config.fix, { font: "bold 11px Courier New", color: "#2e7d32" }).setOrigin(0.5).setAlpha(0);
  ...
  scoreForAttempt(timePctUsed) {
    ...
    if (remaining > 0.6) { points += 50; this.fastBonusCount++; this.createFloatingText(CARD_CX, CARD_Y0 - 20, "⚡ FILED CLEAN +50", HEX_GOLD, "bold 13px Arial", 900); }
    else if (remaining > 0.3) { points += 25; this.fastBonusCount++; this.createFloatingText(CARD_CX, CARD_Y0 - 20, "⚡ +25", HEX_GOLD, "bold 12px Arial", 800); }
    return points;
  }
  ...
  gameOver() {
    ...
    (async () => {
      ...
      this.add.text(640, 350, `Cards Filed: ${this.currentRound} / 15`, { font: "16px Arial", color: HEX_GRAY }).setOrigin(0.5).setDepth(91);
      this._makeButton(640, 420, "RELIGHT THE CANDLE", 240, 50, { stroke: C_RED, textColor: HEX_RED }, () => this.scene.restart());
    })();
  }
  ...
  showScoreTally() {
    ...
    const badgeLbl = this.add.text(640, 485, "add() SCHEMA TUNED", { font: "bold 13px Georgia", color: HEX_GOLD }).setOrigin(0.5).setDepth(91).setAlpha(0);
