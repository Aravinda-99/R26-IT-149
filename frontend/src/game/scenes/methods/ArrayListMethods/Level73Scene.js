...
export class Level73Scene extends Phaser.Scene {
  ...
  createWorksInterior() {
    ...
    this.add.text(100, 175, "VALIDATE BEFORE\nSMELTING", { font: "bold 6px Georgia", color: HEX_RED, align: "center" }).setOrigin(0.5).setAlpha(0.4).setDepth(3);
    ...
    this.add.text(590, 25, "T H E   C O N V E R S I O N   W O R K S", { font: "bold 12px Georgia", color: HEX_COPPER }).setOrigin(0.5).setAlpha(0.7).setDepth(3);
  }
  ...
  createCodeCanvas() {
    ...
    this.tabFilename = this.add.text(CX + CW - 12, CY + TAB_H / 2, "Convert1.java", { font: "11px Courier New", color: "#546e7a" }).setOrigin(1, 0.5).setDepth(11);
  ...
  createBlockPalette() {
    ...
    this.add.text(PX + 10, PY + 8, "ASSAYER'S PARTS", { font: "bold 9px Arial", color: "#3d4450" }).setDepth(11);
  ...
  createRigWindow() {
    ...
    this.add.text(OX + 10, OY + 6, "WORKS RIG — LIVE", { font: "bold 9px Georgia", color: HEX_COPPER }).setAlpha(0.7).setDepth(11);
  ...
  createMiniFurnace() {
    ...
    this.containerValueText = this.add.text(M_CONT_X, M_CONT_Y + 8, "int —", { font: "bold 9px Courier New", color: HEX_GRAY }).setOrigin(0.5);
  ...
  async showNFE() {
    ...
    const banner = this.add.text(M_FURNACE_CX, (M_CHAMBER_Y0 + M_CHAMBER_Y1) / 2, "NFE!", { font: "bold 10px Courier New", color: HEX_RED }).setOrigin(0.5).setScale(1.1).setAlpha(0);
  ...
  showRuntimeHaltStamp() {
    const stamp = this.add.text(OX + OW / 2, OY + 100, "BUILD HALTED", { font: "bold 12px Arial", color: HEX_RED }).setOrigin(0.5).setDepth(30).setScale(1.2).setAngle(-4).setAlpha(0);
    this.missionElements.push(stamp);
    this.rigLayer.add(stamp);
    this.tweens.add({ targets: stamp, scale: 1, alpha: 1, duration: 130 });
    this.screenShake(0.004, 110);
  ...
  createMiniArithmeticStage() {
    ...
    this.add.text(STAGE_X0 + 6, STAGE_Y0 + 4, "ARITHMETIC", { font: "bold 7px Georgia", color: HEX_INDIGO }).setAlpha(0.7);
    this.rigLayer.add(g);
  ...
  async runArithmeticAnimation(aVal, op, bVal, result) {
    ...
    const opText = this.add.text(midX, cy, op, { font: "bold 14px Georgia", color: HEX_INDIGO }).setOrigin(0.5).setAlpha(0);
  ...
  createVariablesStrip() {
    const hdr = this.add.text(OX + 490 - 456 + 14, VARS_Y - 12, "VARIABLES", { font: "bold 7px Georgia", color: HEX_COPPER }).setAlpha(0.7);
  ...
  updateVariablesStrip(vars) {
    ...
    for (const name in vars) {
      ...
      const t = this.add.text(OX + 20, y, text, { font: "bold 7px Courier New", color: v.type === "String" ? HEX_CYAN : HEX_GOLD }).setOrigin(0, 0.5);
  ...
  createManifestStrip() {
    ...
    this.manifestStripText = this.add.text(OX + 8, MANIFEST_Y + 8, "", { font: "10px Arial", color: HEX_COPPER }).setOrigin(0, 0.5).setDepth(17);
  ...
  createHUD() {
    ...
    this.add.text(20, 14, "THE CONVERSION WORKS", { font: "bold 13px Georgia", color: "#b0bec5" }).setDepth(51);
    this.add.text(20, 32, "Restructuring Phase — Type Conversion: parseInt()", { font: "10px Arial", color: "#546e7a" }).setDepth(51);
  ...
  createBit() {
    ...
    const medallionMark = this.add.text(0, 6, "⚒", { font: "7px Arial", color: "#241a0e" }).setOrigin(0.5);
  ...
  async triggerTrilogyFinale() {
    ...
    const totalText = this.add.text(400, 140 + 8 * 24, "TOTAL: 0", { font: "bold 20px Arial", color: HEX_GOLD }).setOrigin(0, 0.5).setDepth(91).setAlpha(0);
    ...
    const furnaceIcon = this.add.text(-14, -6, "🔥", { font: "bold 12px Arial", color: HEX_COPPER }).setOrigin(0.5);
    const gaugeIcon = this.add.text(0, -6, "⏱", { font: "bold 12px Arial", color: HEX_GOLD }).setOrigin(0.5);
    const ingotIcon = this.add.text(14, -6, "⚒", { font: "bold 11px Arial", color: HEX_COPPER }).setOrigin(0.5);
    badge.add([bg, furnaceIcon, gaugeIcon, ingotIcon]);
    this.tweens.add({ targets: badge, alpha: 1, duration: 300, delay: 2500 });
    const badgeLbl = this.add.text(640, 512, "parseInt() MASTERY", { font: "bold 13px Georgia", color: HEX_GOLD }).setOrigin(0.5).setDepth(91).setAlpha(0);
    ...
    const progLabel = this.add.text(640, barY + 26, "TYPE CONVERSION WING — 1 of 3 trilogies complete", { font: "bold 11px Georgia", color: HEX_CYAN }).setOrigin(0.5).setDepth(91).setAlpha(0);
