...
export class Level80Scene extends Phaser.Scene {
  ...
  createInstituteInterior() {
    ...
    this._chartColumns = cols.map((c) => {
      ...
      const lbl = this.add.text(c.x, 72, c.label, { font: "bold 8px Georgia", color: c.color }).setOrigin(0.5).setAlpha(0.6).setDepth(2);
      const sample = this.add.text(c.x, 100, c.sample, { font: "9px Courier New", color: c.color }).setOrigin(0.5).setAlpha(0.4).setDepth(2);
    ...
    this.add.text(640, 25, "T H E   N U M E R A L   L O U P E", { font: "bold 13px Georgia", color: HEX_SILVER }).setOrigin(0.5).setAlpha(0.7).setDepth(3);
  }
  ...
  createGemologistsSlate() {
    ...
    this.add.text(SLATE_X + 14, SLATE_Y + 12, "GEMOLOGIST'S SLATE", { font: "bold 10px Georgia", color: HEX_SILVER }).setDepth(11);
    ...
    this.add.text(SLATE_X + SLATE_W - 101, SLATE_Y + 16, "Character (wrapper class)", { font: "bold 7px Courier New", color: HEX_SILVER }).setOrigin(0.5).setDepth(12);

    this.add.text(SLATE_X + 14, SLATE_Y + 30, "char — single quotes, one character", { font: "italic 9px Georgia", color: HEX_SILVER }).setDepth(11);
  ...
  chalkEvaluationArrow(value) {
    const t = this.add.text(SLATE_X + 14, this._slateY, `→ ${value}`, { font: "bold 12px Courier New", color: value === "true" ? HEX_GREEN_BRIGHT : HEX_GRAY }).setAlpha(0);
    this.slateLines.add(t);
    if (t.width > SLATE_W - 28) t.setFontSize(10);
  ...
  createHUD() {
    ...
    this.add.text(20, 14, "THE NUMERAL LOUPE", { font: "bold 14px Georgia", color: "#b0bec5" }).setDepth(50);
    this.add.text(20, 32, "Accretion Phase — Character Methods: isDigit()", { font: "10px Arial", color: "#546e7a" }).setDepth(50);
  ...
  createClassificationLoupe() {
    ...
    this.verdictText = this.add.text(VERDICT_X + VERDICT_W / 2, VERDICT_Y + 30, "", { font: "bold 22px Georgia", color: HEX_GRAY }).setOrigin(0.5).setDepth(25);
    this.verdictSubText = this.add.text(VERDICT_X + VERDICT_W / 2, VERDICT_Y + 58, "", { font: "bold 10px Georgia", color: HEX_GRAY }).setOrigin(0.5).setDepth(25);
    ...
    this.add.text(CONT_X, CONT_Y - 9, "boolean", { font: "bold 8px Courier New", color: HEX_CYAN }).setOrigin(0.5).setDepth(11);
    this.boolContText = this.add.text(CONT_X, CONT_Y + CONT_H / 2, "—", { font: "bold 13px Courier New", color: HEX_GRAY }).setOrigin(0.5).setDepth(11);
  }
  ...
  async materializeGem(ch) {
    ...
    const txt = this.add.text(0, 0, displayCh, { font: "bold 16px Courier New", color: "#0a1428" }).setOrigin(0.5);
    c.add([g, txt]);
    this.gemLayer.add(c);
    ...
    for (let i = 0; i < 4; i++) {
      const spark = this.add.text(PAD_CX + Phaser.Math.Between(-16, 16), PAD_CY + Phaser.Math.Between(-16, 16), "✦", { font: "8px Arial", color: HEX_SILVER }).setOrigin(0.5).setDepth(22).setAlpha(0);
  ...
  showCompileErrorStamp() {
    const stamp = this.add.text(PAD_CX, 150, "COMPILE ERROR", { font: "bold 16px Arial", color: HEX_RED }).setOrigin(0.5).setDepth(30).setScale(1.3).setAngle(-6).setAlpha(0);
  ...
  showRuntimeHaltStamp() {
    const stamp = this.add.text(PAD_CX, 150, "BUILD HALTED", { font: "bold 15px Arial", color: HEX_RED }).setOrigin(0.5).setDepth(30).setScale(1.2).setAngle(-4).setAlpha(0);
    this.roundElements.push(stamp);
    this.tweens.add({ targets: stamp, scale: 1, alpha: 1, duration: 150 });
    this.screenShake(0.004, 120);
    this.time.delayedCall(850, () => { if (stamp.active) this.tweens.add({ targets: stamp, alpha: 0, duration: 200, onComplete: () => stamp.destroy() }); });
  ...
  createCartridgeTray(config) {
    ...
    bdraw(false, false);
    const bt = this.add.text(0, 0, "CLASSIFY", { font: "bold 12px Arial", color: "#0a1208" }).setOrigin(0.5);
    btn.add([bg, bt]);
    btn.setSize(130, 44);
  ...
  async onArrangePressed(config) {
    ...
    for (const test of config.tests) {
      ...
      this.createFloatingText(470, 155, pass ? "✓" : "✗", pass ? HEX_GREEN_BRIGHT : HEX_RED, "bold 22px Arial", 700);
  ...
  gameOver() {
    ...
    (async () => {
      ...
      this._makeButton(640, 420, "RECALIBRATE THE LOUPE", 250, 50, { stroke: C_RED, textColor: HEX_RED }, () => this.scene.restart());
    })();
  }
  ...
  showScoreTally() {
    ...
    const badgeLbl = this.add.text(640, 505, "isDigit() SCHEMA ACQUIRED", { font: "bold 12px Georgia", color: HEX_GOLD }).setOrigin(0.5).setDepth(91).setAlpha(0);
