...
export class Level50Scene extends Phaser.Scene {
  ...
  createStacksBanner() {
    ...
    this.add.text(400, 83, "T H E   S T A C K S", { font: "bold 16px Georgia", color: HEX_BRASS }).setOrigin(0.5).setAlpha(0.7).setDepth(3);
  }
  ...
  createRunnersStation() {
    ...
    this.slipLabel = this.add.text(SLIP_X1 - 10, SLIP_Y0 + 8, "", { font: "bold 11px Courier New", color: "#8a6435" }).setOrigin(1, 0).setAlpha(0.7).setDepth(6);
    this.slipQuestionText = this.add.text(SLIP_CX, SLIP_Y1 - 16, "", { font: "14px Georgia", color: "#241a0e" }).setOrigin(0.5).setDepth(6);
  ...
  async stampSlip(kind) {
    ...
    const stamp = this.add.text(SLIP_CX, SLIP_CY, labels[kind], { font: "bold 23px Georgia", color: colors[kind] }).setOrigin(0.5).setDepth(30).setScale(1.4).setAngle(-8).setAlpha(0);
  ...
  createMiniConsole() {
    ...
    this.consoleText = this.add.text(CON_X + 10, CON_Y + 14, "", { font: "bold 14px Courier New", color: HEX_GREEN_BRIGHT }).setOrigin(0, 0.5).setDepth(6);
    this._consoleLines = [];
  }
  ...
  createReferenceBookshelf() {
    ...
    this.sizeCounterText = this.add.text(SHELF_X1 - 45, 106, "size: 0", { font: "bold 12px Courier New", color: HEX_BRASS }).setOrigin(1 / 2, 0.5).setDepth(6);
  ...
  createReferenceListStatePanel() {
    ...
    this.typeEchoText = this.add.text(PANEL_X + PANEL_W - 10, PANEL_Y + 42, "", { font: "bold 11px Courier New", color: HEX_GRAY }).setOrigin(1, 0.5).setAlpha(0.6).setDepth(12);
    this.bracketText = this.add.text(PANEL_X + PANEL_W / 2, PANEL_Y + 110, "[]", { font: "bold 14px Courier New", color: HEX_GRAY, wordWrap: { width: PANEL_W - 24 }, align: "center" }).setOrigin(0.5).setDepth(12);
    this.panelSizeText = this.add.text(PANEL_X + PANEL_W / 2, PANEL_Y + 170, "size: 0", { font: "12px Courier New", color: HEX_BRASS }).setOrigin(0.5).setAlpha(0.85).setDepth(12);
    this.panelIndexText = this.add.text(PANEL_X + PANEL_W / 2, PANEL_Y + 192, "", { font: "bold 11px Courier New", color: "#8a6435" }).setOrigin(0.5).setAlpha(0.7).setDepth(12);

    this.add.text(PANEL_X + 14, PANEL_Y + 265, "retrieved:", { font: "12px Georgia", color: "#8a6435" }).setOrigin(0, 0.5).setDepth(12);
    this.retrievedValueText = this.add.text(PANEL_X + 82, PANEL_Y + 265, "—", { font: "bold 14px Courier New", color: HEX_GRAY }).setOrigin(0, 0.5).setDepth(12);
  }
  ...
  createTraversalTracker() {
    ...
    this.add.text(TRK_X + 10, TRK_Y + 12, "TRAVERSAL", { font: "bold 12px Georgia", color: HEX_BRASS }).setOrigin(0, 0.5).setDepth(11);
    this.trackerContainer = this.add.container(0, 0).setDepth(12);
    this.trackerTotalText = this.add.text(TRK_X + 10, TRK_Y + TRK_H - 12, "", { font: "bold 11px Courier New", color: HEX_GOLD }).setOrigin(0, 0.5).setDepth(12);
    this._trackerRows = [];
    this.clearTracker();
  }
  ...
  appendTrackerRow(text, isCrash) {
    ...
    const t = this.add.text(TRK_X + 10, y, text, { font: "12px Courier New", color: isCrash ? HEX_RED : "#e8dfc8" }).setOrigin(0, 0.5).setAlpha(0);
  ...
  clearTracker() {
    ...
    this._trackerDash = this.add.text(TRK_X + TRK_W / 2, TRK_Y + 66, "—", { font: "bold 16px Courier New", color: "#3a2618" }).setOrigin(0.5).setDepth(11);
  }
  ...
  async crashRetrieval(index) {
    ...
    const stamp = this.add.text(SHELF_CX, SHELF_Y0 + 46, "IndexOutOfBoundsException", { font: "bold 12px Courier New", color: HEX_RED }).setOrigin(0.5).setAngle(-6).setAlpha(0).setDepth(20);
  ...
  flashLoopCounter(label) {
    const t = this.add.text(SHELF_CX, SHELF_Y0 + 68, label, { font: "bold 15px Courier New", color: HEX_GOLD }).setOrigin(0.5).setDepth(20).setAlpha(0);
  ...
  async onBubbleSelected(opt, config, bubbleContainer) {
    ...
    if (config.revealNote) this.createFloatingText(SLIP_CX + 50, 430, config.revealNote, HEX_GRAY, "12px Arial", 2800);
  ...
  setupBugHunt(config) {
    this.clearSlip();
    const header = this.add.text(SLIP_CX, SLIP_Y0 + 16, "CLICK THE BUG", { font: "bold 15px Georgia", color: "#c62828" }).setOrigin(0.5);
  ...
  async onTokenClicked(tokenObj, config, lineY) {
    ...
    if (correct) {
      ...
      const fixT = this.add.text(SLIP_CX, lineY - 16, config.fix, { font: "bold 13px Courier New", color: "#2e7d32" }).setOrigin(0.5).setAlpha(0);
  ...
  scoreForAttempt(timePctUsed) {
    ...
    if (remaining > 0.6) { points += 50; this.fastBonusCount++; this.createFloatingText(SLIP_CX, SLIP_Y0 - 20, "⚡ SWIFT PAGE +50", HEX_GOLD, "bold 15px Arial", 900); }
    else if (remaining > 0.3) { points += 25; this.fastBonusCount++; this.createFloatingText(SLIP_CX, SLIP_Y0 - 20, "⚡ +25", HEX_GOLD, "bold 14px Arial", 800); }
    return points;
  }
  ...
  gameOver() {
    ...
    (async () => {
      ...
      this.add.text(640, 350, `Requests Paged: ${this.currentRound} / 15`, { font: "18px Arial", color: HEX_GRAY }).setOrigin(0.5).setDepth(91);
      this._makeButton(525, 420, "LIGHT THE LANTERN", 200, 50, { stroke: C_RED, textColor: HEX_RED }, () => this.scene.restart());
