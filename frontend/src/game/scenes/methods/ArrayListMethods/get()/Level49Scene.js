...
export class Level49Scene extends Phaser.Scene {
  ...
  createConsultationBanner() {
    ...
    this.add.text(400, 83, "C O N S U L T A T I O N   D E S K", { font: "bold 15px Georgia", color: HEX_BRASS }).setOrigin(0.5).setAlpha(0.7).setDepth(3);
  }
  ...
  async populateShelf(initialList, listType) {
    ...
    for (let i = 0; i < initialList.length; i++) {
      ...
      const label = this.add.text(0, 26, `[${i}]`, { font: "bold 10px Courier New", color: HEX_BRASS }).setOrigin(0.5).setAlpha(0.6);
  ...
  async slideInRequestSlip(callText) {
    ...
    const t = this.add.text(0, 0, callText, { font: "bold 14px Courier New", color: "#241a0e", wordWrap: { width: 110 }, align: "center" }).setOrigin(0.5);
  ...
  createListStatePanel() {
    ...
    this.bracketText = this.add.text(PANEL_X + PANEL_W / 2, PANEL_Y + 100, "[]", { font: "bold 17px Courier New", color: HEX_GRAY, wordWrap: { width: PANEL_W - 30 }, align: "center" }).setOrigin(0.5).setDepth(12);
    this.panelSizeText = this.add.text(PANEL_X + PANEL_W / 2, PANEL_Y + 140, "size: 0", { font: "13px Courier New", color: HEX_BRASS }).setOrigin(0.5).setAlpha(0.85).setDepth(12);
    this.panelIndexText = this.add.text(PANEL_X + PANEL_W / 2, PANEL_Y + 163, "", { font: "bold 12px Courier New", color: "#8a6435" }).setOrigin(0.5).setAlpha(0.7).setDepth(12);

    // retrieved-value row
    this.add.text(PANEL_X + 16, PANEL_Y + 205, "retrieved:", { font: "13px Georgia", color: "#8a6435" }).setOrigin(0, 0.5).setDepth(12);
    this.retrievedValueText = this.add.text(PANEL_X + 92, PANEL_Y + 205, "—", { font: "bold 16px Courier New", color: HEX_GRAY }).setOrigin(0, 0.5).setDepth(12);
  }
  ...
  createHUD() {
    ...
    this.add.text(20, 14, "THE CONSULTATION DESK", { font: "bold 16px Georgia", color: "#b0bec5" }).setDepth(50);
    this.add.text(20, 32, "Accretion Phase — get()", { font: "13px Arial", color: "#546e7a" }).setDepth(50);
  ...
  async retrieveGhost(index) {
    ...
    const valText = this.add.text(CRADLE.x, CRADLE.y - 55, String(entry.value), { font: "bold 21px Courier New", color: this._typeColorHex(entry.type) }).setOrigin(0.5).setDepth(30).setAlpha(0);
  ...
  async plusFusion(v1, v2) {
    const a = this.add.text(CRADLE.x - 50, CRADLE.y - 65, String(v1), { font: "bold 19px Courier New", color: HEX_GOLD }).setOrigin(0.5).setDepth(30);
    const plus = this.add.text(CRADLE.x, CRADLE.y - 65, "+", { font: "bold 21px Courier New", color: HEX_GOLD }).setOrigin(0.5).setDepth(30).setAlpha(0);
    const b = this.add.text(CRADLE.x + 50, CRADLE.y - 65, String(v2), { font: "bold 19px Courier New", color: HEX_GOLD }).setOrigin(0.5).setDepth(30);
    ...
    const sum = v1 + v2;
    const sumT = this.add.text(CRADLE.x, CRADLE.y - 65, String(sum), { font: "bold 23px Courier New", color: HEX_GOLD }).setOrigin(0.5).setDepth(30).setScale(0.5);
  ...
  async printToConsole(text) {
    ...
    const t = this.add.text(DESK_X0 + 10, DESK_Y1 + 16, "> ", { font: "bold 16px Courier New", color: HEX_GREEN_BRIGHT }).setOrigin(0, 0.5).setDepth(12);
  ...
  async crashRetrieval(index) {
    ...
    if (this._activeSlip && this._activeSlip.container.active) {
      ...
      const stamp = this.add.text(0, 6, "IndexOutOfBounds\nException", { font: "bold 13px Courier New", color: HEX_RED, align: "center" }).setOrigin(0.5).setAngle(-8).setScale(1.5).setAlpha(0);
      slip.add(stamp);
  ...
  async showCompileErrorStamp() {
    const stamp = this.add.text(620, 96, "COMPILE ERROR", { font: "bold 23px Arial", color: HEX_RED }).setOrigin(0.5).setDepth(30).setScale(1.5).setAngle(-6).setAlpha(0);
  ...
  createCartridgeTray(config) {
    ...
    bdraw(false, false);
    const bt = this.add.text(0, 0, "CONSULT", { font: "bold 16px Arial", color: "#0a0704" }).setOrigin(0.5);
    btn.add([bg, bt]);
    btn.setSize(130, 44);
  ...
  async runBehavioralCheck() {
    ...
    try {
      ...
      console.warn("Level49Scene: /api/wellbeing/predict-struggle unreachable, skipping behavioral signal for this level:", e);
    }
  }
  ...
  gameOver() {
    ...
    (async () => {
      ...
      this._makeButton(525, 420, "RING THE BELL", 200, 50, { stroke: C_RED, textColor: HEX_RED }, () => this.scene.restart());
  ...
  levelComplete() {
    ...
    try { GameManager.completeLevel(49, Math.round((this.correctFirstTry / 12) * 100)); } catch (_) {}
  ...
  showScoreTally() {
    ...
    const badgeLbl = this.add.text(640, 505, "get() SCHEMA ACQUIRED", { font: "bold 15px Georgia", color: HEX_GOLD }).setOrigin(0.5).setDepth(91).setAlpha(0);
