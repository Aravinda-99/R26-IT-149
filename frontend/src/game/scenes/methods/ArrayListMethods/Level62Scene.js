...
export class Level62Scene extends Phaser.Scene {
  ...
  createCalibrationChart() {
    ...
    this.add.text(1140, 96, "2ⁿ", { font: "9px Courier New", color: HEX_BLUE_GRAY }).setDepth(3).setAlpha(0.25);
  }
  ...
  createTrialsBanner() {
    ...
    this.add.text(580, 25, "T H E   E X P O N E N T   T R I A L S", { font: "bold 13px Georgia", color: HEX_BRASS }).setOrigin(0.5).setAlpha(0.7).setDepth(3);
  }
  ...
  createTestCard() {
    ...
    this.add.text(CARD_CX, CARD_Y0 + 12, "EXPONENT TRIAL — TEST CARD", { font: "bold 8px Georgia", color: "#8a6435" }).setOrigin(0.5).setDepth(21);
    this.cardRoundLabel = this.add.text(CARD_X1 - 14, CARD_Y0 + 12, "TRIAL 1/15", { font: "bold 9px Courier New", color: "#8a6435" }).setOrigin(1, 0.5).setDepth(21);
  ...
  createCounterweight() {
    ...
    const massLabel = this.add.text(0, 2, "14 kg", { font: "bold 9px Georgia", color: HEX_BRASS }).setOrigin(0.5).setAlpha(0.6);
  ...
  createMiniCascade() {
    ...
    this.add.text(MCE_CX, MCE_ENTRY_Y + 41, "Math.pow", { font: "bold 9px Courier New", color: HEX_GOLD }).setOrigin(0.5).setDepth(6);
    ...
    for (let i = 0; i < MCE_STAGE_COUNT; i++) {
      ...
      const lbl = this.add.text(MCE_CX, y, "", { font: "bold 8px Courier New", color: HEX_GOLD }).setOrigin(0.5).setDepth(6).setAlpha(0);
  ...
  async mceInstanceCallShudder(value, type) {
    ...
    const q = this.add.text(star.container.x, star.container.y - 18, "?", { font: "bold 14px Georgia", color: HEX_RED }).setOrigin(0.5).setDepth(20).setAlpha(0);
  ...
  showCompileErrorStamp() {
    const stamp = this.add.text(CARD_CX, CARD_Y0 - 22, "COMPILE ERROR", { font: "bold 18px Arial", color: HEX_RED }).setOrigin(0.5).setDepth(30).setScale(1.4).setAngle(-6).setAlpha(0);
  ...
  createTrialSlate() {
    ...
    this.add.text(SLATE_X + 12, SLATE_Y + 10, "TRIAL SLATE", { font: "bold 9px Georgia", color: HEX_BRASS }).setDepth(11);
    this.slateLines = this.add.container(0, 0).setDepth(11);
    this._slateY = SLATE_Y + 32;
    this.add.text(SLATE_X + 12, SLATE_Y + SLATE_H - 18, "result (double):", { font: "10px Georgia", color: "#8a6435" }).setDepth(11);
    this.resultText = this.add.text(SLATE_X + 108, SLATE_Y + SLATE_H - 18, "—", { font: "bold 12px Courier New", color: HEX_GRAY }).setOrigin(0, 0.5).setDepth(11);
  }
  ...
  chalkEvaluationArrow(value, type) {
    ...
    const t = this.add.text(SLATE_X + 12, this._slateY, `→ ${display}`, { font: "bold 11px Courier New", color: type === "double" ? HEX_ORANGE : HEX_GOLD }).setAlpha(0);
    this.slateLines.add(t);
  ...
  createContainerShelf() {
    ...
    this.add.text(SHELF_X + 12, SHELF_Y + 8, "CONTAINERS", { font: "bold 9px Georgia", color: HEX_BRASS }).setDepth(11);
  ...
  updateContainerShelf(vars) {
    ...
    names.forEach((name) => {
      ...
      const meas = this.add.text(0, 0, label, { font: "bold 10px Courier New", color: hexColor });
      ...
      const txt = this.add.text(0, 0, label, { font: "bold 10px Courier New", color: hexColor }).setOrigin(0.5);
      c.add([bg, txt]);
  ...
  createHUD() {
    ...
    this.add.text(20, 14, "THE EXPONENT TRIALS", { font: "bold 15px Georgia", color: "#b0bec5" }).setDepth(50);
    this.add.text(20, 32, "Tuning Phase — Math Methods: pow()", { font: "11px Arial", color: "#546e7a" }).setDepth(50);
  ...
  scoreForAttempt(timePctUsed) {
    ...
    if (remaining > 0.6) { points += 50; this.fastBonusCount++; this.createFloatingText(CARD_CX, CARD_Y0 - 14, "⚡ EARLY BRAKE +50", HEX_GOLD, "bold 13px Arial", 900); }
  ...
  gameOver() {
    ...
    (async () => {
      ...
      this._makeButton(640, 420, "HOIST THE WEIGHT", 220, 50, { stroke: C_RED, textColor: HEX_RED }, () => this.scene.restart());
    })();
  }
  ...
  levelComplete() {
    ...
    try { GameManager.completeLevel(61, Math.round((this.correctFirstTry / 15) * 100)); } catch (_) {}
  ...
  showScoreTally() {
    ...
    const badgeLbl = this.add.text(640, 520, "pow() SCHEMA TUNED", { font: "bold 12px Georgia", color: HEX_GOLD }).setOrigin(0.5).setDepth(91).setAlpha(0);
