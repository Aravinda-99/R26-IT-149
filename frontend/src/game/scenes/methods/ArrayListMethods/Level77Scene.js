...
export class Level77Scene extends Phaser.Scene {
  ...
  createRoomInterior() {
    ...
    this.add.text(triTop.x, triTop.y - 10, "String", { font: "bold 8px Georgia", color: HEX_CREAM }).setOrigin(0.5).setAlpha(0.5).setDepth(2);
    this.add.text(triL.x - 14, triL.y + 8, "int", { font: "bold 8px Georgia", color: HEX_GOLD }).setOrigin(0.5).setAlpha(0.5).setDepth(2);
    this.add.text(triR.x + 18, triR.y + 8, "double", { font: "bold 8px Georgia", color: HEX_ORANGE }).setOrigin(0.5).setAlpha(0.5).setDepth(2);
    this.add.text((triTop.x + triL.x) / 2 - 18, (triTop.y + triL.y) / 2, "parseInt", { font: "6px Courier New", color: HEX_GOLD }).setOrigin(0.5).setAlpha(0.4).setDepth(2);
    this.add.text((triTop.x + triR.x) / 2 + 22, (triTop.y + triR.y) / 2, "parseDouble", { font: "6px Courier New", color: HEX_ORANGE }).setOrigin(0.5).setAlpha(0.4).setDepth(2);
    this.add.text(triTop.x, triL.y + 22, "valueOf ↑", { font: "bold 6px Courier New", color: HEX_CREAM }).setOrigin(0.5).setAlpha(0.45).setDepth(2);
    this._triangleGfx = g;
    ...
    this.add.text(FSIL_X, FSIL_Y + 58, "FURNACE", { font: "6px Georgia", color: HEX_COPPER }).setOrigin(0.5).setAlpha(0.3).setDepth(3);
    ...
    this.add.text(CSIL_X, CSIL_Y + 48, "CRUCIBLE", { font: "6px Georgia", color: HEX_ORANGE }).setOrigin(0.5).setAlpha(0.3).setDepth(3);
    ...
    this.add.text(640, 25, "T H E   I N S C R I P T I O N   P R E S S", { font: "bold 12px Georgia", color: HEX_CREAM }).setOrigin(0.5).setAlpha(0.7).setDepth(3);
  }
  ...
  createInscriptionPress() {
    ...
    this.add.text(PRESS_CX, TRAY_Y0 - 12, "INPUT", { font: "bold 9px Georgia", color: HEX_COPPER }).setOrigin(0.5).setDepth(11);
    ...
    this.armTypeface = this.add.text(0, 0, "", { font: "bold 10px Courier New", color: HEX_CREAM }).setOrigin(0.5).setAlpha(0.6).setScale(-1, 1);
    ...
    this.add.text(CONT_X, CONT_Y - 12, "String", { font: "bold 9px Courier New", color: HEX_CREAM }).setOrigin(0.5).setDepth(12);
    this.containerValueText = this.add.text(CONT_X, CONT_Y + 22, "—", { font: "bold 13px Courier New", color: HEX_GRAY }).setOrigin(0.5).setDepth(12);
  ...
  async materializeInputValue(value, type) {
    ...
    const txt = this.add.text(0, 0, label, { font: "bold 12px Courier New", color: type === "int" ? "#241a0e" : "#e8eaf6" }).setOrigin(0.5);
  ...
  async slideInBlankPaper() {
    ...
    const txt = this.add.text(0, 0, "", { font: "bold 15px Courier New", color: "#241a0e" }).setOrigin(0.5);
    strip.add([bg, txt]);
  ...
  showCompileErrorStamp() {
    const stamp = this.add.text(PRESS_CX, 150, "COMPILE ERROR", { font: "bold 16px Arial", color: HEX_RED }).setOrigin(0.5).setDepth(30).setScale(1.3).setAngle(-6).setAlpha(0);
  ...
  showNFEStamp() {
    const stamp = this.add.text(PRESS_CX, 150, "NumberFormatException", { font: "bold 13px Courier New", color: HEX_RED }).setOrigin(0.5).setDepth(30).setScale(1.2).setAlpha(0);
    this.roundElements.push(stamp);
    this.tweens.add({ targets: stamp, scale: 1, alpha: 1, duration: 150 });
    this.screenShake(0.004, 120);
    this.time.delayedCall(850, () => { if (stamp.active) this.tweens.add({ targets: stamp, alpha: 0, duration: 200, onComplete: () => stamp.destroy() }); });
  ...
  async showConcatAnimation(value, type, strRepr) {
    ...
    const et = this.add.text(0, 0, '""', { font: "bold 10px Courier New", color: "#241a0e" }).setOrigin(0.5);
    ...
    const plus = this.add.text(PRESS_CX, cy, "+", { font: "bold 16px Georgia", color: HEX_GOLD }).setOrigin(0.5).setAlpha(0);
  ...
  createScribesSlate() {
    ...
    this.add.text(SLATE_X + 14, SLATE_Y + 12, "SCRIBE'S SLATE", { font: "bold 10px Georgia", color: HEX_INDIGO }).setDepth(11);
    ...
    this.add.text(SLATE_X + SLATE_W - 90, SLATE_Y + 16, "String (wrapper class)", { font: "bold 7px Courier New", color: HEX_CREAM }).setOrigin(0.5).setDepth(12);

    this.add.text(SLATE_X + 14, SLATE_Y + 30, "number → text", { font: "italic 9px Georgia", color: HEX_CREAM }).setDepth(11);
  ...
  chalkEvaluationArrow(value) {
    const t = this.add.text(SLATE_X + 14, this._slateY, `→ ${value}`, { font: "bold 12px Courier New", color: HEX_CREAM }).setAlpha(0);
    this.slateLines.add(t);
    if (t.width > SLATE_W - 28) t.setFontSize(10);
  ...
  createExpressionMonitor() {
    ...
    this.exprMonitorText = this.add.text(470, 164, "", { font: "10px Courier New", color: HEX_GRAY }).setOrigin(0.5).setDepth(51);
  }
  ...
  createHUD() {
    ...
    this.add.text(20, 14, "THE INSCRIPTION PRESS", { font: "bold 14px Georgia", color: "#b0bec5" }).setDepth(50);
    this.add.text(20, 32, "Accretion Phase — Type Conversion: valueOf()", { font: "10px Arial", color: "#546e7a" }).setDepth(50);
  ...
  createBit() {
    ...
    ["S", "t", "r"].forEach((ch, i) => {
      const bt = this.add.text(-3 + i * 3, -6, ch, { font: "3px Courier New", color: HEX_CREAM }).setOrigin(0.5);
  ...
  async onBubbleSelected(opt, config, bubbleContainer) {
    ...
    if (!this._alive) return;
    if (config.revealNote) this.createFloatingText(470, 155, config.revealNote, HEX_GRAY, "11px Arial", 3000);
  ...
  createCartridgeTray(config) {
    ...
    bdraw(false, false);
    const bt = this.add.text(0, 0, "STAMP", { font: "bold 13px Arial", color: "#0a1208" }).setOrigin(0.5);
    btn.add([bg, bt]);
    btn.setSize(130, 44);
  ...
  async onArrangePressed(config) {
    ...
    this.createFloatingText(470, 155, pass ? "✓" : "✗", pass ? HEX_GREEN_BRIGHT : HEX_RED, "bold 22px Arial", 900);
  ...
  gameOver() {
    ...
    (async () => {
      ...
      this._makeButton(640, 420, "UNJAM THE PRESS", 220, 50, { stroke: C_RED, textColor: HEX_RED }, () => this.scene.restart());
    })();
  }
  ...
  showScoreTally() {
    ...
    const badgeLbl = this.add.text(640, 505, "valueOf() SCHEMA ACQUIRED", { font: "bold 12px Georgia", color: HEX_GOLD }).setOrigin(0.5).setDepth(91).setAlpha(0);
