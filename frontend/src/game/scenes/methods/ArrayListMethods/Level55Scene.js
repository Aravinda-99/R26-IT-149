...
export class Level55Scene extends Phaser.Scene {
  ...
  createObservatoryBanner() {
    ...
    this.add.text(400, 25, "T H E   O B S E R V A T O R Y", { font: "bold 13px Georgia", color: HEX_BRASS }).setOrigin(0.5).setAlpha(0.7).setDepth(3);
  }
  ...
  createGreatComparator() {
    ...
    this.add.text(PED_X + 30, PED_Y0 + 60, "MATH — EST. FOREVER", { font: "bold 7px Georgia", color: HEX_BRASS }).setOrigin(0.5).setAlpha(0.5).setAngle(-90).setDepth(5);
    ...
    this.add.text(BEAM_CX, BEAM_Y - 29, "Math", { font: "bold 16px Courier New", color: HEX_GOLD }).setOrigin(0.5).setDepth(6);
    ...
    this.modeBg = modeBg;
    this.modeText = this.add.text(BEAM_CX, BEAM_Y + 21, ".max", { font: "bold 12px Courier New", color: HEX_CYAN }).setOrigin(0.5).setDepth(6);
    ...
    [["a", CRADLE_A], ["b", CRADLE_B]].forEach(([key, pos]) => {
      ...
      this.add.text(pos.x, pos.y + 30, key, { font: "bold 11px Courier New", color: HEX_BLUE_GRAY }).setOrigin(0.5).setAlpha(0.7).setDepth(5);
    });
  ...
  _makeStar(value, type, x, y) {
    ...
    const txt = this.add.text(0, 0, display, { font: "bold 15px Courier New", color: "#060810" }).setOrigin(0.5);
  ...
  async deliverToVariable(name, value, type) {
    ...
    const valT = this.add.text(0, 0, display, { font: "bold 12px Courier New", color: this._typeColorHex(type) }).setOrigin(0.5);
    c.add([g, nameT, valT]);
  ...
  async instanceCallShudder(value, type) {
    ...
    const q = this.add.text(star.container.x, star.container.y - 34, "?", { font: "bold 20px Georgia", color: HEX_RED }).setOrigin(0.5).setDepth(20).setAlpha(0);
  ...
  createAstronomersSlate() {
    ...
    this.add.text(SLATE_X + 14, SLATE_Y + 16, "OBSERVATION SLATE", { font: "bold 10px Georgia", color: HEX_BRASS }).setDepth(11);
  ...
  chalkEvaluationArrow(value, type) {
    const display = type === "double" ? Number(value).toFixed(1) : String(value);
    const t = this.add.text(SLATE_X + 14, this._slateY, `→ ${display}`, { font: "bold 13px Courier New", color: this._typeColorHex(type) }).setAlpha(0);
    this.slateLines.add(t);
  ...
  createExpressionMonitor() {
    ...
    this.monitorText = this.add.text(W / 2, 32, "", { font: "12px Courier New", color: "#e8eaf6" }).setOrigin(0.5).setDepth(51);
  }
  ...
  createHUD() {
    ...
    this.add.text(20, 14, "THE OBSERVATORY", { font: "bold 15px Georgia", color: "#b0bec5" }).setDepth(50);
    this.add.text(20, 32, "Accretion Phase — Math Methods: max() & min()", { font: "11px Arial", color: "#546e7a" }).setDepth(50);
  ...
  showOptionBubbles(options, config) {
    ...
    shuffled.forEach((opt, i) => {
      ...
      const txt = this.add.text(0, 0, label, { font: "bold 12px Courier New", color: "#e8eaf6" }).setOrigin(0.5);
  ...
  async onBubbleSelected(opt, config, bubbleContainer) {
    ...
    if (config.revealNote) this.createFloatingText(SLATE_X + SLATE_W / 2, SLATE_Y + SLATE_H + 30, config.revealNote, HEX_GRAY, "11px Arial", 2800);
  ...
  createCartridgeTray(config) {
    ...
    bdraw(false, false);
    const bt = this.add.text(0, 0, "MEASURE", { font: "bold 14px Arial", color: "#060810" }).setOrigin(0.5);
    btn.add([bg, bt]);
    btn.setSize(130, 44);
  ...
  async onMeasurePressed(config) {
    ...
    for (let ti = 0; ti < tests.length; ti++) {
      ...
      this.createFloatingText(PLINTH.x, PLINTH.y - 60, pass ? "✓" : "✗", pass ? HEX_GREEN_BRIGHT : HEX_RED, "bold 22px Arial", 900);
  ...
  gameOver() {
    ...
    (async () => {
      ...
      this._makeButton(640, 420, "WAIT FOR CLEAR SKIES", 250, 50, { stroke: C_RED, textColor: HEX_RED }, () => this.scene.restart());
    })();
  }
  ...
  async observatoryFinale() {
    ...
    const spellWord = async (word) => {
      ...
      for (let i = 0; i < word.length; i++) {
        const s = this.add.text(startX + i * 34, 90, word[i], { font: "bold 22px Georgia", color: HEX_GOLD }).setOrigin(0.5).setDepth(5).setAlpha(0);
  ...
  showScoreTally() {
    ...
    const badgeLbl = this.add.text(640, 505, "max()/min() SCHEMA ACQUIRED", { font: "bold 12px Georgia", color: HEX_GOLD }).setOrigin(0.5).setDepth(91).setAlpha(0);
