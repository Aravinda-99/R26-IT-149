...
export class Level65Scene extends Phaser.Scene {
  ...
  createRoomInterior() {
    ...
    [200, 880].forEach((cx, i) => {
      ...
      if (i === 0) {
        ...
        [1, 3, 5, 9].forEach((v, vi) => {
          ...
          this.add.text(px, 88, String(v), { font: "7px Courier New", color: HEX_BLUE_GRAY }).setOrigin(0.5).setAlpha(0.35).setDepth(3);
        });
      } else {
        this.add.text(cx + 60, 84, "A B C ... Z", { font: "8px Courier New", color: HEX_BLUE_GRAY }).setOrigin(0.5).setAlpha(0.4).setDepth(3);
        this.add.text(cx + 60, 100, "a b c ...", { font: "8px Courier New", color: HEX_GREEN_MUTED }).setOrigin(0.5).setAlpha(0.4).setDepth(3);
        this.add.text(cx + 60, 116, "(upper before lower)", { font: "italic 6px Georgia", color: "#3a2618" }).setOrigin(0.5).setAlpha(0.5).setDepth(3);
      }
    });
    ...
    this.add.text(570, 25, "T H E   S O R T I N G   R O O M", { font: "bold 13px Georgia", color: HEX_BRASS }).setOrigin(0.5).setAlpha(0.7).setDepth(4);
  }
  ...
  createSpecimenTray() {
    ...
    this.add.text(TRAY_X0 + 10, TRAY_Y0 - 16, "SPECIMEN TRAY", { font: "bold 9px Georgia", color: HEX_BRASS }).setDepth(11).setAlpha(0.8);
    this.trayTypeBadge = this.add.text(TRAY_X1 - 10, TRAY_Y0 - 16, "int[]", { font: "bold 10px Courier New", color: HEX_BLUE_GRAY }).setOrigin(1, 0).setDepth(11);
  ...
  async populateTray(values, type) {
    ...
    if (n === 0) {
      const t = this.add.text(TRAY_CX, (TRAY_Y0 + TRAY_Y1) / 2, "(empty tray)", { font: "italic 12px Georgia", color: "#3a2618" }).setOrigin(0.5).setAlpha(0);
    ...
    for (let i = 0; i < n; i++) {
      ...
      const idxPlate = this.add.text(cellX + cellW / 2, TRAY_Y1 - 12, `[${i}]`, { font: "bold 10px Courier New", color: HEX_BRASS }).setOrigin(0.5).setAlpha(i === 0 ? 0.9 : 0.6);
    ...
    for (let i = 0; i < n; i++) {
      ...
      const txt = this.add.text(0, 0, display, { font: "bold 13px Courier New", color: "#0a1208" }).setOrigin(0.5);
  ...
  createArrangementEngine() {
    ...
    this.add.text(plateX + plateW / 2, plateY + 7, "Arrays", { font: "bold 11px Courier New", color: HEX_GOLD }).setOrigin(0.5).setDepth(12);
    this.add.text(plateX + plateW / 2, plateY + 16, ".sort", { font: "9px Courier New", color: HEX_CYAN }).setOrigin(0.5).setDepth(12);
    ...
    this.add.text(ax, ay + 14, "ASC", { font: "bold 7px Courier New", color: HEX_GREEN_BRIGHT }).setOrigin(0.5).setDepth(12).setAlpha(0.8);
  ...
  showVoidAnnotation() {
    const t = this.add.text(PLAQUE_X + PLAQUE_W / 2, PLAQUE_Y - 16, "void", { font: "bold 14px Courier New", color: HEX_GRAY }).setOrigin(0.5).setDepth(20).setAlpha(0);
  ...
  createDisplayPlaque() {
    ...
    this.plaqueText = this.add.text(PLAQUE_X + PLAQUE_W / 2, PLAQUE_Y + PLAQUE_H / 2, "", { font: "bold 15px Courier New", color: "#e8eaf6", wordWrap: { width: PLAQUE_W - 20 }, align: "center" }).setOrigin(0.5).setDepth(11);
  ...
  async runToStringScan(values, type) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      const comp = this._trayCompartments[i];
      if (comp && comp.card) {
        const ghost = this.add.text(comp.card.x, comp.card.y, String(values[i]), { font: "bold 12px Courier New", color: "#e8eaf6" }).setOrigin(0.5).setDepth(14);
  ...
  createBeforeAfterStrip() {
    ...
    this.stripBeforeText = this.add.text(STRIP_X + STRIP_W / 2 - 12, STRIP_Y + STRIP_H / 2, "", { font: "11px Courier New", color: HEX_GRAY }).setOrigin(1, 0.5).setDepth(11);
    this.stripArrowText = this.add.text(STRIP_X + STRIP_W / 2, STRIP_Y + STRIP_H / 2, "", { font: "bold 12px Arial", color: HEX_BRASS }).setOrigin(0.5).setDepth(11);
    this.stripAfterText = this.add.text(STRIP_X + STRIP_W / 2 + 12, STRIP_Y + STRIP_H / 2, "", { font: "bold 11px Courier New", color: "#e8eaf6" }).setOrigin(0, 0.5).setDepth(11);
  }
  ...
  async bracketAccessGhost(index) {
    ...
    const ghost = this.add.text(comp.card.x, comp.card.y, String(comp.value), { font: "bold 13px Courier New", color }).setOrigin(0.5).setDepth(14);
  ...
  showCompileErrorStamp() {
    const stamp = this.add.text(SLATE_X + SLATE_W / 2, SLATE_Y + SLATE_H / 2, "COMPILE ERROR", { font: "bold 20px Arial", color: HEX_RED }).setOrigin(0.5).setDepth(30).setScale(1.4).setAngle(-6).setAlpha(0);
  ...
  createCuratorsSlate() {
    ...
    this.add.text(SLATE_X + 14, SLATE_Y + 12, "CURATOR'S SLATE", { font: "bold 10px Georgia", color: HEX_BRASS }).setDepth(11);

    const pillG = this.add.graphics().setDepth(11);
    this.importPillGfx = pillG;
    this.importPillText = this.add.text(SLATE_X + SLATE_W - 12, SLATE_Y + 16, "import java.util.Arrays", { font: "bold 8px Courier New", color: HEX_GREEN_BRIGHT }).setOrigin(1, 0.5).setDepth(12);
    this._drawImportPill(true);

    this.slateLines = this.add.container(0, 0).setDepth(11);
    this._slateY = SLATE_Y + 42;

    this.add.text(SLATE_X + 14, SLATE_Y + SLATE_H - 34, "returns:", { font: "11px Georgia", color: "#8a6435" }).setDepth(11);
    this.resultText = this.add.text(SLATE_X + 70, SLATE_Y + SLATE_H - 34, "—", { font: "bold 13px Courier New", color: HEX_GRAY }).setOrigin(0, 0.5).setDepth(11);
  }
  ...
  async chalkWriteLine(text, color) {
    const t = this.add.text(SLATE_X + 14, this._slateY, "", { font: "bold 12px Courier New", color: color || "#e8eaf6" }).setDepth(11);
  ...
  chalkEvaluationArrow(value) {
    const t = this.add.text(SLATE_X + 14, this._slateY, `→ ${value}`, { font: "bold 12px Courier New", color: HEX_ORANGE }).setAlpha(0);
    this.slateLines.add(t);
    if (t.width > SLATE_W - 28) t.setFontSize(10);
  ...
  createExpressionMonitor() {
    ...
    this.exprMonitorText = this.add.text(640, 79, "", { font: "10px Courier New", color: HEX_GRAY }).setOrigin(0.5).setDepth(51);
  }
  ...
  createHUD() {
    ...
    this.add.text(20, 14, "THE SORTING ROOM", { font: "bold 14px Georgia", color: "#b0bec5" }).setDepth(50);
    this.add.text(20, 32, "Accretion Phase — Arrays Methods: sort()", { font: "10px Arial", color: "#546e7a" }).setDepth(50);
  ...
  createBit() {
    ...
    const paddleArrow = this.add.text(0, -5, "↑", { font: "bold 7px Arial", color: "#0a1208" }).setOrigin(0.5);
  ...
  showQuestionCard(promptText) {
    ...
    const badgeT = this.add.text(-230, 0, String(this.currentRound + 1), { font: "bold 13px Arial", color: "#0a1208" }).setOrigin(0.5);
    const t = this.add.text(-205, 0, promptText, { font: "14px Arial", color: "#e8eaf6", wordWrap: { width: 440 } }).setOrigin(0, 0.5);
  ...
  showOptionBubbles(options, config) {
    ...
    shuffled.forEach((opt, i) => {
      ...
      const txt = this.add.text(0, 0, label, { font: "bold 11px Courier New", color: "#e8eaf6", wordWrap: { width: w - 20 }, align: "center" }).setOrigin(0.5);
  ...
  async onBubbleSelected(opt, config, bubbleContainer) {
    ...
    if (!this._alive) return;
    if (config.revealNote) this.createFloatingText(TRAY_CX, 155, config.revealNote, HEX_GRAY, "11px Arial", 3000);
  ...
  createCartridgeTray(config) {
    ...
    bdraw(false, false);
    const bt = this.add.text(0, 0, "ARRANGE", { font: "bold 13px Arial", color: "#0a1208" }).setOrigin(0.5);
    btn.add([bg, bt]);
    btn.setSize(130, 44);
  ...
  async onArrangePressed(config) {
    ...
    this.createFloatingText(TRAY_CX, 155, pass ? "✓" : "✗", pass ? HEX_GREEN_BRIGHT : HEX_RED, "bold 22px Arial", 900);
  ...
  async crashIOOBE(idx) {
    const stamp = this.add.text(TRAY_CX, TRAY_Y0 - 30, "ArrayIndexOutOfBoundsException", { font: "bold 9px Courier New", color: HEX_RED }).setOrigin(0.5).setAngle(-4).setAlpha(0);
    this.roundElements.push(stamp);
  ...
  gameOver() {
    ...
    (async () => {
      ...
      this._makeButton(640, 420, "RESTART THE ENGINE", 230, 50, { stroke: C_RED, textColor: HEX_RED }, () => this.scene.restart());
    })();
  }
  ...
  showScoreTally() {
    ...
    const badgeLbl = this.add.text(640, 505, "sort() SCHEMA ACQUIRED", { font: "bold 12px Georgia", color: HEX_GOLD }).setOrigin(0.5).setDepth(91).setAlpha(0);
