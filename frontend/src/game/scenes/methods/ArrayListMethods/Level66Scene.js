...
export class Level66Scene extends Phaser.Scene {
  ...
  createHumidifier() {
    ...
    for (let i = 0; i < 3; i++) {
      const v = this.add.text(-8 + i * 8, -22, "~", { font: "10px Georgia", color: HEX_GREEN_MUTED_66 }).setOrigin(0.5).setAlpha(0.15);
      c.add(v);
  ...
  createPatinaReference() {
    ...
    this.add.text(x0 + w / 2, y + h + 8, "PATINA SCALE", { font: "bold 7px Georgia", color: HEX_BLUE_GRAY }).setOrigin(0.5).setAlpha(0.5).setDepth(3);
  ...
  createTrialsBanner() {
    ...
    this.add.text(590, 25, "T H E   C L A S S I F I C A T I O N   T R I A L S", { font: "bold 11px Georgia", color: HEX_BRASS }).setOrigin(0.5).setAlpha(0.7).setDepth(3);
  }
  ...
  createAnnotation(x, y, text, colorHex) {
    const t = this.add.text(x, y, text, { font: "italic 10px Arial", color: colorHex, wordWrap: { width: 300 }, align: "center" }).setOrigin(0.5).setDepth(70).setAlpha(0);
  ...
  createClassificationCard() {
    ...
    this.add.text(CARD_CX, CARD_Y0 + 11, "CLASSIFICATION FORM — SPECIMEN SERIES", { font: "bold 8px Georgia", color: "#8a6435" }).setOrigin(0.5).setDepth(21);
    this.cardRoundLabel = this.add.text(CARD_X1 - 12, CARD_Y0 + 11, "CLASS. 1/15", { font: "bold 9px Courier New", color: "#8a6435" }).setOrigin(1, 0.5).setDepth(21);
    this.cardContentContainer = this.add.container(0, 0).setDepth(21);
    this.cardQuestionText = this.add.text(CARD_CX, CARD_Y1 - 24, "", { font: "bold 12px Georgia", color: "#241a0e", wordWrap: { width: CARD_X1 - CARD_X0 - 36 }, align: "center" }).setOrigin(0.5).setDepth(21);
  ...
  createSpecimenTray() {
    ...
    this.trayFrameGfx = g;
    this.add.text(TRAY_X0 + 10, TRAY_Y0 - 14, "SPECIMEN TRAY", { font: "bold 8px Georgia", color: HEX_BRASS }).setDepth(11).setAlpha(0.7);
  }
  ...
  createMiniTrayAndEngine() {
    ...
    this.miniTrayTypeBadge = this.add.text(MINI_X1 - 8, MINI_TRAY_Y0 - 12, "int[]", { font: "bold 8px Courier New", color: HEX_BLUE_GRAY }).setOrigin(1, 0).setDepth(11);
    ...
    this.add.text(plateX + plateW / 2, plateY + 5, "Arrays.sort", { font: "bold 8px Courier New", color: HEX_GOLD }).setOrigin(0.5).setDepth(12);
  ...
  async populateMiniTray(values, type) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      const idxPlate = this.add.text(cellX + cellW / 2, MINI_TRAY_Y1 - 8, `[${i}]`, { font: "bold 7px Courier New", color: HEX_BRASS }).setOrigin(0.5).setAlpha(i === 0 ? 0.9 : 0.6);
  ...
  showVoidAnnotation() {
    const t = this.add.text(MINI_PLAQUE_X + MINI_PLAQUE_W / 2, MINI_PLAQUE_Y - 12, "void", { font: "bold 11px Courier New", color: HEX_GRAY }).setOrigin(0.5).setDepth(20).setAlpha(0);
  ...
  createMiniPlaque() {
    ...
    this.plaqueText = this.add.text(MINI_PLAQUE_X + MINI_PLAQUE_W / 2, MINI_PLAQUE_Y + MINI_PLAQUE_H / 2, "", { font: "bold 13px Courier New", color: "#e8eaf6", wordWrap: { width: MINI_PLAQUE_W - 16 }, align: "center" }).setOrigin(0.5).setDepth(11);

    this.stripBeforeText = this.add.text(MINI_X0, MINI_STRIP_Y, "", { font: "9px Courier New", color: HEX_GRAY }).setOrigin(0, 0.5).setDepth(11);
    this.stripArrowText = this.add.text(MINI_CX, MINI_STRIP_Y, "", { font: "bold 10px Arial", color: HEX_BRASS }).setOrigin(0.5).setDepth(11);
    this.stripAfterText = this.add.text(MINI_X1, MINI_STRIP_Y, "", { font: "bold 9px Courier New", color: "#e8eaf6" }).setOrigin(1, 0.5).setDepth(11);
  }
  ...
  async runToStringScan(values, type) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      const comp = this._miniCompartments[i];
      if (comp && comp.card) {
        const ghost = this.add.text(comp.card.x, comp.card.y, String(values[i]), { font: "bold 9px Courier New", color: "#e8eaf6" }).setOrigin(0.5).setDepth(14);
  ...
  async bracketAccessGhost(index) {
    ...
    const ghost = this.add.text(comp.card.x, comp.card.y, String(comp.value), { font: "bold 10px Courier New", color }).setOrigin(0.5).setDepth(14);
  ...
  showCompileErrorStamp() {
    const stamp = this.add.text(SLATE_X + SLATE_W / 2, SLATE_Y + SLATE_H / 2, "COMPILE ERROR", { font: "bold 16px Arial", color: HEX_RED }).setOrigin(0.5).setDepth(30).setScale(1.3).setAngle(-6).setAlpha(0);
  ...
  createClassificationSlate() {
    ...
    this.add.text(SLATE_X + 12, SLATE_Y + 10, "CLASSIFICATION SLATE", { font: "bold 9px Georgia", color: HEX_BRASS }).setDepth(11);
    this.slateLines = this.add.container(0, 0).setDepth(11);
    this._slateY = SLATE_Y + 30;
  }

  async chalkWriteLine(text, color) {
    const t = this.add.text(SLATE_X + 12, this._slateY, "", { font: "bold 11px Courier New", color: color || "#e8eaf6" }).setDepth(11);
  ...
  chalkEvaluationArrow(value) {
    const t = this.add.text(SLATE_X + 12, this._slateY, `→ ${value}`, { font: "bold 11px Courier New", color: HEX_ORANGE }).setAlpha(0);
    this.slateLines.add(t);
    if (t.width > SLATE_W - 24) t.setFontSize(9);
  ...
  createHUD() {
    ...
    this.add.text(20, 14, "THE CLASSIFICATION TRIALS", { font: "bold 14px Georgia", color: "#b0bec5" }).setDepth(50);
    this.add.text(20, 32, "Tuning Phase — Arrays Methods: sort()", { font: "10px Arial", color: "#546e7a" }).setDepth(50);
  ...
  createBit() {
    ...
    const check = this.add.text(0, -5, "✓", { font: "bold 7px Arial", color: "#0a1208" }).setOrigin(0.5);
  ...
  async crashIOOBE(idx) {
    const stamp = this.add.text(MINI_CX, MINI_TRAY_Y0 - 20, "ArrayIndexOutOfBoundsException", { font: "bold 9px Courier New", color: HEX_RED }).setOrigin(0.5).setAngle(-4).setAlpha(0);
    this.roundElements.push(stamp);
  ...
  scoreForAttempt(timePctUsed) {
    ...
    if (remaining > 0.6) { points += 50; this.fastBonusCount++; this.createFloatingText(CARD_CX, CARD_Y0 - 14, "⚡ EARLY VERDICT +50", HEX_GOLD, "bold 13px Arial", 900); }
  ...
  gameOver() {
    ...
    (async () => {
      ...
      this.add.text(640, 350, `Specimens Classified: ${this.currentRound} / 15`, { font: "16px Arial", color: HEX_GRAY }).setOrigin(0.5).setDepth(91);
      this._makeButton(640, 420, "WIPE THE TARNISH", 220, 50, { stroke: C_RED, textColor: HEX_RED }, () => this.scene.restart());
    })();
  }
  ...
  showScoreTally() {
    ...
    const badgeLbl = this.add.text(640, 520, "sort() SCHEMA TUNED", { font: "bold 12px Georgia", color: HEX_GOLD }).setOrigin(0.5).setDepth(91).setAlpha(0);
