...
export class Level72Scene extends Phaser.Scene {
  ...
  createTrialsBanner() {
    ...
    this.add.text(640, 25, "T H E   S M E L T I N G   T R I A L S", { font: "bold 11px Georgia", color: HEX_COPPER }).setOrigin(0.5).setAlpha(0.7).setDepth(3);
  }
  ...
  createPressureGauge() {
    ...
    this.add.text(GAUGE_CX, GAUGE_CY + GAUGE_R + 16, "PRESSURE", { font: "bold 9px Georgia", color: HEX_COPPER }).setOrigin(0.5).setAlpha(0.75).setDepth(11);
  ...
  createMiniFurnace() {
    ...
    this.add.text(M_FURNACE_CX, M_HOPPER_Y0 - 10, "STRING IN", { font: "bold 8px Georgia", color: HEX_COPPER }).setOrigin(0.5).setDepth(11);
    ...
    this.add.text(M_CONT_X, M_CONT_Y - 10, "int", { font: "bold 8px Courier New", color: HEX_GOLD }).setOrigin(0.5).setDepth(12);
    this.containerValueText = this.add.text(M_CONT_X, M_CONT_Y + 20, "—", { font: "bold 13px Courier New", color: HEX_GRAY }).setOrigin(0.5).setDepth(12);
    this.containerNameText = this.add.text(M_CONT_X, M_CONT_Y + 36, "", { font: "italic 7px Courier New", color: HEX_GRAY }).setOrigin(0.5).setDepth(12);
  ...
  async materializePaperStrip(value) {
    ...
    const txt = this.add.text(0, 0, value, { font: "bold 11px Courier New", color: "#241a0e" }).setOrigin(0.5);
  ...
  async formIntegerBar(value) {
    ...
    const txt = this.add.text(0, 0, String(value), { font: "bold 12px Courier New", color: "#241a0e" }).setOrigin(0.5);
  ...
  async showNFE(inputStr) {
    ...
    const banner = this.add.text(M_FURNACE_CX, (M_CHAMBER_Y0 + M_CHAMBER_Y1) / 2, "NumberFormatException", { font: "bold 10px Courier New", color: HEX_RED }).setOrigin(0.5).setDepth(30).setScale(1.2).setAlpha(0).setWordWrapWidth(120);
  ...
  showCompileErrorStamp() {
    const stamp = this.add.text(M_FURNACE_CX, 90, "COMPILE ERROR", { font: "bold 14px Arial", color: HEX_RED }).setOrigin(0.5).setDepth(30).setScale(1.3).setAngle(-6).setAlpha(0);
  ...
  createMiniSlate() {
    ...
    this.add.text(SLATE_X + 10, SLATE_Y + 8, "MINI SLATE", { font: "bold 8px Georgia", color: HEX_COPPER }).setDepth(11);
  ...
  createVariablesPanel() {
    ...
    this.add.text(VARS_X + 10, VARS_Y + 6, "VARIABLES", { font: "bold 8px Georgia", color: HEX_COPPER }).setDepth(11);
  ...
  updateVariablesPanel(vars) {
    ...
    for (const name in vars) {
      ...
      const t = this.add.text(VARS_X + 10, y, text, { font: "bold 8px Courier New", color: v.type === "String" ? HEX_CYAN : HEX_GOLD }).setOrigin(0, 0.5);
  ...
  createHUD() {
    ...
    this.add.text(20, 14, "THE SMELTING TRIALS", { font: "bold 14px Georgia", color: "#b0bec5" }).setDepth(50);
    this.add.text(20, 32, "Tuning Phase — Type Conversion: Integer.parseInt()", { font: "10px Arial", color: "#546e7a" }).setDepth(50);
  ...
  scoreForAttempt(timePctUsed) {
    ...
    if (remaining > 0.6) { points += 50; this.fastBonusCount++; this.createFloatingText(CARD_CX, CARD_Y0 - 14, "⚡ EARLY LOCK +50", HEX_GOLD, "bold 13px Arial", 900); }
  ...
  gameOver() {
    ...
    (async () => {
      ...
      this._makeButton(640, 420, "RESTOKE THE FURNACE", 240, 50, { stroke: C_RED, textColor: HEX_RED }, () => this.scene.restart());
    })();
  }
  ...
  showScoreTally() {
    ...
    const badgeLbl = this.add.text(640, 520, "parseInt() SCHEMA TUNED", { font: "bold 13px Georgia", color: HEX_GOLD }).setOrigin(0.5).setDepth(91).setAlpha(0);
