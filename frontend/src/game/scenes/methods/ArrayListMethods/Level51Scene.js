...
export class Level51Scene extends Phaser.Scene {
  ...
  createRestorationInterior() {
    ...
    this.add.text(400, 26, "THE RESTORATION ROOM", { font: "bold 13px Georgia", color: HEX_BRASS }).setOrigin(0.5).setAlpha(0.7).setDepth(3);
  }
  ...
  createMastersPlaque() {
    ...
    this.plaqueText = this.add.text(890, 60, "AT THE BENCH", { font: "bold 10px Georgia", color: HEX_BRASS }).setOrigin(0.5).setAlpha(0.4).setDepth(5);
  ...
  createCodeCanvas() {
    ...
    this.tabFilename = this.add.text(CX + CW - 12, CY + TAB_H / 2, "Restore1.java", { font: "11px Courier New", color: "#546e7a" }).setOrigin(1, 0.5).setDepth(11);
  ...
  createBlockPalette() {
    ...
    this.add.text(PX + 10, PY + 8, "MASTER'S PARTS", { font: "bold 9px Arial", color: "#3d4450" }).setDepth(11);
  ...
  createRigWindow() {
    ...
    this.add.text(OX + 10, OY + 6, "RESTORATION RIG — LIVE", { font: "bold 9px Georgia", color: HEX_BRASS }).setAlpha(0.7).setDepth(11);
  ...
  createMiniBookshelf() {
    ...
    this.typeStampText = this.add.text(MS_CX, OY + 46, "", { font: "bold 7px Courier New", color: HEX_CYAN }).setOrigin(0.5);
    this.sizeCounterText = this.add.text(MS_CX, MS_BOT + 10, "size: 0", { font: "bold 7px Courier New", color: HEX_BRASS }).setOrigin(0.5);
  ...
  _makeMiniBook(entry, x, y) {
    ...
    const txt = this.add.text(0, 0, label, { font: "bold 7px Georgia", color: "#0a0704" }).setOrigin(0.5);
  ...
  createMiniListStatePanel() {
    ...
    this.bracketText = this.add.text(LP_CX, OY + 88, "[]", { font: "bold 9px Courier New", color: HEX_GRAY, wordWrap: { width: 132 }, align: "center" }).setOrigin(0.5);
    this.panelSizeText = this.add.text(LP_CX, OY + 132, "size: 0", { font: "8px Courier New", color: HEX_BRASS }).setOrigin(0.5).setAlpha(0.85);
    this.panelIndexText = this.add.text(LP_CX, OY + 146, "", { font: "bold 7px Courier New", color: "#8a6435", wordWrap: { width: 132 }, align: "center" }).setOrigin(0.5).setAlpha(0.7);
    const rl = this.add.text(LP_X, OY + 166, "retrieved:", { font: "7px Georgia", color: "#8a6435" });
    this.retrievedValueText = this.add.text(LP_X + 48, OY + 170, "—", { font: "bold 9px Courier New", color: HEX_GRAY }).setOrigin(0, 0.5);
  ...
  createMiniTraversalTracker() {
    const hdr = this.add.text(TRK_X, OY + 40, "TRAVERSAL", { font: "bold 7px Georgia", color: HEX_BRASS }).setAlpha(0.7);
    this.trackerContainer = this.add.container(0, 0);
    this.trackerTotalText = this.add.text(TRK_X, OY + 196, "", { font: "bold 8px Courier New", color: HEX_GOLD });
  ...
  createMiniCrossWingCameos() {
    ...
    this.pressText = this.add.text(TRK_X + 60, OY + 217, "", { font: "bold 7px Courier New", color: HEX_ORANGE }).setOrigin(0.5);
  ...
  miniDispenseTo(name, value, javaType) {
    ...
    if (!this.containerObjs[name]) {
      ...
      const t = this.add.text(LP_X + 6, y + 8, "", { font: "bold 7px Courier New", color: HEX_CYAN }).setOrigin(0, 0.5);
  ...
  async crashRetrieval(index) {
    ...
    const stamp = this.add.text(OX + OW / 2, OY + 120, "IndexOutOfBoundsException", { font: "bold 10px Courier New", color: HEX_RED }).setOrigin(0.5).setAngle(-6).setAlpha(0).setDepth(25);
  ...
  buildReportRows(mission) {
    ...
    mission.tests.forEach((test, i) => {
      ...
      const expT = this.add.text(160, 0, (test.expectedOutput || "").slice(0, 22), { font: "9px Courier New", color: "#78909c" }).setOrigin(0, 0.5);
      const actualT = this.add.text(310, 0, "", { font: "9px Courier New", color: HEX_RED }).setOrigin(0, 0.5);
      const statusT = this.add.text(RW - 26, 0, "…", { font: "13px Arial", color: "#78909c" }).setOrigin(0.5);
  ...
  createHUD() {
    ...
    this.add.text(20, 14, "THE RESTORATION ROOM", { font: "bold 13px Georgia", color: "#b0bec5" }).setDepth(51);
    this.add.text(20, 32, "Restructuring Phase — ArrayList Methods: get()", { font: "10px Arial", color: "#546e7a" }).setDepth(51);
  ...
  gameOver() {
    ...
    this._makeButton(640, 420, "BACK TO THE BENCH", 230, 50, { stroke: C_RED, textColor: HEX_RED }, () => this.scene.restart());
  }
  ...
  showScoreTally() {
    ...
    lines.forEach((s, i) => {
      const t = this.add.text(470, 178 + i * 23, s, { font: "13px Arial", color: HEX_GRAY }).setOrigin(0, 0.5).setDepth(91).setAlpha(0);
      this.tweens.add({ targets: t, alpha: 1, duration: 250, delay: 300 + i * 120 });
    });
    const totalText = this.add.text(470, 178 + 6 * 23, "TOTAL: 0", { font: "bold 20px Arial", color: HEX_GOLD }).setOrigin(0, 0.5).setDepth(91).setAlpha(0);
    ...
    const badgeLbl = this.add.text(640, 478, "get() MASTERY", { font: "bold 13px Georgia", color: HEX_GOLD }).setOrigin(0.5).setDepth(91).setAlpha(0);
    const badgeSub = this.add.text(640, 494, "Accretion ✓  Tuning ✓  Restructuring ✓", { font: "10px Arial", color: "#78909c" }).setOrigin(0.5).setDepth(91).setAlpha(0);
    this.tweens.add({ targets: [badgeLbl, badgeSub], alpha: 1, duration: 300, delay: 2150 });

    const wingLbl = this.add.text(640, 512, "ARRAYLIST WING — 2 of 3 trilogies complete", { font: "bold 11px Georgia", color: "#8c7ae6" }).setOrigin(0.5).setDepth(91).setAlpha(0);
    ...
    ["add ✓", "get ✓", "remove"].forEach((s, i) => {
      const t = this.add.text(513 + i * 127, 545, s, { font: "bold 9px Arial", color: i < 2 ? "#8c7ae6" : "#546e7a" }).setOrigin(0.5).setDepth(91).setAlpha(0);
