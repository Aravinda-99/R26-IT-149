...
export class Level70Scene extends Phaser.Scene {
  ...
  createBureauInterior() {
    ...
    for (let r = 0; r < 5; r++) {
      for (let c = 0; c < 2; c++) {
        ...
        const t = this.add.text(lx, ly, "· · · · · · · · · ·", { font: "8px Georgia", color: "#e8eaf6" }).setAlpha(0.12).setDepth(2);
    ...
    this.add.text(640, 25, "T H E   C U R A T O R ' S   B U R E A U", { font: "bold 13px Georgia", color: HEX_GOLD }).setOrigin(0.5).setAlpha(0.8).setDepth(3);
  }
  ...
  createWingCrest() {
    ...
    g.closePath();
    g.strokePath();
    const plaque = this.add.text(0, -8, "▭", { font: "7px Arial", color: HEX_BLUE_GRAY }).setOrigin(0.5).setAlpha(0.7);
    const sort = this.add.text(0, 0, "⚙", { font: "7px Arial", color: HEX_CYAN }).setOrigin(0.5).setAlpha(0.7);
    const dual = this.add.text(0, 9, "⧉", { font: "7px Arial", color: HEX_GOLD }).setOrigin(0.5).setAlpha(0.7);
  ...
  createBlockPalette() {
    ...
    this.add.text(PX + 10, PY + 8, "CURATOR'S PARTS", { font: "bold 9px Arial", color: "#3d4450" }).setDepth(11);
  ...
  createRigWindow() {
    ...
    this.add.text(OX + 10, OY + 6, "BUREAU RIG — LIVE", { font: "bold 9px Georgia", color: HEX_BRASS }).setAlpha(0.7).setDepth(11);
  ...
  createMiniReplicationFrame() {
    ...
    this.varLabels = {};
    this.varLabels.orig = this.add.text(MINI_ORIG_X1 - 3, MINI_TRAY_Y0 - 10, "", { font: "italic 7px Courier New", color: HEX_CYAN }).setOrigin(1, 0);
    this.varLabels.orig2 = this.add.text(MINI_ORIG_X1 - 3, MINI_TRAY_Y0 - 19, "", { font: "italic 7px Courier New", color: HEX_GREEN_BRIGHT }).setOrigin(1, 0);
    this.varLabels.copy1 = this.add.text(MINI_COPY1_X1 - 3, MINI_TRAY_Y0 - 10, "", { font: "italic 7px Courier New", color: HEX_CYAN }).setOrigin(1, 0).setAlpha(0);
    this.varLabels.copy2 = this.add.text(MINI_COPY2_X1 - 3, MINI_TRAY2_Y0 - 10, "", { font: "italic 7px Courier New", color: HEX_CYAN }).setOrigin(1, 0).setAlpha(0);
    ...
    this.bridgeArrow = this.add.text((MINI_ORIG_X1 + MINI_COPY1_X0) / 2, MINI_BRIDGE_CY, "→", { font: "bold 10px Georgia", color: HEX_BRASS }).setOrigin(0.5).setAlpha(0.6);
  ...
  async _populateTrayGeneric(trayKey, values, type) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      const idxPlate = this.add.text(cellX + cellW / 2, bounds.y1 - 5, `${i}`, { font: "bold 5px Courier New", color: HEX_BRASS }).setOrigin(0.5).setAlpha(0.5);
      comp.add(idxPlate);
  ...
  async _buildCard(layer, comp, value, type, isGhostDefault) {
    ...
    const txt = this.add.text(0, 0, display, isGhostDefault
      ? { font: "italic 6px Courier New", color: "#78909c" }
      : { font: "bold 7px Courier New", color: "#0a1208" }).setOrigin(0.5);
  ...
  async flyGhostCopies(sourceTrayKey, values, copyLength, type, destTrayKey) {
    ...
    for (let i = 0; i < crossCount; i++) {
      ...
      if (!fast) {
        const ghost = this.add.text(srcComp.x + srcComp.w / 2, (srcComp.bounds.y0 + srcComp.bounds.y1) / 2 - 2, this._fmtVal(values[i]), { font: "bold 6px Courier New", color: HEX_CYAN }).setOrigin(0.5).setDepth(18).setAlpha(0.9);
  ...
  createMiniContainersWithRefs() {
    const hdr = this.add.text(CONT_X, CONT_Y0 - 12, "REFERENCES", { font: "bold 7px Georgia", color: HEX_BRASS }).setAlpha(0.7);
  ...
  createHUD() {
    ...
    this.add.text(20, 14, "THE CURATOR'S BUREAU", { font: "bold 13px Georgia", color: "#b0bec5" }).setDepth(51);
    this.add.text(20, 32, "Restructuring Phase — Arrays Methods: copyOf()", { font: "10px Arial", color: "#546e7a" }).setDepth(51);
  ...
  showCompileErrorStamp() {
    const stamp = this.add.text(OX + OW / 2, OY + 100, "COMPILE ERROR", { font: "bold 13px Arial", color: HEX_RED }).setOrigin(0.5).setDepth(30).setScale(1.3).setAngle(-6).setAlpha(0);
  ...
  async crashIOOBE(idx, trayKey) {
    ...
    const stamp = this.add.text(cx, bounds.y0 - 8, "IOOBE", { font: "bold 7px Courier New", color: HEX_RED }).setOrigin(0.5).setAngle(-3).setAlpha(0);
    this.missionElements.push(stamp);
    this.rigLayer.add(stamp);
  ...
  gameOver() {
    ...
    (async () => {
      ...
      this.add.text(640, 350, `Missions Catalogued: ${this.currentMission} / 6`, { font: "16px Arial", color: HEX_GRAY }).setOrigin(0.5).setDepth(91);
  ...
  async ceremonyPhase3_CentralPanel() {
    ...
    lines.forEach((s, i) => {
      const t = this.add.text(380, 150 + i * 22, s, { font: "13px Arial", color: HEX_GRAY }).setOrigin(0, 0.5).setDepth(91).setAlpha(0);
      this.tweens.add({ targets: t, alpha: 1, duration: 250, delay: 300 + i * 120 });
      this._ceremonyElements.push(t);
    });
    const totalText = this.add.text(380, 150 + 8 * 22, "TOTAL: 0", { font: "bold 20px Arial", color: HEX_GOLD }).setOrigin(0, 0.5).setDepth(91).setAlpha(0);
    ...
    const dual = this.add.text(-14, -6, "⧉", { font: "bold 12px Arial", color: HEX_CYAN }).setOrigin(0.5);
    const sand = this.add.text(0, -6, "⏳", { font: "bold 12px Arial", color: HEX_GOLD }).setOrigin(0.5);
    const keys = this.add.text(14, -6, "🔑", { font: "bold 11px Arial", color: HEX_BRASS }).setOrigin(0.5);
    badge.add([bg, dual, sand, keys]);
    this.tweens.add({ targets: badge, alpha: 1, duration: 300, delay: 2500 });
    const badgeLbl = this.add.text(640, 532, "copyOf() MASTERY", { font: "bold 13px Georgia", color: HEX_GOLD }).setOrigin(0.5).setDepth(91).setAlpha(0);
    const badgeSub = this.add.text(640, 548, "Accretion ✓  Tuning ✓  Restructuring ✓", { font: "10px Arial", color: "#78909c" }).setOrigin(0.5).setDepth(91).setAlpha(0);
  ...
  async ceremonyPhase4_WingSeal() {
    ...
    [-220, 220].forEach((sx) => {
      const s = this.add.text(sx, 0, "◆", { font: "12px Arial", color: HEX_GOLD }).setOrigin(0.5).setAlpha(0.7);
      banner.add(s);
    });
    const title = this.add.text(0, -18, "ARRAYS WING — COMPLETE", { font: "bold 17px Georgia", color: HEX_GOLD }).setOrigin(0.5);
    const caption = this.add.text(0, 30, "7 levels · 3 methods · one museum of order", { font: "italic 11px Georgia", color: HEX_BLUE_GRAY }).setOrigin(0.5).setAlpha(0.85);
    ...
    const colTexts = cols.map((c) => {
      const t = this.add.text(c.x, 4, c.label, { font: "bold 13px Georgia", color: HEX_GREEN_BRIGHT }).setOrigin(0.5).setAlpha(0);
      banner.add(t);
      return t;
    });
