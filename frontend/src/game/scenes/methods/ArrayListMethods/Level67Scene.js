...
export class Level67Scene extends Phaser.Scene {
  ...
  createWorkshopInterior() {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      const t = this.add.text(90, 370 + i * 55, ["÷", "═", "✓"][i], { font: "16px Georgia", color: HEX_BRASS }).setOrigin(0.5).setAlpha(0.35).setDepth(3);
    ...
    this.add.text(600, 25, "T H E   A R R A N G E M E N T   W O R K S H O P", { font: "bold 11px Georgia", color: HEX_BRASS }).setOrigin(0.5).setAlpha(0.7).setDepth(3);
  }
  ...
  createQualityStamp() {
    ...
    const t = this.add.text(0, 0, "Q.A.", { font: "bold 10px Georgia", color: "#5d7a5d" }).setOrigin(0.5);
    c.add([g, t]);
    c.setAlpha(0.4);
  ...
  createCodeCanvas() {
    ...
    this.tabFilename = this.add.text(CX + CW - 12, CY + TAB_H / 2, "Arrange1.java", { font: "11px Courier New", color: "#546e7a" }).setOrigin(1, 0.5).setDepth(11);
  ...
  createBlockPalette() {
    ...
    this.add.text(PX + 10, PY + 8, "ARRANGER'S TOOLS", { font: "bold 9px Arial", color: "#3d4450" }).setDepth(11);
  ...
  createRigWindow() {
    ...
    this.add.text(OX + 10, OY + 6, "WORKSHOP RIG — LIVE", { font: "bold 9px Georgia", color: HEX_BRASS }).setAlpha(0.7).setDepth(11);
  ...
  createMiniTrayAndEngine() {
    ...
    this.rigLayer.add(g);
    this.miniTrayTypeBadge = this.add.text(MINI_X1 - 4, MINI_TRAY_Y0 - 10, "int[]", { font: "bold 7px Courier New", color: HEX_BLUE_GRAY }).setOrigin(1, 0);
  ...
  async populateMiniTray(values, type) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      const idxPlate = this.add.text(cellX + cellW / 2, MINI_TRAY_Y1 - 6, `[${i}]`, { font: "bold 6px Courier New", color: HEX_BRASS }).setOrigin(0.5).setAlpha(i === 0 ? 0.9 : 0.55);
    ...
    for (let i = 0; i < n; i++) {
      ...
      const txt = this.add.text(0, 0, display, { font: "bold 7px Courier New", color: "#0a1208" }).setOrigin(0.5);
  ...
  showVoidAnnotation() {
    const t = this.add.text(MINI_PLAQUE_X + MINI_PLAQUE_W / 2, MINI_PLAQUE_Y - 10, "void", { font: "bold 9px Courier New", color: HEX_GRAY }).setOrigin(0.5).setAlpha(0);
    this.rigLayer.add(t);
  ...
  createMiniPlaque() {
    ...
    this.rigLayer.add(g);
    this.plaqueText = this.add.text(MINI_PLAQUE_X + MINI_PLAQUE_W / 2, MINI_PLAQUE_Y + MINI_PLAQUE_H / 2, "", { font: "bold 9px Courier New", color: "#e8eaf6", wordWrap: { width: MINI_PLAQUE_W - 10 }, align: "center" }).setOrigin(0.5);
    this.rigLayer.add(this.plaqueText);

    this.stripBeforeText = this.add.text(MINI_X0, MINI_STRIP_Y, "", { font: "7px Courier New", color: HEX_GRAY }).setOrigin(0, 0.5);
    this.stripArrowText = this.add.text(MINI_CX, MINI_STRIP_Y, "", { font: "bold 8px Arial", color: HEX_BRASS }).setOrigin(0.5);
    this.stripAfterText = this.add.text(MINI_X1, MINI_STRIP_Y, "", { font: "bold 7px Courier New", color: "#e8eaf6" }).setOrigin(1, 0.5);
  ...
  async runToStringScan(values, type) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      const comp = this._miniCompartments[i];
      if (comp && comp.card) {
        const ghost = this.add.text(comp.card.x, comp.card.y, String(values[i]), { font: "bold 7px Courier New", color: "#e8eaf6" }).setOrigin(0.5);
  ...
  async bracketAccessGhost(index) {
    ...
    const ghost = this.add.text(comp.card.x, comp.card.y, String(comp.value), { font: "bold 8px Courier New", color }).setOrigin(0.5);
    this.rigLayer.add(ghost);
  ...
  showCompileErrorStamp() {
    const stamp = this.add.text(OX + OW / 2, OY + 100, "COMPILE ERROR", { font: "bold 13px Arial", color: HEX_RED }).setOrigin(0.5).setScale(1.3).setAngle(-6).setAlpha(0);
  ...
  async crashIOOBE(idx) {
    const stamp = this.add.text(MINI_CX, MINI_TRAY_Y0 - 16, "ArrayIndexOutOfBoundsException", { font: "bold 7px Courier New", color: HEX_RED }).setOrigin(0.5).setAngle(-3).setAlpha(0);
    this.missionElements.push(stamp);
    this.rigLayer.add(stamp);
  ...
  createHUD() {
    ...
    this.add.text(20, 14, "THE ARRANGEMENT WORKSHOP", { font: "bold 13px Georgia", color: "#b0bec5" }).setDepth(51);
    this.add.text(20, 32, "Restructuring Phase — Arrays Methods: sort()", { font: "10px Arial", color: "#546e7a" }).setDepth(51);
  ...
  gameOver() {
    ...
    (async () => {
      ...
      this._makeButton(640, 420, "REOPEN THE WORKSHOP", 240, 50, { stroke: C_RED, textColor: HEX_RED }, () => this.scene.restart());
    })();
  }
  ...
  showScoreTally() {
    ...
    const totalText = this.add.text(390, 168 + lines.length * 22, "TOTAL: 0", { font: "bold 20px Arial", color: HEX_GOLD }).setOrigin(0, 0.5).setDepth(91).setAlpha(0);
    ...
    const icon1 = this.add.text(-16, 0, "▭", { font: "12px Arial", color: HEX_GOLD }).setOrigin(0.5);
    const icon2 = this.add.text(0, 0, "⚙", { font: "14px Arial", color: HEX_GOLD }).setOrigin(0.5);
    const icon3 = this.add.text(16, 0, "📋", { font: "12px Arial", color: HEX_GOLD }).setOrigin(0.5);
    badge.add([bg, icon1, icon2, icon3]);
    this.tweens.add({ targets: badge, alpha: 1, duration: 300, delay: 2100 });
    const badgeLbl = this.add.text(640, 492, "sort() MASTERY", { font: "bold 13px Georgia", color: HEX_GOLD }).setOrigin(0.5).setDepth(91).setAlpha(0);
    const badgeSub = this.add.text(640, 508, "Accretion ✓  Tuning ✓  Restructuring ✓", { font: "9px Georgia", color: HEX_GRAY }).setOrigin(0.5).setDepth(91).setAlpha(0);
    ...
    const progLabel = this.add.text(640, barY + barH + 14, "ARRAYS WING — 2 of 3 trilogies complete", { font: "bold 11px Georgia", color: "#5d7a5d" }).setOrigin(0.5).setDepth(91).setAlpha(0);
    const tickLbl = this.add.text(640, barY - 10, "toString ✓   sort ✓ new   copyOf", { font: "9px Courier New", color: HEX_GRAY }).setOrigin(0.5).setDepth(91).setAlpha(0);
  ...
  _makeButton(x, y, label, w, h, style, onClick, depth = 95) {
    ...
    draw(false);
    const t = this.add.text(0, 0, label, { font: "bold 13px Arial", color: style.textColor }).setOrigin(0.5);
