...
export class Level71Scene extends Phaser.Scene {
  ...
  createOfficeInterior() {
    ...
    cells.forEach((c) => {
      ...
      this.add.text(c.x, 78, c.tag, { font: "bold 7px Georgia", color: HEX_COPPER }).setOrigin(0.5).setAlpha(0.4).setDepth(3);
      this.add.text(c.x, 108, c.label, { font: "7px Courier New", color: HEX_CYAN }).setOrigin(0.5).setAlpha(0.35).setDepth(3);
    });
    ...
    this.add.text(640, 25, "T H E   I N T E G E R   F U R N A C E", { font: "bold 12px Georgia", color: HEX_COPPER }).setOrigin(0.5).setAlpha(0.7).setDepth(3);
  }
  ...
  createConversionFurnace() {
    ...
    this.add.text(FURNACE_CX, HOPPER_Y0 - 12, "STRING INPUT", { font: "bold 9px Georgia", color: HEX_COPPER }).setOrigin(0.5).setDepth(11);
    ...
    this.add.text(CONT_X, CONT_Y - 12, "int", { font: "bold 9px Courier New", color: HEX_GOLD }).setOrigin(0.5).setDepth(12);
  ...
  async formIntegerBar(value) {
    ...
    const txt = this.add.text(0, 0, String(value), { font: "bold 17px Courier New", color: "#241a0e" }).setOrigin(0.5);
  ...
  async showNFE(inputStr) {
    ...
    const banner = this.add.text(FURNACE_CX, (CHAMBER_Y0 + CHAMBER_Y1) / 2, "NumberFormatException", { font: "bold 15px Courier New", color: HEX_RED }).setOrigin(0.5).setDepth(30).setScale(1.3).setAlpha(0);
  ...
  showCompileErrorStamp() {
    const stamp = this.add.text(FURNACE_CX, 150, "COMPILE ERROR", { font: "bold 16px Arial", color: HEX_RED }).setOrigin(0.5).setDepth(30).setScale(1.3).setAngle(-6).setAlpha(0);
  ...
  createAssayersSlate() {
    ...
    this.add.text(SLATE_X + SLATE_W - 92, SLATE_Y + 16, "Integer (wrapper class)", { font: "bold 7px Courier New", color: HEX_COPPER }).setOrigin(0.5).setDepth(12);
  ...
  createHUD() {
    ...
    this.add.text(20, 14, "THE INTEGER FURNACE", { font: "bold 14px Georgia", color: "#b0bec5" }).setDepth(50);
    this.add.text(20, 32, "Accretion Phase — Type Conversion: parseInt()", { font: "10px Arial", color: "#546e7a" }).setDepth(50);
  ...
  createCartridgeTray(config) {
    ...
    bdraw(false, false);
    const bt = this.add.text(0, 0, "SMELT", { font: "bold 13px Arial", color: "#0a1208" }).setOrigin(0.5);
    btn.add([bg, bt]);
    btn.setSize(130, 44);
  ...
  gameOver() {
    ...
    (async () => {
      ...
      this._makeButton(640, 420, "RELIGHT THE FURNACE", 240, 50, { stroke: C_RED, textColor: HEX_RED }, () => this.scene.restart());
    })();
  }
  ...
  async furnaceFinale() {
    ...
    for (let i = 0; i < 8; i++) {
      ...
      const txt = this.add.text(0, 0, String(val), { font: "bold 12px Courier New", color: "#241a0e" }).setOrigin(0.5);
      bar.add([bg, txt]);
  ...
  showScoreTally() {
    ...
    const badgeLbl = this.add.text(640, 505, "parseInt() SCHEMA ACQUIRED", { font: "bold 12px Georgia", color: HEX_GOLD }).setOrigin(0.5).setDepth(91).setAlpha(0);
