...
export class Level78Scene extends Phaser.Scene {
  ...
  createConversionTriangleWall() {
    ...
    this.add.text(pStr.x, pStr.y - 10, "String", { font: "bold 7px Georgia", color: HEX_CREAM }).setOrigin(0.5).setDepth(3).setAlpha(0.7);
    this.add.text(pInt.x, pInt.y + 10, "int", { font: "bold 7px Georgia", color: HEX_GOLD }).setOrigin(0.5).setDepth(3).setAlpha(0.7);
    this.add.text(pDbl.x, pDbl.y + 10, "double", { font: "bold 7px Georgia", color: HEX_ORANGE }).setOrigin(0.5).setDepth(3).setAlpha(0.7);
    this._triangleGfx = g;
  }
  ...
  createTrialsBanner() {
    ...
    this.add.text(640, 25, "THE INSCRIPTION TRIALS", { font: "bold 12px Georgia", color: HEX_CREAM }).setOrigin(0.5).setAlpha(0.7).setDepth(3);
  }
  ...
  createInscriptionOrder() {
    ...
    this.orderHeaderText = this.add.text(ORDER_CX, ORDER_Y0 + 12, "", { font: "bold 8px Georgia", color: HEX_COPPER }).setOrigin(0.5).setDepth(21);
    this.orderRoundLabel = this.add.text(ORDER_X1 - 14, ORDER_Y0 + 12, "SEAL 1/15", { font: "bold 9px Courier New", color: HEX_COPPER }).setOrigin(1, 0.5).setDepth(21);
    this.orderContentContainer = this.add.container(0, 0).setDepth(21);
    this.orderQuestionText = this.add.text(ORDER_CX, ORDER_Y1 - 26, "", { font: "bold 12px Georgia", color: "#e0d6c8", wordWrap: { width: ORDER_X1 - ORDER_X0 - 40 }, align: "center" }).setOrigin(0.5).setDepth(21);
  ...
  createWaxSeal() {
    ...
    const ringFace = this.add.text(0, 0, "S", { font: "bold 8px Georgia", color: HEX_COPPER }).setOrigin(0.5);
  ...
  async waxFullySet() {
    ...
    this._stopWaxPulse();
    const sealed = this.add.text(WAX_CX, WAX_CY, "SEALED", { font: "bold 10px Georgia", color: HEX_CREAM }).setOrigin(0.5).setDepth(18).setAlpha(0);
  ...
  createMiniTripleApparatus() {
    const drawFrame = (x0, x1, y0, y1, label, colorHex, labelHex) => {
      ...
      const t = this.add.text((x0 + x1) / 2, y0 - 10, label, { font: "bold 7px Georgia", color: labelHex }).setOrigin(0.5).setDepth(11).setAlpha(0.5);
      return { g, t };
    };
    ...
    this._mfContText = this.add.text(mfCx, MINI_TOP_Y1 - 12, "int —", { font: "bold 8px Courier New", color: HEX_GRAY }).setOrigin(0.5).setDepth(12);
    this._mcContText = this.add.text(mcCx, MINI_TOP_Y1 - 12, "double —", { font: "bold 8px Courier New", color: HEX_GRAY }).setOrigin(0.5).setDepth(12);
    this._mpContText = this.add.text(mpCx, MINI_BOT_Y1 - 12, "String —", { font: "bold 8px Courier New", color: HEX_GRAY }).setOrigin(0.5).setDepth(12);
  }
  ...
  async runMiniFurnaceConversion(strValue) {
    ...
    if (!valid) {
      ...
      await this.delay(200);
      const nfe = this.add.text(cx, MINI_TOP_Y0 + 100, "NFE", { font: "bold 10px Courier New", color: HEX_RED }).setOrigin(0.5).setDepth(22).setAlpha(0);
  ...
  async runMiniCrucibleConversion(strValue) {
    ...
    const valText = this.add.text(cx, MINI_TOP_Y0 + 85, display, { font: "bold 7px Courier New", color: "#241a0e" }).setOrigin(0.5).setDepth(22).setAlpha(0);
  ...
  async runMiniPress(value, type, strRepr) {
    ...
    const chipTxt = this.add.text(0, 0, inputDisplay, { font: "bold 7px Courier New", color: "#1a1408" }).setOrigin(0.5);
  ...
  createTrialSlate() {
    ...
    this.add.text(SLATE_X + 10, SLATE_Y + 8, "TRIAL SLATE", { font: "bold 8px Georgia", color: HEX_CREAM }).setDepth(11);
  ...
  createContainerShelf() {
    ...
    this.add.text(SHELF_X + 10, SHELF_Y + 6, "VARIABLES", { font: "bold 8px Georgia", color: HEX_CREAM }).setDepth(11);
  ...
  createHUD() {
    ...
    this.add.text(20, 14, "THE INSCRIPTION TRIALS", { font: "bold 14px Georgia", color: "#b0bec5" }).setDepth(50);
    this.add.text(20, 32, "Tuning Phase — Type Conversion: valueOf()", { font: "10px Arial", color: "#546e7a" }).setDepth(50);
  ...
  createBit() {
    ...
    const ringFace = this.add.text(0, 0, "S→", { font: "bold 4px Georgia", color: HEX_COPPER }).setOrigin(0.5);
  ...
  async onWaxTimeout(config) {
    ...
    if (!this._alive) return;
    if (config.revealNote) this.createFloatingText(ORDER_CX, ORDER_Y1 + 40, config.revealNote, HEX_GRAY, "10px Arial", 2800);
  ...
  setupBugHunt(config) {
    this.clearOrderContent();
    this.orderHeaderText.setText(`INSCRIPTION ORDER — SEAL ${this.currentRound + 1}`);
    const header = this.add.text(ORDER_CX, ORDER_Y0 + 36, "CLICK THE BUG", { font: "bold 12px Georgia", color: "#c62828" }).setOrigin(0.5);
  ...
  async onTokenClicked(tokenObj, config, lineY) {
    ...
    if (correct) {
      ...
      if (config.fix) {
        const fixT = this.add.text(ORDER_CX, lineY - 14, config.fix, { font: "bold 10px Courier New", color: "#2e7d32", wordWrap: { width: 380 } }).setOrigin(0.5).setAlpha(0);
  ...
  showCompileErrorStamp() {
    const stamp = this.add.text(ORDER_CX, ORDER_Y0 - 22, "COMPILE ERROR", { font: "bold 16px Arial", color: HEX_RED }).setOrigin(0.5).setDepth(30).setScale(1.3).setAngle(-6).setAlpha(0);
  ...
  scoreForAttempt(timePctUsed) {
    ...
    if (remaining > 0.6) { points += 50; this.fastBonusCount++; this.createFloatingText(ORDER_CX, ORDER_Y0 - 14, "⚡ HOT SEAL +50", HEX_GOLD, "bold 13px Arial", 900); }
    else if (remaining > 0.3) { points += 25; this.fastBonusCount++; this.createFloatingText(ORDER_CX, ORDER_Y0 - 14, "⚡ +25", HEX_GOLD, "bold 12px Arial", 800); }
    return points;
  }
  ...
  gameOver() {
    ...
    (async () => {
      ...
      this.add.text(640, 350, `Seals Inscribed: ${this.currentRound} / 15`, { font: "16px Arial", color: HEX_GRAY }).setOrigin(0.5).setDepth(91);
      this._makeButton(640, 420, "REMELT THE WAX", 220, 50, { stroke: C_RED, textColor: HEX_RED }, () => this.scene.restart());
    })();
  }
  ...
  showScoreTally() {
    ...
    const badgeS = this.add.text(0, 0, "S", { font: "bold 11px Georgia", color: "#3a2a00" }).setOrigin(0.5);
    badge.add([bg, badgeS]);
    this.tweens.add({ targets: badge, alpha: 1, duration: 300, delay: 2100 });
    const badgeLbl = this.add.text(640, 520, "valueOf() SCHEMA TUNED", { font: "bold 13px Georgia", color: HEX_GOLD }).setOrigin(0.5).setDepth(91).setAlpha(0);
