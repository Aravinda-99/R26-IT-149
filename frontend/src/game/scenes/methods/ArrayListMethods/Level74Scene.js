...
export class Level74Scene extends Phaser.Scene {
  ...
  createRoomInterior() {
    ...
    this.add.text(520, 30, "0.0 ————————— 10.0", { font: "7px Courier New", color: HEX_ORANGE }).setOrigin(0.5).setAlpha(0.3).setDepth(2);
    ...
    this._furnaceSilLabel = this.add.text((FSIL_X0 + FSIL_X1) / 2, FSIL_Y1 - 4, "INTEGER FURNACE", { font: "bold 6px Georgia", color: HEX_COPPER }).setOrigin(0.5).setAlpha(0.3).setDepth(3);
    ...
    this.add.text(640, 25, "T H E   D E C I M A L   C R U C I B L E", { font: "bold 13px Georgia", color: HEX_ORANGE }).setOrigin(0.5).setAlpha(0.7).setDepth(3);
  }
  ...
  createDecimalCrucible() {
    ...
    this.add.text(CRUCIBLE_CX, HOPPER_Y0 - 12, "STRING INPUT", { font: "bold 9px Georgia", color: HEX_COPPER }).setOrigin(0.5).setDepth(11);
    ...
    this.vesselValueText = this.add.text(VESSEL_X1 + 14, (VESSEL_Y0 + VESSEL_Y1) / 2, "", { font: "bold 16px Courier New", color: HEX_ORANGE }).setOrigin(0, 0.5).setDepth(13);
    this.vesselDotZeroText = this.add.text(0, 0, "", { font: "12px Courier New", color: HEX_ORANGE }).setOrigin(0, 0.5).setDepth(13).setAlpha(0.6);
    ...
    this.add.text(CONT_X, CONT_Y - 12, "double", { font: "bold 9px Courier New", color: HEX_ORANGE }).setOrigin(0.5).setDepth(12);
    this.containerValueText = this.add.text(CONT_X, CONT_Y + 25, "—", { font: "bold 14px Courier New", color: HEX_GRAY }).setOrigin(0.5).setDepth(12);
    this.containerNameText = this.add.text(CONT_X, CONT_Y + 45, "", { font: "italic 8px Courier New", color: HEX_GRAY }).setOrigin(0.5).setDepth(12);
  ...
  async materializePaperStrip(value) {
    ...
    const txt = this.add.text(0, 0, value, { font: "bold 16px Courier New", color: "#241a0e" }).setOrigin(0.5);
  ...
  async settleLevel(value) {
    ...
    if (magnitude / maxDisplay > 1) {
      const overflow = this.add.text(vcx0 + (vcx1 - vcx0) / 2, VESSEL_Y0 - 10, "▲", { font: "bold 14px Arial", color: HEX_AMBER }).setOrigin(0.5).setDepth(13);
  ...
  async showNFE(inputStr) {
    ...
    const banner = this.add.text(CRUCIBLE_CX, (VESSEL_Y0 + VESSEL_Y1) / 2, "NumberFormatException", { font: "bold 15px Courier New", color: HEX_RED }).setOrigin(0.5).setDepth(30).setScale(1.3).setAlpha(0);
  ...
  showCompileErrorStamp() {
    const stamp = this.add.text(CRUCIBLE_CX, 150, "COMPILE ERROR", { font: "bold 16px Arial", color: HEX_RED }).setOrigin(0.5).setDepth(30).setScale(1.3).setAngle(-6).setAlpha(0);
  ...
  async showParseIntContrastReveal(strValue) {
    ...
    const txt = this.add.text(0, 0, strValue, { font: "bold 10px Courier New", color: "#241a0e" }).setOrigin(0.5);
    strip.add([bg, txt]);
    ...
    await this.delay(250);
    const banner = this.add.text(fcx, FSIL_Y0 + 85, "NFE", { font: "bold 11px Courier New", color: HEX_RED }).setOrigin(0.5).setDepth(30).setAlpha(0);
  ...
  createAssayersSlate() {
    ...
    this.add.text(SLATE_X + 14, SLATE_Y + 12, "ASSAYER'S SLATE", { font: "bold 10px Georgia", color: HEX_INDIGO }).setDepth(11);
    ...
    this.add.text(SLATE_X + SLATE_W - 95, SLATE_Y + 16, "Double (wrapper class)", { font: "bold 7px Courier New", color: HEX_ORANGE }).setOrigin(0.5).setDepth(12);

    // parseInt vs parseDouble comparison strip
    this.add.text(SLATE_X + 14, SLATE_Y + 30, "parseInt → int", { font: "bold 8px Courier New", color: HEX_GOLD }).setDepth(11);
    this.add.text(SLATE_X + 130, SLATE_Y + 30, "parseDouble → double", { font: "bold 8px Courier New", color: HEX_ORANGE }).setDepth(11);
  ...
  createHUD() {
    ...
    this.add.text(20, 14, "THE DECIMAL CRUCIBLE", { font: "bold 14px Georgia", color: "#b0bec5" }).setDepth(50);
    this.add.text(20, 32, "Accretion Phase — Type Conversion: parseDouble()", { font: "10px Arial", color: "#546e7a" }).setDepth(50);
  ...
  createCartridgeTray(config) {
    ...
    bdraw(false, false);
    const bt = this.add.text(0, 0, "DISSOLVE", { font: "bold 13px Arial", color: "#0a1208" }).setOrigin(0.5);
    btn.add([bg, bt]);
    btn.setSize(130, 44);
  ...
  async showParseIntBareNFE(strVal) {
    ...
    const stamp = this.add.text(CRUCIBLE_CX, 150, "NumberFormatException", { font: "bold 14px Courier New", color: HEX_RED }).setOrigin(0.5).setDepth(30).setAlpha(0);
    this.roundElements.push(stamp);
  ...
  gameOver() {
    ...
    (async () => {
      ...
      this._makeButton(640, 420, "REHEAT THE CRUCIBLE", 240, 50, { stroke: C_RED, textColor: HEX_RED }, () => this.scene.restart());
    })();
  }
  ...
  showScoreTally() {
    ...
    const badgeLbl = this.add.text(640, 505, "parseDouble() SCHEMA ACQUIRED", { font: "bold 12px Georgia", color: HEX_GOLD }).setOrigin(0.5).setDepth(91).setAlpha(0);
