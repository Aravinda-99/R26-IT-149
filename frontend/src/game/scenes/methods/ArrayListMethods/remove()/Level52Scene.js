...
export class Level52Scene extends Phaser.Scene {
  ...
  createDeaccessionBanner() {
    ...
    this.add.text(400, 43, "D E A C C E S S I O N   O F F I C E", { font: "bold 15px Georgia", color: HEX_BRASS }).setOrigin(0.5).setAlpha(0.7).setDepth(3);
  }
  ...
  createBookshelf() {
    ...
    this.typeStampText = this.add.text(240, 150, "", { font: "bold 14px Courier New", color: HEX_CYAN }).setOrigin(0.5).setDepth(6);
    ...
    this.sizeCounterText = this.add.text(386, 150, "size: 0", { font: "bold 13px Courier New", color: HEX_BRASS }).setOrigin(0.5).setDepth(6);
  ...
  _makeBookVisual(entry, x, y) {
    ...
    const label = this.add.text(0, 26, "", { font: "bold 10px Courier New", color: HEX_BRASS }).setOrigin(0.5).setAlpha(0.6);
    c.add([g, txt, label]);
  ...
  createDeaccessionCrate() {
    ...
    this.crateLabel = this.add.text(CRATE_CX, (CRATE.y0 + CRATE.y1) / 2 + 12, "WITHDRAWN", { font: "bold 16px Georgia", color: HEX_STAMP_RED }).setOrigin(0.5).setAlpha(0.5).setAngle(-2).setDepth(9);

    // return chute
    const ch = this.add.graphics().setDepth(6);
    ch.lineStyle(1.5, C_BRASS, 0.6);
    ch.strokeRoundedRect(CHUTE.x - 45, CHUTE.y - 12, 90, 24, 5);
    this.add.text(CHUTE.x, CHUTE.y - 20, "RETURNED VALUE", { font: "bold 10px Georgia", color: HEX_BRASS }).setOrigin(0.5).setAlpha(0.7).setDepth(6);
  }
  ...
  async slideInWithdrawalOrder(callText) {
    ...
    const t = this.add.text(0, 0, callText, { font: "bold 14px Courier New", color: "#241a0e", wordWrap: { width: 120 }, align: "center" }).setOrigin(0.5);
  ...
  async printToConsole(text) {
    ...
    const t = this.add.text(CONSOLE_POS.x, CONSOLE_POS.y, "> ", { font: "bold 15px Courier New", color: HEX_GREEN_BRIGHT }).setOrigin(0, 0.5).setDepth(12);
  ...
  async stampThump(bookContainer) {
    ...
    const mark = this.add.text(0, 0, "WITHDRAWN", { font: "bold 12px Georgia", color: HEX_STAMP_RED }).setOrigin(0.5).setAngle(-20).setAlpha(0);
  ...
  _popReturnValue(entry, fromX, fromY) {
    const valText = this.add.text(fromX, fromY, String(entry.value), { font: "bold 19px Courier New", color: this._typeColorHex(entry.type) }).setOrigin(0.5).setDepth(30);
  ...
  async deliverToVariable(name, value, type) {
    ...
    const valT = this.add.text(0, 0, String(value), { font: "bold 14px Courier New", color: this._typeColorHex(type) }).setOrigin(0.5);
  ...
  async notFoundFizzle() {
    ...
    if (this._activeOrder && this._activeOrder.container.active) {
      const stamp = this.add.text(0, 8, "NOT FOUND — no change", { font: "bold 11px Courier New", color: "#78909c" }).setOrigin(0.5).setAngle(-6).setAlpha(0);
  ...
  async retrieveGhost(index) {
    ...
    this.updateRemovedRow(entry.value, entry.type);
    const valText = this.add.text(SHELF_CX + 130, shelfY - 45, String(entry.value), { font: "bold 19px Courier New", color: this._typeColorHex(entry.type) }).setOrigin(0.5).setDepth(30).setAlpha(0);
    this.roundElements.push(valText);
  ...
  async crashWithdrawal(index) {
    ...
    if (this._activeOrder && this._activeOrder.container.active) {
      ...
      const stamp = this.add.text(0, 6, "IndexOutOfBounds\nException", { font: "bold 12px Courier New", color: HEX_RED, align: "center" }).setOrigin(0.5).setAngle(-8).setScale(1.5).setAlpha(0);
      order.add(stamp);
  ...
  createListStatePanel() {
    ...
    this.bracketText = this.add.text(PANEL_X + PANEL_W / 2, PANEL_Y + 88, "[]", { font: "bold 17px Courier New", color: HEX_GRAY, wordWrap: { width: PANEL_W - 30 }, align: "center" }).setOrigin(0.5).setDepth(12);
    this.panelSizeText = this.add.text(PANEL_X + PANEL_W / 2, PANEL_Y + 124, "size: 0", { font: "13px Courier New", color: HEX_BRASS }).setOrigin(0.5).setAlpha(0.85).setDepth(12);
    this.panelIndexText = this.add.text(PANEL_X + PANEL_W / 2, PANEL_Y + 145, "", { font: "bold 12px Courier New", color: "#8a6435" }).setOrigin(0.5).setAlpha(0.7).setDepth(12);

    this.add.text(PANEL_X + 16, PANEL_Y + 172, "removed:", { font: "13px Georgia", color: "#8a6435" }).setOrigin(0, 0.5).setDepth(12);
    this.removedValueText = this.add.text(PANEL_X + 84, PANEL_Y + 172, "—", { font: "bold 15px Courier New", color: HEX_GRAY }).setOrigin(0, 0.5).setDepth(12);

    // before/after strip
    this.add.text(PANEL_X + 16, PANEL_Y + 200, "before:", { font: "13px Courier New", color: "#8a6435" }).setOrigin(0, 0.5).setDepth(12);
    this.beforeRowText = this.add.text(PANEL_X + 78, PANEL_Y + 200, "", { font: "bold 13px Courier New", color: "#b0bec5" }).setOrigin(0, 0.5).setDepth(12);
    this.add.text(PANEL_X + 16, PANEL_Y + 224, "after:", { font: "13px Courier New", color: "#8a6435" }).setOrigin(0, 0.5).setDepth(12);
    this.afterRowText = this.add.text(PANEL_X + 78, PANEL_Y + 224, "", { font: "bold 13px Courier New", color: HEX_GREEN_BRIGHT }).setOrigin(0, 0.5).setDepth(12);
  }
  ...
  async onBubbleSelected(opt, config, bubbleContainer) {
    ...
    this.fillAfterRow();
    if (config.revealNote) this.createFloatingText(PANEL_X + PANEL_W / 2, PANEL_Y + PANEL_H + 40, config.revealNote, HEX_GRAY, "13px Arial", 2800);
  ...
  createCartridgeTray(config) {
    ...
    bdraw(false, false);
    const bt = this.add.text(0, 0, "WITHDRAW", { font: "bold 16px Arial", color: HEX_STAMP_RED }).setOrigin(0.5);
    btn.add([bg, bt]);
    btn.setSize(140, 44);
  ...
  async onWithdrawPressed(config) {
    ...
    for (let ti = 0; ti < tests.length; ti++) {
      ...
      if (!this._alive) return;
      this.fillBeforeRow();
      if (tests.length > 1) this.createFloatingText(SHELF_CX, SHELF_Y0 - 14, `TEST ${ti + 1}: [${listForTest.join(", ")}]`, HEX_BRASS, "bold 14px Courier New", 1400);
  ...
  gameOver() {
    ...
    (async () => {
      ...
      this.add.text(640, 350, `Orders Executed: ${this.currentRound} / 12`, { font: "18px Arial", color: HEX_GRAY }).setOrigin(0.5).setDepth(91);
      this._makeButton(525, 420, "REOPEN THE LEDGER", 200, 50, { stroke: C_RED, textColor: HEX_RED }, () => this.scene.restart());
  ...
  showScoreTally() {
    ...
    const badgeLbl = this.add.text(640, 505, "remove() SCHEMA ACQUIRED", { font: "bold 15px Georgia", color: HEX_GOLD }).setOrigin(0.5).setDepth(91).setAlpha(0);
