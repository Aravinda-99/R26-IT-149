...
export class Level64Scene extends Phaser.Scene {
  ...
  createHallInterior() {
    ...
    this.add.text(580, 25, "T H E   S P E C I M E N   H A L L", { font: "bold 13px Georgia", color: HEX_BRASS }).setOrigin(0.5).setAlpha(0.7).setDepth(4);
  }
  ...
  createDisplayPlaque() {
    ...
    this.plaqueText = this.add.text(PLAQUE_X + PLAQUE_W / 2, PLAQUE_Y + PLAQUE_H / 2, "", { font: "bold 16px Courier New", color: "#e8eaf6", wordWrap: { width: PLAQUE_W - 20 }, align: "center" }).setOrigin(0.5).setDepth(11);
  ...
  createHUD() {
    ...
    this.add.text(20, 14, "THE SPECIMEN HALL", { font: "bold 14px Georgia", color: "#b0bec5" }).setDepth(50);
    this.add.text(20, 32, "Accretion Phase — Arrays Methods: toString()", { font: "10px Arial", color: "#546e7a" }).setDepth(50);
  ...
  async onBubbleSelected(opt, config, bubbleContainer) {
    ...
    if (!this._alive) return;
    if (config.revealNote) this.createFloatingText(TRAY_CX, 175, config.revealNote, HEX_GRAY, "11px Arial", 3000);
  ...
  createCartridgeTray(config) {
    ...
    bdraw(false, false);
    const bt = this.add.text(0, 0, "LABEL", { font: "bold 14px Arial", color: "#0a1208" }).setOrigin(0.5);
    btn.add([bg, bt]);
    btn.setSize(130, 44);
  ...
  async onLabelPressed(config) {
    ...
    this.createFloatingText(TRAY_CX, 175, pass ? "✓" : "✗", pass ? HEX_GREEN_BRIGHT : HEX_RED, "bold 22px Arial", 900);
  ...
  gameOver() {
    ...
    (async () => {
      ...
      this._makeButton(640, 420, "REOPEN THE HALL", 220, 50, { stroke: C_RED, textColor: HEX_RED }, () => this.scene.restart());
    })();
  }
  ...
  showScoreTally() {
    ...
    const badgeLbl = this.add.text(640, 505, "toString() SCHEMA ACQUIRED", { font: "bold 12px Georgia", color: HEX_GOLD }).setOrigin(0.5).setDepth(91).setAlpha(0);
