...
export class Level56Scene extends Phaser.Scene {
  ...
  createMeridianTelescope() {
    ...
    this.add.text(90, 392, "MERIDIAN CIRCLE", { font: "bold 8px Georgia", color: HEX_BRASS }).setOrigin(0.5).setAlpha(0.5).setDepth(4);
  }
  ...
  createMeridianBanner() {
    ...
    this.add.text(580, 25, "T H E   M E R I D I A N   T R I A L S", { font: "bold 13px Georgia", color: HEX_BRASS }).setOrigin(0.5).setAlpha(0.7).setDepth(3);
  }
  ...
  createObservationSlip() {
    ...
    this.add.text(SLIP_CX, SLIP_Y0 + 12, "TRANSIT OBSERVATION — FORM 7", { font: "bold 8px Georgia", color: "#8a6435" }).setOrigin(0.5).setDepth(21);
    this.slipRoundLabel = this.add.text(SLIP_X1 - 14, SLIP_Y0 + 12, "TRANSIT 1/15", { font: "bold 9px Courier New", color: "#8a6435" }).setOrigin(1, 0.5).setDepth(21);
    this.slipContentContainer = this.add.container(0, 0).setDepth(21);
    this.slipQuestionText = this.add.text(SLIP_CX, SLIP_Y1 - 26, "", { font: "bold 12px Georgia", color: "#241a0e", wordWrap: { width: SLIP_X1 - SLIP_X0 - 40 }, align: "center" }).setOrigin(0.5).setDepth(21);
  ...
  createMiniComparator() {
    ...
    this.add.text(MC_BEAM_CX, MC_BEAM_Y - 18, "Math", { font: "bold 11px Courier New", color: HEX_GOLD }).setOrigin(0.5).setDepth(6);
    ...
    this.mcModeText = this.add.text(MC_BEAM_CX, MC_BEAM_Y + 12, ".max", { font: "bold 9px Courier New", color: HEX_CYAN }).setOrigin(0.5).setDepth(6);
  ...
  async mcInstanceCallShudder(value, type) {
    ...
    const q = this.add.text(star.container.x, star.container.y - 24, "?", { font: "bold 16px Georgia", color: HEX_RED }).setOrigin(0.5).setDepth(20).setAlpha(0);
  ...
  showCompileErrorStamp() {
    const stamp = this.add.text(SLIP_CX, SLIP_Y0 - 22, "COMPILE ERROR", { font: "bold 18px Arial", color: HEX_RED }).setOrigin(0.5).setDepth(30).setScale(1.4).setAngle(-6).setAlpha(0);
  ...
  async placeOnRail(entries) {
    ...
    for (let v = Math.ceil(lo / tickStep) * tickStep; v <= hi; v += tickStep) {
      ...
      const lbl = this.add.text(x, RAIL_Y + 13, isZero ? "0" : String(Math.round(v * 10) / 10), { font: "9px Courier New", color: HEX_BLUE_GRAY }).setOrigin(0.5).setAlpha(0.7);
    ...
    for (const e of entries) {
      ...
      const lbl = this.add.text(0, -14, e.type === "double" ? Number(e.value).toFixed(1) : String(e.value), { font: "bold 9px Courier New", color: e.type === "double" ? HEX_ORANGE : HEX_GOLD }).setOrigin(0.5);
      m.add([dot, lbl]);
  ...
  createObservationSlate() {
    ...
    this.add.text(SLATE_X + 12, SLATE_Y + 10, "OBSERVATION SLATE", { font: "bold 9px Georgia", color: HEX_BRASS }).setDepth(11);
    this.slateLines = this.add.container(0, 0).setDepth(11);
    this._slateY = SLATE_Y + 32;
    this.add.text(SLATE_X + 12, SLATE_Y + SLATE_H - 18, "result:", { font: "10px Georgia", color: "#8a6435" }).setDepth(11);
    this.resultText = this.add.text(SLATE_X + 58, SLATE_Y + SLATE_H - 18, "—", { font: "bold 12px Courier New", color: HEX_GRAY }).setOrigin(0, 0.5).setDepth(11);
  }
  ...
  chalkEvaluationArrow(value, type) {
    const display = type === "double" ? Number(value).toFixed(1) : String(value);
    const t = this.add.text(SLATE_X + 12, this._slateY, `→ ${display}`, { font: "bold 11px Courier New", color: this._mcTypeColorHex(type) }).setAlpha(0);
    this.slateLines.add(t);
  ...
  createHUD() {
    ...
    this.add.text(20, 14, "THE MERIDIAN TRIALS", { font: "bold 15px Georgia", color: "#b0bec5" }).setDepth(50);
    this.add.text(20, 32, "Tuning Phase — Math Methods: max() & min()", { font: "11px Arial", color: "#546e7a" }).setDepth(50);
  ...
  async onTransitTimeout(config) {
    ...
    if (!this._alive) return;
    if (config.revealNote) this.createFloatingText(SLIP_CX, SLIP_Y1 + 40, config.revealNote, HEX_GRAY, "10px Arial", 2800);
  ...
  setupBugHunt(config) {
    this.clearSlipContent();
    const header = this.add.text(SLIP_CX, SLIP_Y0 + 36, "CLICK THE BUG", { font: "bold 12px Georgia", color: "#c62828" }).setOrigin(0.5);
  ...
  async onTokenClicked(tokenObj, config, lineY) {
    ...
    if (correct) {
      ...
      const fixT = this.add.text(SLIP_CX, lineY - 14, config.fix, { font: "bold 10px Courier New", color: "#2e7d32", wordWrap: { width: 380 } }).setOrigin(0.5).setAlpha(0);
  ...
  scoreForAttempt(timePctUsed) {
    ...
    if (remaining > 0.6) { points += 50; this.fastBonusCount++; this.createFloatingText(SLIP_CX, SLIP_Y0 - 14, "⚡ EARLY TRANSIT +50", HEX_GOLD, "bold 13px Arial", 900); }
    else if (remaining > 0.3) { points += 25; this.fastBonusCount++; this.createFloatingText(SLIP_CX, SLIP_Y0 - 14, "⚡ +25", HEX_GOLD, "bold 12px Arial", 800); }
    return points;
  }
  ...
  gameOver() {
    ...
    (async () => {
      ...
      this.add.text(640, 350, `Transits Logged: ${this.currentRound} / 15`, { font: "16px Arial", color: HEX_GRAY }).setOrigin(0.5).setDepth(91);
      this._makeButton(640, 420, "AWAIT THE NEXT STAR", 240, 50, { stroke: C_RED, textColor: HEX_RED }, () => this.scene.restart());
    })();
  }
  ...
  levelComplete() {
    ...
    try { GameManager.completeLevel(55, Math.round((this.correctFirstTry / 15) * 100)); } catch (_) {}
  ...
  showScoreTally() {
    ...
    const badgeLbl = this.add.text(640, 520, "max()/min() SCHEMA TUNED", { font: "bold 12px Georgia", color: HEX_GOLD }).setOrigin(0.5).setDepth(91).setAlpha(0);
