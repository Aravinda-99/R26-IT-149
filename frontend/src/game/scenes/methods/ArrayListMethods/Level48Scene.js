...
export class Level48Scene extends Phaser.Scene {
  ...
  createReadingRoomInterior() {
    ...
    this.add.text(400, 26, "T H E   R E A D I N G   R O O M", { font: "bold 13px Georgia", color: HEX_BRASS }).setOrigin(0.5).setAlpha(0.7).setDepth(3);
  }
  ...
  createQuietSign() {
    ...
    this.quietSignText = this.add.text(890, 60, "QUIET PLEASE", { font: "bold 10px Georgia", color: HEX_BRASS }).setOrigin(0.5).setAlpha(0.4).setDepth(5);
  ...
  createCodeCanvas() {
    ...
    this.tabFilename = this.add.text(CX + CW - 12, CY + TAB_H / 2, "Reading1.java", { font: "11px Courier New", color: "#546e7a" }).setOrigin(1, 0.5).setDepth(11);
  ...
  createRigWindow() {
    ...
    this.add.text(OX + 10, OY + 6, "ARCHIVE RIG — LIVE", { font: "bold 9px Georgia", color: HEX_BRASS }).setAlpha(0.7).setDepth(11);
  ...
  createMiniBookshelf() {
    ...
    this.typeStampText = this.add.text(MS_CX - 20, OY + 48, "", { font: "bold 8px Courier New", color: HEX_CYAN }).setOrigin(0.5);
    this.sizeCounterText = this.add.text(MS_X1 - 4, OY + 48, "size: 0", { font: "bold 8px Courier New", color: HEX_BRASS }).setOrigin(1, 0.5);
    ...
    for (let i = 0; i < 7; i++) {
      ...
      const idxText = this.add.text(MS_X0 + 10, y, String(i), { font: "bold 8px Courier New", color: HEX_GRAY }).setOrigin(0.5);
  ...
  createMiniListStatePanel() {
    const hdr = this.add.text(LP_X, OY + 40, "LIST STATE", { font: "bold 8px Georgia", color: HEX_BRASS }).setAlpha(0.7);
    this.bracketText = this.add.text(LP_CX, OY + 82, "[]", { font: "bold 10px Courier New", color: HEX_GRAY, wordWrap: { width: 250 }, align: "center" }).setOrigin(0.5);
    this.panelSizeText = this.add.text(LP_CX, OY + 112, "size: 0", { font: "9px Courier New", color: HEX_BRASS }).setOrigin(0.5).setAlpha(0.85);
    this.panelIndexText = this.add.text(LP_CX, OY + 128, "", { font: "bold 8px Courier New", color: "#8a6435", wordWrap: { width: 250 }, align: "center" }).setOrigin(0.5).setAlpha(0.7);
  ...
  createMiniCrossWingCameos() {
    ...
    const pl = this.add.text(OX + 336, OY + 207, "PRESS", { font: "bold 6px Georgia", color: HEX_BRASS }).setAlpha(0.7);
    this.pressText = this.add.text(OX + 382, OY + 218, "", { font: "bold 8px Courier New", color: HEX_ORANGE }).setOrigin(0.5);
  ...
  miniDispenseTo(name, value) {
    ...
    if (!this.containerObjs[name]) {
      ...
      const t = this.add.text(LP_X + 18, y + 10, "", { font: "bold 8px Courier New", color: HEX_CYAN }).setOrigin(0, 0.5);
  ...
  _makeMiniBook(value) {
    ...
    const txt = this.add.text(0, 0, label, { font: "bold 8px Georgia", color: "#0a0704" }).setOrigin(0.5);
  ...
  async showRigStamp(label) {
    const stamp = this.add.text(OX + OW / 2, OY + 120, label, { font: "bold 11px Courier New", color: HEX_RED }).setOrigin(0.5).setAngle(-6).setAlpha(0).setDepth(25);
  ...
  buildReportRows(mission) {
    ...
    mission.tests.forEach((test, i) => {
      ...
      const expT = this.add.text(140, 0, expected, { font: "9px Courier New", color: "#78909c" }).setOrigin(0, 0.5);
      const actualT = this.add.text(290, 0, "", { font: "9px Courier New", color: HEX_RED }).setOrigin(0, 0.5);
  ...
  createHUD() {
    ...
    this.add.text(20, 14, "THE READING ROOM", { font: "bold 13px Georgia", color: "#b0bec5" }).setDepth(51);
    this.add.text(20, 32, "Restructuring Phase — ArrayList Methods: add()", { font: "10px Arial", color: "#546e7a" }).setDepth(51);
  ...
  gameOver() {
    ...
    this._makeButton(640, 420, "REOPEN THE ROOM", 230, 50, { stroke: C_RED, textColor: HEX_RED }, () => this.scene.restart());
  }
  ...
  levelComplete() {
    ...
    try { GameManager.completeLevel(47, Math.round((this.flawlessCount / MISSIONS.length) * 100)); } catch (_) {}
  ...
  showScoreTally() {
    ...
    const badgeLbl = this.add.text(640, 478, "add() MASTERY", { font: "bold 13px Georgia", color: HEX_GOLD }).setOrigin(0.5).setDepth(91).setAlpha(0);
    ...
    const wingLbl = this.add.text(640, 512, "ARRAYLIST WING — 1 of 3 trilogies complete", { font: "bold 11px Georgia", color: "#8c7ae6" }).setOrigin(0.5).setDepth(91).setAlpha(0);
