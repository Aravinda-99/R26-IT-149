...
export class Level68Scene extends Phaser.Scene {
  ...
  createBenchInterior() {
    ...
    const arrow = this.add.text(500, 80, "→", { font: "bold 16px Georgia", color: HEX_BRASS }).setOrigin(0.5).setAlpha(0.6).setDepth(2);
    this.add.text(500, 80, "copyOf", { font: "italic 8px Georgia", color: HEX_BRASS }).setOrigin(0.5, 2.4).setAlpha(0.5).setDepth(2);
    this.add.text(500, 120, "independent", { font: "italic 9px Georgia", color: HEX_BLUE_GRAY }).setOrigin(0.5).setAlpha(0.4).setDepth(2);
    const ne = this.add.text(468, 120, "≠", { font: "bold 10px Georgia", color: HEX_GRAY }).setOrigin(0.5).setAlpha(0.5).setDepth(2);
    ...
    this.add.text(640, 25, "T H E   C O P Y   B E N C H", { font: "bold 14px Georgia", color: HEX_BRASS }).setOrigin(0.5).setAlpha(0.7).setDepth(3);
  }
  ...
  createAnnotation(x, y, text, colorHex) {
    const t = this.add.text(x, y, text, { font: "italic 11px Georgia", color: colorHex, wordWrap: { width: 320 }, align: "center" }).setOrigin(0.5).setDepth(70).setAlpha(0);
  ...
  createReplicationFrame() {
    ...
    this.origLabelPlate = this.add.text(ORIG_X0 + 6, TRAY_Y0 - 16, "ORIGINAL", { font: "bold 10px Georgia", color: HEX_BRASS }).setDepth(11);
    this.copyLabelPlate = this.add.text(COPY_X0 + 6, TRAY_Y0 - 16, "COPY", { font: "bold 10px Georgia", color: HEX_BRASS }).setDepth(11).setAlpha(0);
    this.origVarLabel = this.add.text(ORIG_X1 - 6, TRAY_Y0 - 16, "", { font: "italic 9px Courier New", color: HEX_CYAN }).setOrigin(1, 0).setDepth(11);
    this.origVarLabel2 = this.add.text(ORIG_X1 - 6, TRAY_Y0 - 30, "", { font: "italic 9px Courier New", color: HEX_GREEN_BRIGHT }).setOrigin(1, 0).setDepth(11);
    this.copyVarLabel = this.add.text(COPY_X1 - 6, TRAY_Y0 - 16, "", { font: "italic 9px Courier New", color: HEX_CYAN }).setOrigin(1, 0).setDepth(11).setAlpha(0);
    ...
    bridgeG.lineBetween(BRIDGE_X0, BRIDGE_CY + 14, BRIDGE_X1, BRIDGE_CY + 14);
    this.bridgeArrow = this.add.text((BRIDGE_X0 + BRIDGE_X1) / 2, BRIDGE_CY, "→", { font: "bold 20px Georgia", color: HEX_BRASS }).setOrigin(0.5).setDepth(10).setAlpha(0.6);
    ...
    this.add.text(DIAL_X, DIAL_Y - 34, "len", { font: "bold 10px Courier New", color: HEX_CYAN }).setOrigin(0.5).setDepth(14);
    ...
    this.dialValueText = this.add.text(DIAL_X, DIAL_Y + 30, "", { font: "bold 9px Courier New", color: HEX_CYAN }).setOrigin(0.5).setDepth(14);
  }
  ...
  async populateOriginalTray(values, type, varName) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      const idxPlate = this.add.text(cellX + cellW / 2, TRAY_Y1 - 10, `[${i}]`, { font: "bold 9px Courier New", color: HEX_BRASS }).setOrigin(0.5).setAlpha(i === 0 ? 0.9 : 0.6);
  ...
  async _buildCard(layer, comp, value, type, isGhostDefault) {
    ...
    const txt = this.add.text(0, 0, display, isGhostDefault
      ? { font: "italic 11px Courier New", color: "#78909c" }
      : { font: "bold 13px Courier New", color: "#0a1208" }).setOrigin(0.5);
  ...
  async materializeCopyTray(length, type, varName) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      const idxPlate = this.add.text(cellX + cellW / 2, TRAY_Y1 - 10, `[${i}]`, { font: "bold 9px Courier New", color: HEX_BRASS }).setOrigin(0.5).setAlpha(0.6);
  ...
  async flyGhostCopies(values, copyLength, type) {
    ...
    for (let i = 0; i < crossCount; i++) {
      ...
      const ghost = this.add.text(origComp.x + origComp.w / 2, (TRAY_Y0 + TRAY_Y1) / 2 - 6, this._fmtVal(values[i]), { font: "bold 11px Courier New", color: HEX_CYAN }).setOrigin(0.5).setDepth(17).setAlpha(0.9);
  ...
  async showTruncation(skippedIndices) {
    ...
    this.createFloatingText(ORIG_X1 - 30, TRAY_Y0 - 4, "truncated", HEX_RED, "italic 10px Arial", 1200);
  ...
  createCuratorsSlate() {
    ...
    this.add.text(SLATE_X + 14, SLATE_Y + SLATE_H - 18, "returns:", { font: "11px Georgia", color: "#8a6435" }).setDepth(11);
    this.resultText = this.add.text(SLATE_X + 70, SLATE_Y + SLATE_H - 18, "—", { font: "bold 13px Courier New", color: HEX_GRAY }).setOrigin(0, 0.5).setDepth(11);
  }
  ...
  createExpressionMonitor() {
    ...
    this.exprMonitorText = this.add.text(640, 209, "", { font: "10px Courier New", color: HEX_GRAY }).setOrigin(0.5).setDepth(51);
  }
  ...
  createHUD() {
    ...
    this.add.text(20, 14, "THE COPY BENCH", { font: "bold 14px Georgia", color: "#b0bec5" }).setDepth(50);
    this.add.text(20, 32, "Accretion Phase — Arrays Methods: copyOf()", { font: "10px Arial", color: "#546e7a" }).setDepth(50);
  ...
  createBit() {
    ...
    const stampT = this.add.text(0, -4, "DUP", { font: "bold 5px Arial", color: "#0a1208" }).setOrigin(0.5);
  ...
  showQuestionCard(promptText) {
    ...
    const badgeT = this.add.text(-248, 0, String(this.currentRound + 1), { font: "bold 12px Arial", color: "#0a1208" }).setOrigin(0.5);
    const t = this.add.text(-226, 0, promptText, { font: "13px Arial", color: "#e8eaf6", wordWrap: { width: 480 } }).setOrigin(0, 0.5);
  ...
  async onBubbleSelected(opt, config, bubbleContainer) {
    ...
    if (!this._alive) return;
    if (config.revealNote) this.createFloatingText(640, 200, config.revealNote, HEX_GRAY, "11px Arial", 3000);
    await this.delay(350);
    if (!this._alive) return;
  ...
  _drawSlotPlaceholder(slotId, config) {
    ...
    if (!filled) {
      ...
      const label = this.add.text(x + w / 2, y + h / 2, hintDef ? hintDef.hint : "", { font: "italic 8px Courier New", color: "#3d4450" }).setOrigin(0.5).setDepth(17);
      def.hintLabel = label;
  ...
  createCartridgeTray(config) {
    ...
    bdraw(false, false);
    const bt = this.add.text(0, 0, "REPLICATE", { font: "bold 12px Arial", color: "#0a1208" }).setOrigin(0.5);
    btn.add([bg, bt]);
    btn.setSize(140, 40);
  ...
  async onArrangePressed(config) {
    ...
    this.createFloatingText(640, 200, pass ? "✓" : "✗", pass ? HEX_GREEN_BRIGHT : HEX_RED, "bold 22px Arial", 900);
  ...
  showCompileErrorStamp() {
    const stamp = this.add.text(640, 230, "COMPILE ERROR", { font: "bold 16px Arial", color: HEX_RED }).setOrigin(0.5).setDepth(30).setScale(1.3).setAngle(-6).setAlpha(0);
  ...
  async crashIOOBE(idx, trayKey) {
    ...
    const stamp = this.add.text(cx, TRAY_Y0 - 16, "ArrayIndexOutOfBoundsException", { font: "bold 8px Courier New", color: HEX_RED }).setOrigin(0.5).setAngle(-3).setAlpha(0);
    this.roundElements.push(stamp);
  ...
  gameOver() {
    ...
    (async () => {
      ...
      this._makeButton(640, 420, "RESTART THE BENCH", 230, 50, { stroke: C_RED, textColor: HEX_RED }, () => this.scene.restart());
    })();
  }
  ...
  showScoreTally() {
    ...
    const badgeLbl = this.add.text(640, 505, "copyOf() SCHEMA ACQUIRED", { font: "bold 12px Georgia", color: HEX_GOLD }).setOrigin(0.5).setDepth(91).setAlpha(0);
