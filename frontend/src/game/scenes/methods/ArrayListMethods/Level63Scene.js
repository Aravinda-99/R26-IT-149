...
export class Level63Scene extends Phaser.Scene {
  ...
  createWorksInterior() {
    ...
    this._chalkFormulae = formulae.map((f) => this.add.text(f.x, f.y, f.t, { font: "italic 16px Georgia", color: "#e8eaf6" }).setAlpha(0.12).setDepth(2));
    ...
    for (let i = 0; i < 8; i++) {
      ...
      this.add.text(x, y, symbols[i % symbols.length], { font: "6px Georgia", color: "#8a6435" }).setOrigin(0.5).setAlpha(0.25).setDepth(3);
    }
    ...
    this.add.text(580, 25, "T H E   F O R M U L A   W O R K S", { font: "bold 12px Georgia", color: HEX_GOLD }).setOrigin(0.5).setAlpha(0.8).setDepth(3);

    const wcg = this.add.graphics().setDepth(3);
    wcg.lineStyle(2, C_BRASS, 1);
    wcg.strokeRect(866, 40, 28, 24);
    this.add.text(880, 46, "★abs²", { font: "5px Georgia", color: HEX_BRASS }).setOrigin(0.5).setAlpha(0.4).setDepth(3);
  }
  ...
  createWingCrest() {
    ...
    g.closePath();
    g.strokePath();
    const cradles = this.add.text(0, -8, "◡◡", { font: "8px Arial", color: HEX_BLUE_GRAY }).setOrigin(0.5).setAlpha(0.7);
    const obelisk = this.add.text(0, 0, "▲", { font: "7px Arial", color: HEX_GOLD }).setOrigin(0.5).setAlpha(0.7);
    const stages = this.add.text(0, 9, "≡", { font: "8px Arial", color: HEX_CYAN }).setOrigin(0.5).setAlpha(0.7);
  ...
  createCodeCanvas() {
    ...
    this.tabFilename = this.add.text(CX + CW - 12, CY + TAB_H / 2, "Formula1.java", { font: "11px Courier New", color: "#546e7a" }).setOrigin(1, 0.5).setDepth(11);
  ...
  createBlockPalette() {
    ...
    this.add.text(PX + 10, PY + 8, "FORMULIST'S PARTS", { font: "bold 9px Arial", color: "#3d4450" }).setDepth(11);
  ...
  createRigWindow() {
    ...
    this.add.text(OX + 10, OY + 6, "FORMULA RIG — LIVE", { font: "bold 9px Georgia", color: HEX_BRASS }).setAlpha(0.7).setDepth(11);
  ...
  createMiniCascade() {
    ...
    this.powLabel = this.add.text(MCE_CX, MCE_ENTRY_Y + 22, "Math.pow", { font: "bold 7px Courier New", color: HEX_GOLD }).setOrigin(0.5).setAlpha(0.85);
    ...
    for (let i = 0; i < MCE_STAGE_COUNT; i++) {
      ...
      const lbl = this.add.text(MCE_CX, y, "", { font: "bold 6px Courier New", color: HEX_GOLD }).setOrigin(0.5).setAlpha(0);
  ...
  _mceMakeStar(value, type, x, y, scale = 1) {
    ...
    const txt = this.add.text(0, 0, display, { font: "bold 6px Courier New", color: "#060810" }).setOrigin(0.5);
  ...
  createMiniRail() {
    ...
    this.absLabel = this.add.text(RAIL_X0 - 2, RAIL_Y - 18, "Math.abs", { font: "bold 7px Courier New", color: HEX_GOLD }).setAlpha(0.8);
  ...
  showLengthChip(midX, length, type) {
    ...
    const t = this.add.text(0, 0, display, { font: "bold 8px Courier New", color: HEX_CYAN }).setOrigin(0.5);
  ...
  async markerShudder(value, type, varName) {
    ...
    const q = this.add.text(marker.container.x, marker.container.y - 20, "?", { font: "bold 14px Georgia", color: HEX_RED }).setOrigin(0.5);
    this.rigLayer.add(q);
  ...
  createMiniAccumulationTracker() {
    const hdr = this.add.text(TRK_X, TRK_Y0 - 12, "ACCUMULATION TRACKER", { font: "bold 7px Georgia", color: HEX_BRASS }).setAlpha(0.7);
  ...
  populateMiniShelf(initialList) {
    ...
    this.currentList.forEach((entry, i) => {
      ...
      const lbl = this.add.text(0, 9, `[${i}]`, { font: "bold 5px Courier New", color: HEX_GRAY }).setOrigin(0.5);
      c.add([g, t, lbl]);
  ...
  createHUD() {
    ...
    this.add.text(20, 14, "THE FORMULA WORKS", { font: "bold 13px Georgia", color: "#b0bec5" }).setDepth(51);
    this.add.text(20, 32, "Restructuring Phase — Math Methods: pow()", { font: "10px Arial", color: "#546e7a" }).setDepth(51);
  ...
  async crashGet(index) {
    const stamp = this.add.text(OX + OW / 2, OY + 130, "IndexOutOfBoundsException", { font: "bold 7px Courier New", color: HEX_RED }).setOrigin(0.5).setAngle(-6).setAlpha(0);
  ...
  gameOver() {
    ...
    (async () => {
      ...
      this._makeButton(640, 420, "REOPEN THE WORKS", 240, 50, { stroke: C_RED, textColor: HEX_RED }, () => this.scene.restart());
    })();
  }
  ...
  async ceremonyPhase3_CentralPanel() {
    ...
    const stages = this.add.text(-14, -6, "≡", { font: "bold 12px Arial", color: HEX_GOLD }).setOrigin(0.5);
    const weight = this.add.text(0, -6, "⚖", { font: "bold 12px Arial", color: HEX_CYAN }).setOrigin(0.5);
    const compass = this.add.text(14, -6, "◺", { font: "bold 12px Arial", color: HEX_BLUE_GRAY }).setOrigin(0.5);
    badge.add([bg, stages, weight, compass]);
    this.tweens.add({ targets: badge, alpha: 1, duration: 300, delay: 2500 });
    const badgeLbl = this.add.text(640, 532, "pow() MASTERY", { font: "bold 13px Georgia", color: HEX_GOLD }).setOrigin(0.5).setDepth(91).setAlpha(0);
  ...
  async ceremonyPhase4_WingSeal() {
    ...
    const title = this.add.text(0, -18, "MATH WING — COMPLETE", { font: "bold 17px Georgia", color: HEX_GOLD }).setOrigin(0.5);
    const caption = this.add.text(0, 30, "9 levels · 3 methods · 3 instruments beneath one dome", { font: "italic 11px Georgia", color: HEX_BLUE_GRAY }).setOrigin(0.5).setAlpha(0.85);
