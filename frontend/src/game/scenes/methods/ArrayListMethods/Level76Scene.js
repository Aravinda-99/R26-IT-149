...
export class Level76Scene extends Phaser.Scene {
  ...
  createWorksInterior() {
    ...
    this.add.text(87, 148, "int", { font: "bold 7px Courier New", color: HEX_GOLD }).setOrigin(0.5).setAlpha(0.5).setDepth(3);
    this.add.text(133, 148, "double", { font: "bold 7px Courier New", color: HEX_ORANGE }).setOrigin(0.5).setAlpha(0.5).setDepth(3);
    this.add.text(87, 160, "parseInt", { font: "6px Georgia", color: HEX_GOLD }).setOrigin(0.5).setAlpha(0.4).setDepth(3);
    this.add.text(133, 160, "parseDouble", { font: "6px Georgia", color: HEX_ORANGE }).setOrigin(0.5).setAlpha(0.4).setDepth(3);
    this.add.text(110, 178, "dot in text? → double", { font: "italic 6px Georgia", color: HEX_BLUE_GRAY }).setOrigin(0.5).setAlpha(0.4).setDepth(3);
    ...
    this.add.text(590, 25, "T H E   D E C I M A L   W O R K S", { font: "bold 12px Georgia", color: HEX_ORANGE }).setOrigin(0.5).setAlpha(0.7).setDepth(3);
  }
  ...
  createQualitySeal() {
    ...
    const label = this.add.text(0, 0, "QA", { font: "bold 10px Georgia", color: HEX_CYAN }).setOrigin(0.5);
    c.add([g, label]);
    c.setAlpha(0.4);
  ...
  createCodeCanvas() {
    ...
    this.tabFilename = this.add.text(CX + CW - 12, CY + TAB_H / 2, "Measure1.java", { font: "11px Courier New", color: "#546e7a" }).setOrigin(1, 0.5).setDepth(11);
  ...
  createBlockPalette() {
    ...
    this.add.text(PX + 10, PY + 8, "ASSAYER'S PRECISION PARTS", { font: "bold 9px Arial", color: "#3d4450" }).setDepth(11);
  ...
  createRigWindow() {
    ...
    this.add.text(OX + 10, OY + 6, "WORKS RIG — LIVE", { font: "bold 9px Georgia", color: HEX_ORANGE }).setAlpha(0.7).setDepth(11);
  ...
  createDualApparatus() {
    const drawFrame = (x0, x1, label, colorHex) => {
      ...
      this.rigLayer.add(g);
      const t = this.add.text((x0 + x1) / 2, MF_Y0 - 9, label, { font: "bold 6px Georgia", color: colorHex === C_COPPER ? HEX_COPPER : HEX_ORANGE }).setOrigin(0.5).setAlpha(0.5);
      this.rigLayer.add(t);
      return { g, t };
    };
    ...
    this._mfContText = this.add.text(mfCx, MF_Y1 - 12, "int —", { font: "bold 8px Courier New", color: HEX_GRAY }).setOrigin(0.5);
    this._mcContText = this.add.text(mcCx, MF_Y1 - 12, "double —", { font: "bold 8px Courier New", color: HEX_GRAY }).setOrigin(0.5);
  ...
  async runFurnaceConversion(strValue) {
    ...
    if (!valid) {
      ...
      const nfe = this.add.text(cx, MF_Y0 + 80, "NFE", { font: "bold 8px Courier New", color: HEX_RED }).setOrigin(0.5).setAlpha(0);
    ...
    const txt = this.add.text(0, 0, String(value), { font: "bold 7px Courier New", color: "#241a0e" }).setOrigin(0.5);
    bar.add([bg, txt]);
  ...
  async runCrucibleConversion(strValue) {
    ...
    const valText = this.add.text(cx, MF_Y0 + 68, display, { font: "bold 6px Courier New", color: "#241a0e" }).setOrigin(0.5).setAlpha(0);
  ...
  async runCastAnimation(doubleValue, intResult) {
    ...
    const funnel = this.add.text((mcCx + mfCx) / 2, funnelY, "(int)", { font: "bold 7px Courier New", color: HEX_GOLD }).setOrigin(0.5).setAlpha(0);
    ...
    drop.destroy();
    const bar = this.add.text(mfCx, MF_Y0 + 68, String(intResult), { font: "bold 8px Courier New", color: HEX_GOLD }).setOrigin(0.5).setAlpha(0);
  ...
  createMiniArithmeticStage() {
    ...
    this.rigLayer.add(g);
    const t = this.add.text(STAGE_X0 + 4, STAGE_Y0 + 3, "MATH", { font: "bold 6px Georgia", color: HEX_INDIGO }).setAlpha(0.6);
    this.rigLayer.add(t);
  ...
  async runArithmeticAnimation(aVal, op, bVal, result) {
    ...
    const makeBar = (x, val) => {
      ...
      bg.fillRoundedRect(-w / 2, -h / 2, w, h, 2);
      bg.strokeRoundedRect(-w / 2, -h / 2, w, h, 2);
      const t = this.add.text(0, 0, String(val), { font: "bold 6px Courier New", color: "#241a0e" }).setOrigin(0.5);
      c.add([bg, t]);
    ...
    const opText = this.add.text(midX, cy, op, { font: "bold 11px Georgia", color: HEX_INDIGO }).setOrigin(0.5).setAlpha(0);
  ...
  createVariablesStrip() {
    const hdr = this.add.text(OX + 14, VARS_Y - 12, "VARIABLES", { font: "bold 7px Georgia", color: HEX_ORANGE }).setAlpha(0.7);
  ...
  updateVariablesStrip(vars) {
    ...
    for (const name in vars) {
      ...
      const t = this.add.text(OX + 20, y, text, { font: "bold 7px Courier New", color: v.type === "String" ? HEX_CYAN : v.type === "double" ? HEX_ORANGE : HEX_GOLD }).setOrigin(0, 0.5);
  ...
  createManifestStrip() {
    ...
    this.manifestStripText = this.add.text(OX + 8, MANIFEST_Y + 8, "", { font: "10px Arial", color: HEX_ORANGE }).setOrigin(0, 0.5).setDepth(17);
  ...
  createHUD() {
    ...
    this.add.text(20, 14, "THE DECIMAL WORKS", { font: "bold 13px Georgia", color: "#b0bec5" }).setDepth(51);
    this.add.text(20, 32, "Restructuring Phase — Type Conversion: parseDouble()", { font: "10px Arial", color: "#546e7a" }).setDepth(51);
  ...
  gameOver() {
    ...
    (async () => {
      ...
      this._makeButton(640, 420, "RESTART THE WORKS", 240, 50, { stroke: C_RED, textColor: HEX_RED }, () => this.scene.restart());
    })();
  }
  ...
  async triggerTrilogyFinale() {
    ...
    lines.forEach((s, i) => {
      const t = this.add.text(400, 140 + i * 24, s, { font: "13px Arial", color: HEX_GRAY }).setOrigin(0, 0.5).setDepth(91).setAlpha(0);
      this.tweens.add({ targets: t, alpha: 1, duration: 250, delay: 300 + i * 120 });
      this._ceremonyElements.push(t);
    });
    const totalText = this.add.text(400, 140 + 7 * 24, "TOTAL: 0", { font: "bold 20px Arial", color: HEX_GOLD }).setOrigin(0, 0.5).setDepth(91).setAlpha(0);
    ...
    const crucibleIcon = this.add.text(-14, -6, "⚗️", { font: "bold 12px Arial", color: HEX_ORANGE }).setOrigin(0.5);
    const buretteIcon = this.add.text(0, -6, "🧪", { font: "bold 12px Arial", color: HEX_GOLD }).setOrigin(0.5);
    const dualIcon = this.add.text(14, -6, "⚖️", { font: "bold 11px Arial", color: HEX_COPPER }).setOrigin(0.5);
    badge.add([bg, crucibleIcon, buretteIcon, dualIcon]);
    this.tweens.add({ targets: badge, alpha: 1, duration: 300, delay: 2450 });
    const badgeLbl = this.add.text(640, 512, "parseDouble() MASTERY", { font: "bold 13px Georgia", color: HEX_GOLD }).setOrigin(0.5).setDepth(91).setAlpha(0);
    const badgeSub = this.add.text(640, 528, "Accretion ✓  Tuning ✓  Restructuring ✓", { font: "10px Arial", color: "#78909c" }).setOrigin(0.5).setDepth(91).setAlpha(0);
    ...
    const progLabel = this.add.text(640, barY + 26, "TYPE CONVERSION WING — 2 of 3 trilogies complete", { font: "bold 11px Georgia", color: HEX_ORANGE }).setOrigin(0.5).setDepth(91).setAlpha(0);
