...
export class Level69Scene extends Phaser.Scene {
  ...
  createLeaderboard() {
    ...
    for (let i = 0; i < 5; i++) {
      const y = 58 + i * 22;
      const rank = this.add.text(216, y, `${i + 1}.`, { font: "11px Courier New", color: HEX_BLUE_GRAY }).setAlpha(0.15).setDepth(2);
      const name = this.add.text(240, y, "—————", { font: "11px Courier New", color: HEX_BLUE_GRAY }).setAlpha(0.15).setDepth(2);
      const timeT = this.add.text(700, y, "--.-s", { font: "11px Courier New", color: HEX_BLUE_GRAY }).setAlpha(0.15).setDepth(2);
  ...
  createTrialsBanner() {
    ...
    this.add.text(640, 25, "T H E   R E P L I C A T I O N   T R I A L S", { font: "bold 13px Georgia", color: HEX_BRASS }).setOrigin(0.5).setAlpha(0.7).setDepth(3);
  }
  ...
  createDuplicationOrder() {
    ...
    this.add.text(CARD_CX, CARD_Y0 + 12, "DUPLICATION ORDER — WORK NO. 1", { font: "bold 10px Georgia", color: "#8a6435" }).setOrigin(0.5).setDepth(21);
    this.cardHeaderText = this.add.text(CARD_CX, CARD_Y0 + 12, "", { font: "bold 10px Georgia", color: "#8a6435" }).setOrigin(0.5).setDepth(21);
    this.cardRoundLabel = this.add.text(CARD_X1 - 14, CARD_Y0 + 12, "WORK 1/15", { font: "bold 11px Courier New", color: "#8a6435" }).setOrigin(1, 0.5).setDepth(21);
  ...
  createMiniReplicationFrame() {
    ...
    this.origVarLabel = this.add.text(MINI_ORIG_X1 - 4, MINI_TRAY_Y0 - 12, "", { font: "italic 10px Courier New", color: HEX_CYAN }).setOrigin(1, 0).setDepth(11);
    this.origVarLabel2 = this.add.text(MINI_ORIG_X1 - 4, MINI_TRAY_Y0 - 22, "", { font: "italic 10px Courier New", color: HEX_GREEN_BRIGHT }).setOrigin(1, 0).setDepth(11);
    this.copyVarLabel = this.add.text(MINI_COPY_X1 - 4, MINI_TRAY_Y0 - 12, "", { font: "italic 10px Courier New", color: HEX_CYAN }).setOrigin(1, 0).setDepth(11).setAlpha(0);
    ...
    bridgeG.lineBetween(MINI_ORIG_X1, MINI_BRIDGE_CY + 8, MINI_COPY_X0, MINI_BRIDGE_CY + 8);
    this.bridgeArrow = this.add.text((MINI_ORIG_X1 + MINI_COPY_X0) / 2, MINI_BRIDGE_CY, "→", { font: "bold 14px Georgia", color: HEX_BRASS }).setOrigin(0.5).setDepth(10).setAlpha(0.6);
  ...
  async populateOriginalTray(values, type, varName) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      const idxPlate = this.add.text(cellX + cellW / 2, MINI_TRAY_Y1 - 6, `[${i}]`, { font: "bold 8px Courier New", color: HEX_BRASS }).setOrigin(0.5).setAlpha(0.55);
  ...
  async _buildCard(layer, comp, value, type, isGhostDefault) {
    ...
    const txt = this.add.text(0, 0, display, isGhostDefault
      ? { font: "italic 9px Courier New", color: "#78909c" }
      : { font: "bold 10px Courier New", color: "#0a1208" }).setOrigin(0.5);
  ...
  async flyGhostCopies(values, copyLength, type) {
    ...
    for (let i = 0; i < crossCount; i++) {
      ...
      const ghost = this.add.text(origComp.x + origComp.w / 2, (MINI_TRAY_Y0 + MINI_TRAY_Y1) / 2 - 3, this._fmtVal(values[i]), { font: "bold 9px Courier New", color: HEX_CYAN }).setOrigin(0.5).setDepth(17).setAlpha(0.9);
  ...
  createMiniSlate() {
    ...
    this.add.text(SLATE_X + 10, SLATE_Y + 8, "MINI SLATE", { font: "bold 10px Georgia", color: HEX_BRASS }).setDepth(11);
  ...
  createContainerShelf() {
    ...
    this.add.text(SHELF_X + 10, SHELF_Y + 6, "REFERENCES", { font: "bold 10px Georgia", color: HEX_BRASS }).setDepth(11);
  ...
  updateContainerShelfWithReferences(vars) {
    ...
    for (const name in vars) {
      ...
      if (suffix) {
        const st = this.add.text(SHELF_X + SHELF_W - 10, y, suffix, { font: "italic 9px Courier New", color: HEX_GRAY }).setOrigin(1, 0.5);
  ...
  createHUD() {
    ...
    this.add.text(20, 14, "THE REPLICATION TRIALS", { font: "bold 16px Georgia", color: "#b0bec5" }).setDepth(50);
    this.add.text(20, 32, "Tuning Phase — Arrays Methods: copyOf()", { font: "12px Arial", color: "#546e7a" }).setDepth(50);
  ...
  showCompileErrorStamp() {
    const stamp = this.add.text(CARD_CX, CARD_Y0 - 22, "COMPILE ERROR", { font: "bold 18px Arial", color: HEX_RED }).setOrigin(0.5).setDepth(30).setScale(1.3).setAngle(-6).setAlpha(0);
  ...
  async crashIOOBE(idx, trayKey) {
    ...
    const stamp = this.add.text(cx, MINI_TRAY_Y0 - 12, "IOOBE", { font: "bold 10px Courier New", color: HEX_RED }).setOrigin(0.5).setAngle(-3).setAlpha(0);
    this.roundElements.push(stamp);
  ...
  scoreForAttempt(timePctUsed) {
    ...
    if (remaining > 0.6) { points += 50; this.fastBonusCount++; this.createFloatingText(CARD_CX, CARD_Y0 - 14, "⚡ EARLY SEAL +50", HEX_GOLD, "bold 15px Arial", 900); }
  ...
  async runBehavioralCheck() {
    ...
    try {
      ...
      console.warn("Level69Scene: /api/wellbeing/predict-struggle unreachable, skipping behavioral signal for this level:", e);
    }
  }
  ...
  gameOver() {
    ...
    (async () => {
      ...
      this._makeButton(525, 420, "REFILL THE COLUMN", 200, 50, { stroke: C_RED, textColor: HEX_RED }, () => this.scene.restart());
  ...
  levelComplete() {
    ...
    try { GameManager.completeLevel(69, Math.round((this.correctFirstTry / 15) * 100)); } catch (_) {}
  ...
  showScoreTally() {
    ...
    const badgeLbl = this.add.text(640, 520, "copyOf() SCHEMA TUNED", { font: "bold 15px Georgia", color: HEX_GOLD }).setOrigin(0.5).setDepth(91).setAlpha(0);
