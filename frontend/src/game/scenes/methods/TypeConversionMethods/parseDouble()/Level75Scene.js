...
export class Level75Scene extends Phaser.Scene {
  ...
  createPeriodicPoster() {
    ...
    cells.forEach((c) => {
      ...
      this.add.text(c.x, 124, c.label, { font: "8px Courier New", color: HEX_COPPER }).setOrigin(0.5).setAlpha(0.4).setDepth(3);
    });
    this.add.text(110, 160, "parseInt · parseDouble · valueOf", { font: "7px Georgia", color: HEX_CYAN }).setOrigin(0.5).setAlpha(0.35).setDepth(3);
  }
  ...
  createTrialsBanner() {
    ...
    this.add.text(640, 25, "T H E   P R E C I S I O N   T R I A L S", { font: "bold 13px Georgia", color: HEX_ORANGE }).setOrigin(0.5).setAlpha(0.7).setDepth(3);
  }
  ...
  createPrecisionCard() {
    ...
    this.cardRoundLabel = this.add.text(CARD_X1 - 14, CARD_Y0 + 12, "SAMPLE 1/15", { font: "bold 11px Courier New", color: "#8a6435" }).setOrigin(1, 0.5).setDepth(21);
  ...
  createTitrationBurette() {
    ...
    this.endpointLabel = this.add.text(FLASK_X + FLASK_W / 2 + 2, FLASK_Y0 + (FLASK_Y1 - FLASK_Y0) * 0.15, "EP", { font: "bold 8px Georgia", color: HEX_RED }).setOrigin(0, 0.5).setDepth(11).setAlpha(0.6);
  ...
  async reachEndpoint() {
    ...
    const stamp = this.add.text(FLASK_X, FLASK_Y1 + 14, "ENDPOINT", { font: "bold 10px Courier New", color: HEX_RED }).setOrigin(0.5).setDepth(12).setAlpha(0);
  ...
  createMiniDualApparatus() {
    const drawFrame = (x0, x1, label, colorHex) => {
      ...
      const t = this.add.text((x0 + x1) / 2, MINI_Y0 - 10, label, { font: "bold 9px Georgia", color: colorHex === C_COPPER ? HEX_COPPER : HEX_ORANGE }).setOrigin(0.5).setDepth(11).setAlpha(0.5);
      return { g, t };
    };
    ...
    this._mfContText = this.add.text(mfCx, MINI_Y1 - 14, "int —", { font: "bold 11px Courier New", color: HEX_GRAY }).setOrigin(0.5).setDepth(12);
    this._mcContText = this.add.text(mcCx, MINI_Y1 - 14, "double —", { font: "bold 11px Courier New", color: HEX_GRAY }).setOrigin(0.5).setDepth(12);
  }
  ...
  async runMiniFurnaceConversion(strValue) {
    ...
    if (!valid) {
      ...
      const nfe = this.add.text(cx, MINI_Y0 + 110, "NFE", { font: "bold 12px Courier New", color: HEX_RED }).setOrigin(0.5).setDepth(22).setAlpha(0);
    ...
    const txt = this.add.text(0, 0, String(value), { font: "bold 10px Courier New", color: "#241a0e" }).setOrigin(0.5);
    bar.add([bg, txt]);
  ...
  async runMiniCrucibleConversion(strValue) {
    ...
    const valText = this.add.text(cx, MINI_Y0 + 95, display, { font: "bold 9px Courier New", color: "#241a0e" }).setOrigin(0.5).setDepth(22).setAlpha(0);
  ...
  createTrialSlate() {
    ...
    this.add.text(SLATE_X + 10, SLATE_Y + 8, "TRIAL SLATE", { font: "bold 10px Georgia", color: HEX_ORANGE }).setDepth(11);
  ...
  createContainerShelf() {
    ...
    this.add.text(SHELF_X + 10, SHELF_Y + 6, "VARIABLES", { font: "bold 10px Georgia", color: HEX_ORANGE }).setDepth(11);
  ...
  createHUD() {
    ...
    this.add.text(20, 14, "THE PRECISION TRIALS", { font: "bold 16px Georgia", color: "#b0bec5" }).setDepth(50);
    this.add.text(20, 32, "Tuning Phase — Type Conversion: parseDouble()", { font: "12px Arial", color: "#546e7a" }).setDepth(50);
  ...
  scoreForAttempt(timePctUsed) {
    ...
    if (remaining > 0.6) { points += 50; this.fastBonusCount++; this.createFloatingText(CARD_CX, CARD_Y0 - 14, "⚡ EARLY CLOSE +50", HEX_GOLD, "bold 15px Arial", 900); }
  ...
  async runBehavioralCheck() {
    ...
    try {
      ...
      console.warn("Level75Scene: /api/wellbeing/predict-struggle unreachable, skipping behavioral signal for this level:", e);
    }
  }
  ...
  gameOver() {
    ...
    (async () => {
      ...
      this.add.text(640, 350, `Trials Calibrated: ${this.currentRound} / 15`, { font: "18px Arial", color: HEX_GRAY }).setOrigin(0.5).setDepth(91);
      this._makeButton(525, 420, "REFILL THE BURETTE", 200, 50, { stroke: C_RED, textColor: HEX_RED }, () => this.scene.restart());
  ...
  levelComplete() {
    ...
    try { GameManager.completeLevel(75, Math.round((this.correctFirstTry / 15) * 100)); } catch (_) {}
  ...
  showScoreTally() {
    ...
    const badgeLbl = this.add.text(640, 520, "parseDouble() SCHEMA TUNED", { font: "bold 15px Georgia", color: HEX_GOLD }).setOrigin(0.5).setDepth(91).setAlpha(0);
