...
export class Level58Scene extends Phaser.Scene {
  ...
  createHallInterior() {
    ...
    this.add.text(630, 25, "T H E   D I S T A N C E   H A L L", { font: "bold 15px Georgia", color: HEX_BRASS }).setOrigin(0.5).setAlpha(0.7).setDepth(3);
  }
  ...
  createMeasuringRail() {
    ...
    this.add.text(NP_X, NP_Y - 8, "Math", { font: "bold 18px Courier New", color: HEX_GOLD }).setOrigin(0.5).setDepth(7);
    this.methodPlateText = this.add.text(NP_X, NP_Y + 8, ".abs", { font: "bold 14px Courier New", color: HEX_CYAN }).setOrigin(0.5).setDepth(7);
    ...
    const zeroLabel = this.add.text(0, 16, "0", { font: "bold 16px Courier New", color: HEX_GOLD }).setOrigin(0.5);
  ...
  rescaleRail(values) {
    ...
    for (let v = Math.ceil(lo / tickStep) * tickStep; v <= hi; v += tickStep) {
      ...
      if (n % 2 === 0) {
        const lbl = this.add.text(x, RAIL_Y + 22, this._fmtNum(v), { font: "bold 13px Courier New", color: HEX_BLUE_GRAY }).setOrigin(0.5).setAlpha(0.7);
  ...
  _makeMarker(value, type, size, dashed) {
    ...
    const txt = this.add.text(0, -size / 2 - 14, display, { font: "bold 17px Courier New", color: type === "double" ? HEX_ORANGE : HEX_GOLD }).setOrigin(0.5);
    c.add([g, txt]);
  ...
  showLengthChip(midX, length, type) {
    ...
    const t = this.add.text(0, 0, display, { font: "bold 18px Courier New", color: HEX_CYAN }).setOrigin(0.5);
    c.add([g, t]);
  ...
  async runDifferenceMeasurement(aRes, bRes) {
    ...
    this.createFloatingText(640, RAIL_Y - 110, annoText, HEX_BLUE_GRAY, "bold 15px Courier New", 900);
  ...
  async deliverToVariable(name, value, type) {
    ...
    const display = type === "double" ? Number(value).toFixed(1) : String(value);
    const valT = this.add.text(0, 0, display, { font: "bold 14px Courier New", color: type === "double" ? HEX_ORANGE : HEX_GOLD }).setOrigin(0.5);
    c.add([g, nameT, valT]);
  ...
  async markerShudder(value, type, varName) {
    ...
    const q = this.add.text(marker.container.x, marker.container.y - 30, "?", { font: "bold 21px Georgia", color: HEX_RED }).setOrigin(0.5).setDepth(20).setAlpha(0);
  ...
  createSurveyorsSlate() {
    ...
    this.add.text(SLATE_X + 14, SLATE_Y + 16, "SURVEYOR'S SLATE", { font: "bold 12px Georgia", color: HEX_BRASS }).setDepth(11);
  ...
  chalkEvaluationArrow(value, type) {
    const display = type === "double" ? Number(value).toFixed(1) : String(value);
    const t = this.add.text(SLATE_X + 14, this._slateY, `→ ${display}`, { font: "bold 15px Courier New", color: type === "double" ? HEX_ORANGE : HEX_GOLD }).setAlpha(0);
    this.slateLines.add(t);
  ...
  createHUD() {
    ...
    this.add.text(20, 14, "THE DISTANCE HALL", { font: "bold 16px Georgia", color: "#b0bec5" }).setDepth(50);
    this.add.text(20, 32, "Accretion Phase — Math Methods: abs()", { font: "12px Arial", color: "#546e7a" }).setDepth(50);
  ...
  async onBubbleSelected(opt, config, bubbleContainer) {
    ...
    if (!this._alive) return;
    if (config.revealNote) this.createFloatingText(640, RAIL_Y - 150, config.revealNote, HEX_GRAY, "13px Arial", 2800);
  ...
  createCartridgeTray(config) {
    ...
    bdraw(false, false);
    const bt = this.add.text(0, 0, "SURVEY", { font: "bold 16px Arial", color: "#060810" }).setOrigin(0.5);
    btn.add([bg, bt]);
    btn.setSize(130, 44);
  ...
  async runBehavioralCheck() {
    ...
    try {
      ...
      console.warn("Level58Scene: /api/wellbeing/predict-struggle unreachable, skipping behavioral signal for this level:", e);
    }
  }
  ...
  gameOver() {
    ...
    (async () => {
      ...
      this._makeButton(525, 420, "RETURN TO THE OBELISK", 200, 50, { stroke: C_RED, textColor: HEX_RED }, () => this.scene.restart());
