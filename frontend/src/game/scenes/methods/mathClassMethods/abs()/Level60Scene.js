...
export class Level60Scene extends Phaser.Scene {
  ...
  createOfficeInterior() {
    ...
    certXs.forEach((cx) => {
      ...
      const check = this.add.text(cx + 40, 98, "", { font: "bold 12px Arial", color: HEX_GREEN_BRIGHT }).setOrigin(0.5).setDepth(3);
    ...
    this.add.text(95, 388, "PASS", { font: "bold 9px Georgia", color: HEX_BRASS }).setOrigin(0.5).setAlpha(0.4).setDepth(3);
    this.add.text(95, 436, "FAIL", { font: "bold 9px Georgia", color: HEX_BRASS }).setOrigin(0.5).setAlpha(0.4).setDepth(3);
    ...
    this.add.text(640, 25, "T H E   S T A N D A R D S   O F F I C E", { font: "bold 14px Georgia", color: HEX_BRASS }).setOrigin(0.5).setAlpha(0.7).setDepth(3);
  }
  ...
  createCertificationSeal() {
    ...
    const star = this.add.text(0, 0, "★", { font: "12px Arial", color: HEX_BLUE_GRAY }).setOrigin(0.5).setAlpha(0.6);
    const t1 = this.add.text(0, -8, "STANDARDS", { font: "bold 8px Georgia", color: HEX_BLUE_GRAY }).setOrigin(0.5).setAlpha(0.5);
    const t2 = this.add.text(0, 8, "OFFICE", { font: "bold 8px Georgia", color: HEX_BLUE_GRAY }).setOrigin(0.5).setAlpha(0.5);
    c.add([g, star, t1, t2]);
    c.setAlpha(0.4);
  ...
  createCodeCanvas() {
    ...
    this.tabFilename = this.add.text(CX + CW - 12, CY + TAB_H / 2, "Std1.java", { font: "13px Courier New", color: "#546e7a" }).setOrigin(1, 0.5).setDepth(11);
  ...
  createBlockPalette() {
    ...
    this.add.text(PX + 10, PY + 8, "CLERK'S INSTRUMENTS", { font: "bold 11px Arial", color: "#3d4450" }).setDepth(11);
  ...
  createRigWindow() {
    ...
    this.add.text(OX + 10, OY + 6, "STANDARDS RIG — LIVE", { font: "bold 11px Georgia", color: HEX_BRASS }).setAlpha(0.7).setDepth(11);
  ...
  createMiniRail() {
    ...
    this.absLabel = this.add.text(RAIL_X0 - 2, RAIL_Y - 24, "Math.abs", { font: "bold 11px Courier New", color: HEX_GOLD }).setAlpha(0.8);
  ...
  rescaleRail(values) {
    ...
    for (let v = Math.ceil(lo / tickStep) * tickStep; v <= hi; v += tickStep) {
      ...
      if (n % 2 === 0) {
        const lbl = this.add.text(x, RAIL_Y + 14, this._fmtNum(v), { font: "bold 10px Courier New", color: HEX_BLUE_GRAY }).setOrigin(0.5).setAlpha(0.7);
  ...
  _makeMarker(value, type, size, dashed) {
    ...
    const txt = this.add.text(0, -size / 2 - 10, display, { font: "bold 11px Courier New", color: type === "double" ? HEX_ORANGE : HEX_GOLD }).setOrigin(0.5);
    c.add([g, txt]);
  ...
  showLengthChip(midX, length, type) {
    ...
    const t = this.add.text(0, 0, display, { font: "bold 13px Courier New", color: HEX_CYAN }).setOrigin(0.5);
    c.add([g, t]);
  ...
  async checkAgainstFence(chip, chipValue) {
    ...
    const mark = this.add.text(chip.x, chip.y - 16, pass ? "✓" : "✗", { font: "bold 14px Arial", color: pass ? HEX_GREEN_BRIGHT : HEX_RED }).setOrigin(0.5).setAlpha(0);
  ...
  async runDifferenceMeasurement(aRes, bRes) {
    ...
    this.createFloatingText(OX + OW / 2, RAIL_Y - 60, annoText, HEX_BLUE_GRAY, "bold 13px Courier New", 800);
  ...
  async markerShudder(value, type, varName) {
    ...
    const q = this.add.text(marker.container.x, marker.container.y - 24, "?", { font: "bold 18px Georgia", color: HEX_RED }).setOrigin(0.5);
    this.rigLayer.add(q);
  ...
  createMiniComparator() {
    ...
    this.mcModeText = this.add.text(MC_BEAM_CX, MC_BEAM_Y - 14, "Math.max/min", { font: "bold 10px Courier New", color: HEX_CYAN }).setOrigin(0.5);
  ...
  _mcMakeStar(value, type, x, y) {
    ...
    g.strokePoints(pts, true);
    const display = type === "double" ? Number(value).toFixed(1) : String(value);
    const txt = this.add.text(0, 0, display, { font: "bold 9px Courier New", color: "#060810" }).setOrigin(0.5);
  ...
  createMiniDeviationTracker() {
    const hdr = this.add.text(TRK_X, TRK_Y0 - 12, "DEVIATION TRACKER", { font: "bold 9px Georgia", color: HEX_BRASS }).setAlpha(0.7);
  ...
  async retrieveGhost(index) {
    ...
    const t = this.add.text(0, 0, String(entry.value), { font: "bold 8px Courier New", color: "#0a0704" }).setOrigin(0.5);
    ghost.add([g, t]);
  ...
  createHUD() {
    ...
    this.add.text(20, 14, "THE STANDARDS OFFICE", { font: "bold 15px Georgia", color: "#b0bec5" }).setDepth(51);
    this.add.text(20, 32, "Restructuring Phase — Math Methods: abs()", { font: "12px Arial", color: "#546e7a" }).setDepth(51);
  ...
  async crashGet(index) {
    const stamp = this.add.text(OX + OW / 2, OY + 130, "IndexOutOfBoundsException", { font: "bold 10px Courier New", color: HEX_RED }).setOrigin(0.5).setAngle(-6).setAlpha(0);
  ...
  async runBehavioralCheck() {
    ...
    try {
      ...
      console.warn("Level60Scene: /api/wellbeing/predict-struggle unreachable, skipping behavioral signal for this level:", e);
    }
  }
  ...
  gameOver() {
    ...
    (async () => {
      ...
      this.add.text(640, 350, `Missions Certified: ${this.currentMission} / ${MISSIONS.length}`, { font: "18px Arial", color: HEX_GRAY }).setOrigin(0.5).setDepth(91);
      this._makeButton(525, 420, "RE-OPEN THE OFFICE", 200, 50, { stroke: C_RED, textColor: HEX_RED }, () => this.scene.restart());
  ...
  showScoreTally() {
    ...
    const badgeLbl = this.add.text(640, 462, "abs() MASTERY", { font: "bold 15px Georgia", color: HEX_GOLD }).setOrigin(0.5).setDepth(91).setAlpha(0);
    ...
    const progressLbl = this.add.text(640, barY + 26, "MATH WING — 2 of 3 trilogies complete", { font: "bold 13px Georgia", color: HEX_CYAN }).setOrigin(0.5).setDepth(91).setAlpha(0);
