...
export class Level61Scene extends Phaser.Scene {
  ...
  createTowerInterior() {
    ...
    this.add.text(200, 25, "T H E   P O W E R   T O W E R", { font: "bold 16px Georgia", color: HEX_BRASS }).setOrigin(0.5).setAlpha(0.7).setDepth(3);
  }
  ...
  createCascadeEngine() {
    ...
    this.add.text(ENGINE_CX, BASE_PLATE_Y + 34, "MATH — EST. FOREVER", { font: "bold 9px Georgia", color: HEX_BRASS }).setOrigin(0.5).setAlpha(0.5).setDepth(5);
    ...
    this.add.text(ENGINE_CX, BASE_PLATE_Y - 26, "Math", { font: "bold 18px Courier New", color: HEX_GOLD }).setOrigin(0.5).setDepth(6);
    this.methodPlateText = this.add.text(ENGINE_CX, BASE_PLATE_Y - 8, ".pow", { font: "bold 14px Courier New", color: HEX_CYAN }).setOrigin(0.5).setDepth(6);
    ...
    this.add.text(ENGINE_CX, ENTRY_PORT_Y + 18, "base", { font: "italic 11px Georgia", color: HEX_BRASS }).setOrigin(0.5).setAlpha(0.6).setDepth(4);
    ...
    for (let i = 0; i < STAGE_COUNT; i++) {
      ...
      const insc = this.add.text(ENGINE_CX, y, "", { font: "bold 13px Courier New", color: HEX_GOLD }).setOrigin(0.5).setDepth(6).setAlpha(0);
      this.stageInscriptions.push(insc);

      const label = this.add.text(ENGINE_CX + 82, y, "", { font: "bold 11px Courier New", color: HEX_BLUE_GRAY }).setOrigin(0, 0.5).setDepth(6).setAlpha(0);
  ...
  createExponentDial() {
    ...
    for (let i = 0; i <= 6; i++) {
      ...
      this.add.text(lp.x, lp.y, String(i), { font: "10px Courier New", color: HEX_BLUE_GRAY }).setOrigin(0.5).setDepth(5);
    }
    ...
    this.add.text(DIAL_X, DIAL_Y + DIAL_R + 22, "exp", { font: "bold 13px Courier New", color: HEX_CYAN }).setOrigin(0.5).setDepth(5);
  }
  ...
  _makeStar(value, type, x, y, scale = 1) {
    ...
    const txt = this.add.text(0, 0, display, { font: "bold 13px Courier New", color: "#060810" }).setOrigin(0.5);
  ...
  async deliverToVariable(name, value, type) {
    ...
    const valT = this.add.text(0, 0, display, { font: "bold 14px Courier New", color: hexColor }).setOrigin(0.5);
    c.add([g, nameT, valT]);
  ...
  async entryPortShudder(value, type) {
    ...
    const q = this.add.text(star.container.x, star.container.y - 24, "?", { font: "bold 19px Georgia", color: HEX_RED }).setOrigin(0.5).setDepth(20).setAlpha(0);
  ...
  showCompileErrorStamp() {
    const stamp = this.add.text(ENGINE_CX, 300, "COMPILE ERROR", { font: "bold 21px Arial", color: HEX_RED }).setOrigin(0.5).setDepth(30).setScale(1.5).setAngle(-6).setAlpha(0);
  ...
  createEngineersSlate() {
    ...
    this.add.text(SLATE_X + 14, SLATE_Y + 16, "ENGINEER'S SLATE", { font: "bold 12px Georgia", color: HEX_BRASS }).setDepth(11);
    this.slateLines = this.add.container(0, 0).setDepth(11);
    this._slateY = SLATE_Y + 42;

    this.add.text(SLATE_X + 14, SLATE_Y + SLATE_H - 24, "result (double):", { font: "13px Georgia", color: "#8a6435" }).setDepth(11);
    this.resultText = this.add.text(SLATE_X + 130, SLATE_Y + SLATE_H - 24, "—", { font: "bold 16px Courier New", color: HEX_GRAY }).setOrigin(0, 0.5).setDepth(11);
  }
  ...
  chalkExpandedForm(base, exp) {
    ...
    const t = this.add.text(SLATE_X + 14, this._slateY, text, { font: "bold 14px Courier New", color: HEX_BLUE_GRAY }).setAlpha(0);
    this.slateLines.add(t);
  ...
  chalkEvaluationArrow(value, type = "double") {
    const display = type === "double" ? this._fmtDoubleForPrint(Number(value)) : String(value);
    const t = this.add.text(SLATE_X + 14, this._slateY, `→ ${display}`, { font: "bold 15px Courier New", color: HEX_ORANGE }).setAlpha(0);
    this.slateLines.add(t);
  ...
  showQuestionCard(promptText) {
    ...
    const badgeT = this.add.text(-220, 0, String(this.currentRound + 1), { font: "bold 15px Arial", color: "#060810" }).setOrigin(0.5);
    const t = this.add.text(-195, 0, promptText, { font: "16px Arial", color: "#e8eaf6", wordWrap: { width: 420 } }).setOrigin(0, 0.5);
  ...
  async onBubbleSelected(opt, config, bubbleContainer) {
    ...
    if (!this._alive) return;
    if (config.revealNote) this.createFloatingText(ENGINE_CX, 60, config.revealNote, HEX_GRAY, "13px Arial", 2800);
  ...
  createCartridgeTray(config) {
    ...
    bdraw(false, false);
    const bt = this.add.text(0, 0, "ENGAGE", { font: "bold 16px Arial", color: "#060810" }).setOrigin(0.5);
    btn.add([bg, bt]);
    btn.setSize(130, 44);
  ...
  async onEngagePressed(config) {
    ...
    for (let ti = 0; ti < tests.length; ti++) {
      ...
      if (tests.length > 1) this.createFloatingText(ENGINE_CX, 60, `TEST ${ti + 1}`, HEX_BRASS, "bold 14px Courier New", 1200);
      ...
      this.createFloatingText(PLINTH_X, PLINTH_Y - 60, pass ? "✓" : "✗", pass ? HEX_GREEN_BRIGHT : HEX_RED, "bold 23px Arial", 900);
  ...
  gameOver() {
    ...
    (async () => {
      ...
      this._makeButton(525, 420, "RE-ENTER THE TOWER", 200, 50, { stroke: C_RED, textColor: HEX_RED }, () => this.scene.restart());
  ...
  showScoreTally() {
    ...
    const badgeLbl = this.add.text(640, 505, "pow() SCHEMA ACQUIRED", { font: "bold 15px Georgia", color: HEX_GOLD }).setOrigin(0.5).setDepth(91).setAlpha(0);
